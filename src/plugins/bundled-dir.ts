...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "kibo-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.KIBO_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "kibo.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.KIBO_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveKiboPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveKiboPackageRootSync({ argv1: process.argv[1] }),
