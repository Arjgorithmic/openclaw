...
function resolveKimiBaseUrl(kimi?: KimiConfig, kiboConfig?: KiboConfig): string {
  ...
  const moonshotBaseUrl = kiboConfig?.models?.providers?.moonshot?.baseUrl;
...
function createKimiToolDefinition(
  ...
  kiboConfig: KiboConfig | undefined,
  ...
  return {
    ...
    execute: async (args) => {
      ...
      const baseUrl = resolveKimiBaseUrl(kimiConfig, kiboConfig);
