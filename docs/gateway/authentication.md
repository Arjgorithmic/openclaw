...
Kibo supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Kibo also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `kibo gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.kibo/.env` so the daemon can read it:
...
API keys for daemon use: `kibo onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.kibo/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in Kibo as a supported token
path. Anthropic staff has since told us that Kibo-style Claude CLI usage is
...
```bash
kibo models auth paste-token --provider openrouter
```
...
- If auth exists but Kibo cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Kibo integration path is allowed again.
- Kibo therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `KIBO_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Kibo retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `kibo models status --probe` shows omitted
...
Run `kibo models status` to confirm which profile is expiring. If an
