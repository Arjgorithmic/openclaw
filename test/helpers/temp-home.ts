...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked KIBO_HOME.
  delete process.env.KIBO_HOME;
  process.env.KIBO_STATE_DIR = path.join(base, ".kibo");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "kibo-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".kibo", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
