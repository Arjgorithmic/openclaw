...
function isLikelyKiboCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "kibo" ||
    entryBasename === "kibo.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Kibo CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureKiboModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyKiboCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): KiboConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureKiboModelsJson(cfg);
    } catch {
