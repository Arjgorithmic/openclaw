---
summary: "CLI reference for `kibo message` (send + channel actions)"
read_when:
...
- On cache miss, Kibo will attempt a live directory lookup when the provider supports it.
...
- `kibo message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Kibo does not force a `default` account SecretRef scope
...
```
kibo message send --channel discord \
...
```bash
kibo message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
kibo message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
kibo message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
kibo message poll --channel msteams \
...
```
kibo message react --channel slack \
...
```
kibo message react --channel signal \
...
```
kibo message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
kibo message send --channel telegram --target @mychat \
