---
summary: "Kibo Gateway CLI (`kibo gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Kibo’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `kibo gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.kibo/kibo.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
- `kibo onboard --mode local` and `kibo setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
- `--token <token>`: token override (also sets `KIBO_GATEWAY_TOKEN` for the process).
...
```bash
kibo gateway health --url ws://127.0.0.1:18789
```
...
```bash
kibo gateway usage-cost
kibo gateway usage-cost --days 7
kibo gateway usage-cost --json
```
...
kibo gateway status --require-rpc
```
...
```bash
kibo gateway call status
kibo gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
- For password auth on `gateway run`, prefer `KIBO_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
- In inferred auth mode, shell-only `KIBO_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
- If both `gateway.auth.token` and `gateway.auth.password` are configured and `gateway.auth.mode` is unset, install is blocked until mode is set explicitly.
- Lifecycle commands accept `--json` for scripting.

## Discover gateways (Bonjour)

`gateway discover` scans for Gateway beacons (`_kibo-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `kibo.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
kibo gateway discover --timeout 4000
kibo gateway discover --json | jq '.beacons[].wsUrl'
```
