...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Kibo Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`kibo acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Kibo channel
conversations instead of hosting an ACP harness session, use
[`kibo mcp serve`](/cli/mcp) instead.
...
- Kibo acts as an ACP server
- an IDE or ACP client connects to Kibo
- Kibo forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Kibo runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Kibo: use `kibo acp`
- Kibo should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Kibo gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
kibo acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
kibo acp --url wss://gateway-host:18789 --token-file ~/.kibo/gateway.token

# Attach to an existing session key
kibo acp --session agent:main:main

# Attach by label (must already exist)
kibo acp --session-label "support inbox"

# Reset the session key before the first prompt
kibo acp --session agent:main:main --reset-session
```
...
kibo acp client --server-args --url wss://gateway-host:18789 --token-file ~/.kibo/gateway.token

# Override the server command (default: kibo)
kibo acp client --server "node" --server-args kibo.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Kibo through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Kibo Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `kibo acp` over stdio.

Example config (persisted):

```bash
kibo config set gateway.remote.url wss://gateway-host:18789
kibo config set gateway.remote.token <token>
```
...
```bash
kibo acp --session agent:main:main
kibo acp --session agent:design:main
kibo acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Kibo plugin tools, enable the
...
Kibo bot over ACP, use `acpx` with its built-in `kibo` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx kibo` at `kibo acp`.
3. Target the Kibo session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Kibo ACP session
acpx kibo exec "Summarize the active Kibo session state."

# Persistent named session for follow-up turns
acpx kibo sessions ensure --name codex-bridge
acpx kibo -s codex-bridge --cwd /path/to/repo \
  "Ask my Kibo work agent for recent context relevant to this repo."
```

If you want `acpx kibo` to target a specific Gateway and session key every
time, override the `kibo` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "kibo": {
      "command": "env KIBO_HIDE_BANNER=1 KIBO_SUPPRESS_NOTES=1 kibo acp --url ws://127.0.0.1:18789 --token-file ~/.kibo/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Kibo checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env KIBO_HIDE_BANNER=1 KIBO_SUPPRESS_NOTES=1 node kibo.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Kibo agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Kibo ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`KIBO_GATEWAY_TOKEN`, `KIBO_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`KIBO_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `KIBO_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `kibo acp client` sets `KIBO_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `kibo`).
