...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: KiboPluginServiceContext): Promise<void> {
      if (process.env.KIBO_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (KIBO_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (process.env.KIBO_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: KiboPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
