...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.kibo/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "kibo doctor" and "kibo approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
