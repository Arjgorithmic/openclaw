...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.kiboCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.kiboCanvasA2UIAction &&
        typeof window.kiboCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.kiboSendUserAction === "function";
  ...
  window.addEventListener("kibo:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Kibo node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.kiboSendUserAction === "function"
        ? window.kiboSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.KIBO_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.KIBO_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Kibo Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
