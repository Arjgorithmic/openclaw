...
describe("usage-format", () => {
  const originalAgentDir = process.env.KIBO_AGENT_DIR;
  let agentDir: string;

  beforeEach(async () => {
    agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-usage-format-"));
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.KIBO_AGENT_DIR = originalAgentDir;
    }
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  ...
  it("prefers models.json pricing over kibo config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to kibo config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
