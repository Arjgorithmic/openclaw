...
import type { KiboPluginDefinition, KiboPluginModule } from "./types.js";
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    ["kibo/plugin-sdk"]: _ignoredLegacyRootAlias,
    ["@kibo/plugin-sdk"]: _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as KiboPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getJiti(safeSource)(safeSource) as KiboPluginModule;
    } catch (error) {
