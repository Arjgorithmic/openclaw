...
@Suite struct WakeWordGateTests {
    @Test func matchRequiresGapAfterTrigger() {
        let transcript = "hey kibo do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["kibo"], minPostTriggerGap: 0.3)
    ...
    @Test func matchHandlesMultiWordTriggers() {
        ...
        let config = WakeWordGateConfig(triggers: ["hey kibo"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
    ...
    @Test func commandTextHandlesForeignRangeIndices() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "kibo", start: 0.2, duration: 0.1, range: transcript.range(of: "kibo")),
