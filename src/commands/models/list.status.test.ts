...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveKiboAgentDir: vi.fn().mockReturnValue("/tmp/kibo-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/kibo-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/kibo-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/kibo-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/kibo-dev/kibo.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveKiboAgentDir: mocks.resolveKiboAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/kibo-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveKiboAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/kibo-dev/kibo.json");
    expect(payload.auth.storePath).toBe("/tmp/kibo-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/kibo-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/kibo-agent-custom");
