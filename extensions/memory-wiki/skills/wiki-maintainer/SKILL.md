...
description: Maintain the Kibo memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `kibo wiki ingest`, `kibo wiki compile`, and `kibo wiki lint` as the default maintenance loop.
- In `bridge` mode, run `kibo wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `kibo wiki unsafe-local import` only when the user explicitly opted into private local path access.
