...
const KIBO_ATTRIBUTION_PRODUCT = "Kibo";
const KIBO_ATTRIBUTION_ORIGINATOR = "kibo";
...
function formatKiboUserAgent(version: string): string {
  return `${KIBO_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: KIBO_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: KIBO_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatKiboUserAgent(identity.version),
    },
  };
}
