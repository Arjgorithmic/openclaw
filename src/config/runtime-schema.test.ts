...
import type { ConfigFileSnapshot, KiboConfig } from "./types.js";

const mockLoadConfig = vi.hoisted(() => vi.fn<() => KiboConfig>());
...
function makeSnapshot(params: { valid: boolean; config?: KiboConfig }): ConfigFileSnapshot {
  return {
    path: "/tmp/kibo.json",
    exists: true,
    raw: "{}",
