    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kibo nodes status", "List known nodes with live status."],
          ["kibo nodes pairing pending", "Show pending node pairing requests."],
          [
            'kibo nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["kibo nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.kibo.ai/cli/nodes")}\n`,
    );
