...
vi.mock("kibo/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: kibo", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "kibo" });
  });

  it("runTavilyExtract sends X-Client-Source: kibo", async () => {
