...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.KIBO_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveKiboPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    KIBO_STATE_DIR: path.join(os.tmpdir(), "kibo-config-doc-baseline-state"),
    KIBO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
