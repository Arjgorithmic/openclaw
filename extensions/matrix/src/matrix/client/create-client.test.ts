...
describe("createMatrixClient", () => {
  const storagePaths = {
    rootDir: "/tmp/kibo-matrix-create-client-test",
    storagePath: "/tmp/kibo-matrix-create-client-test/storage.json",
    recoveryKeyPath: "/tmp/kibo-matrix-create-client-test/recovery.key",
    idbSnapshotPath: "/tmp/kibo-matrix-create-client-test/idb.snapshot",
    metaPath: "/tmp/kibo-matrix-create-client-test/storage-meta.json",
    accountKey: "default",
  ...
  it("persists storage metadata by default", async () => {
    ...
    expect(MatrixClientMock).toHaveBeenCalledWith("https://matrix.example.org", "tok", {
      ...
      cryptoDatabasePrefix: "kibo-matrix-default-token-hash",
