...
type DriverMode = "token" | "webhook" | "kibo";
...
function resolveStateDir(): string {
  const override = process.env.KIBO_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.KIBO_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".kibo");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver kibo] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Kibo spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|kibo> Driver transport mode (default: token)\n" +
    ...
    "  --kibo-bin <path>        Kibo CLI binary for driver=kibo (default: kibo)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  KIBO_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  KIBO_DISCORD_SMOKE_DRIVER\n" +
    "  KIBO_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  KIBO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  KIBO_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  KIBO_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  KIBO_DISCORD_SMOKE_AGENT\n" +
    "  KIBO_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  KIBO_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  KIBO_DISCORD_SMOKE_POLL_MS\n" +
    "  KIBO_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  KIBO_DISCORD_SMOKE_KIBO_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.KIBO_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.KIBO_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "kibo"
        ? "kibo"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.KIBO_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.KIBO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.KIBO_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.KIBO_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.KIBO_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.KIBO_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.KIBO_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.KIBO_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.KIBO_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.KIBO_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const kiboBin =
    resolveArg("--kibo-bin") || process.env.KIBO_DISCORD_SMOKE_KIBO_BIN || "kibo";
...
async function kiboCliJson<T>(params: { kiboBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.kiboBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`kibo ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithKibo(params: {
  kiboBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await kiboCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    kiboBin: params.kiboBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "kibo") {
    return await readMessagesWithKibo({
      kiboBin: params.args.kiboBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `kibo-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("kibo message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "kibo"
            ? await readMessagesWithKibo({
                kiboBin: args.kiboBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Kibo.`,
        diagnostics: {
          bindingCandidates: [
            {
