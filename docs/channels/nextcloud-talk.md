...
Nextcloud Talk ships as a bundled plugin in current Kibo releases, so
...
```bash
kibo plugins install @kibo/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
kibo plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Kibo" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `kibo pairing list nextcloud-talk`
  - `kibo pairing approve nextcloud-talk <CODE>`
