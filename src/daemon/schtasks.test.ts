...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when KIBO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".kibo", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when KIBO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", KIBO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".kibo-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers KIBO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        KIBO_PROFILE: "rescue",
        KIBO_STATE_DIR: "C:\\State\\kibo",
      },
      expected: path.join("C:\\State\\kibo", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", KIBO_PROFILE: "default" },
      expected: path.join("/home/test", ".kibo", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\kibo",
          environment: {
            NODE_ENV: "production",
            KIBO_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\kibo\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\kibo\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Kibo Share\\node.exe" "\\\\fileserver\\Kibo Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Kibo Share\\node.exe",
            "\\\\fileserver\\Kibo Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from KIBO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ KIBO_STATE_DIR: path.join(tmpDir, "custom-state") }),
