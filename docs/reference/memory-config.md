...
This page lists every configuration knob for Kibo memory search. For
...
`kibo.json` unless noted otherwise.
...
When `provider` is not set, Kibo selects the first available:
...
If Kibo runs on EC2 with a Bedrock-enabled instance role, just set the
provider and model:
...
When sqlite-vec is unavailable, Kibo falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.kibo/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
Kibo prefers the current QMD collection and MCP query shapes, but keeps
older QMD releases working by falling back to legacy `--mask` collection flags
and older MCP tool names when needed.

QMD model overrides stay on the QMD side, not Kibo config. If you need to
