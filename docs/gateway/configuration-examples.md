---
summary: "Schema-accurate configuration examples for common Kibo setups"
read_when:
  - Learning how to configure Kibo
...
```json5
{
  agent: { workspace: "~/.kibo/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.kibo/kibo.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.kibo/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/kibo" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.kibo/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
