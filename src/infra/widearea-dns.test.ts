...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "kibo.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Kibo)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "kibo.internal", expected: "kibo.internal." },
    { value: "kibo.internal.", expected: "kibo.internal." },
    { value: "  kibo.internal  ", expected: "kibo.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { KIBO_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { KIBO_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("kibo.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "kibo.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN kibo.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_kibo-gw._tcp IN PTR studio-london._kibo-gw._tcp`,
      `studio-london._kibo-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Kibo)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/kibo`,
  ...
  it.each([
    {
      ...
      overrides: { tailnetDns: "kibos-mac-studio-1.sheep-coho.ts.net" },
      records: [`tailnetDns=kibos-mac-studio-1.sheep-coho.ts.net`],
    },
    {
      ...
      records: [
        `$ORIGIN kibo.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._kibo-gw._tcp IN TXT`,
        `displayName=Mac Studio (Kibo)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("kibo.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("kibo.internal."),
