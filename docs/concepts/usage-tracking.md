...
- `/usage cost` in chats: local cost summary aggregated from Kibo session logs.
- CLI: `kibo status --usage` prints a full per-provider breakdown.
- CLI: `kibo channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Kibo treats
  ...
  quota, so Kibo inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Kibo prefers the
...
can supply plugin-specific usage auth logic; otherwise Kibo falls back to
