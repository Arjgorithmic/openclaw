...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: KiboProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: KiboProtocol.AnyCodable] = [
            "presence": .init([KiboProtocol.AnyCodable(entry)]),
        ]
        let payload = KiboProtocol.AnyCodable(payloadMap)
