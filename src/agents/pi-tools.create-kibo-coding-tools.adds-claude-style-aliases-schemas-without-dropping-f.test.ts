...
describe("createKiboCodingTools", () => {
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-canonical-"));
    try {
      const tools = createKiboCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-legacy-alias-"));
    try {
      const tools = createKiboCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-structured-write-"));
    try {
      const tools = createKiboCodingTools({ workspaceDir: tmpDir });
      ...
      await expect(
        writeTool?.execute("tool-structured-write", {
          ...
          content: [
            ...
            { type: "input_text", text: "const root = path.join(process.env.HOME, 'kibo');\n" },
          ],
        }),
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-structured-edit-"));
    try {
