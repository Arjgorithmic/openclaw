...
val androidStoreFile = providers.gradleProperty("KIBO_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("KIBO_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("KIBO_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("KIBO_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set KIBO_ANDROID_STORE_FILE, " +
            "KIBO_ANDROID_STORE_PASSWORD, KIBO_ANDROID_KEY_ALIAS, and " +
            "KIBO_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/KiboKit/Sources/KiboKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.kibo.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "KIBO_ENABLE_SMS", "false")
            buildConfigField("boolean", "KIBO_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "KIBO_ENABLE_SMS", "true")
            buildConfigField("boolean", "KIBO_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "kibo-$versionName-$buildType.apk"
                    } else {
                        "kibo-$versionName-$flavorName-$buildType.apk"
                    }
