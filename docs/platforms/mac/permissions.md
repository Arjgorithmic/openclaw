...
- Same path: run the app from a fixed location (for Kibo, `dist/Kibo.app`).
...
```bash
sudo tccutil reset Accessibility ai.kibo.mac
sudo tccutil reset ScreenCapture ai.kibo.mac
...
Workaround: move files into the Kibo workspace (`~/.kibo/workspace`) if you want to avoid per-folder grants.
