...
homepage: https://github.com/Arjgorithmic/openclaw/automation/hooks#command-logger
metadata:
  {
    "kibo":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.kibo/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.kibo/logs/commands.log ~/.kibo/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/kibo`:
   ```
   /home/username/.kibo/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
