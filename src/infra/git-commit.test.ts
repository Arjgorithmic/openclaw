...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`kibo-${label}-`);
}
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the kibo package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "kibo");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "kibo", version: "2026.3.10" }),
      "utf-8",
    );
