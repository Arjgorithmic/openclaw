...
type LoadKiboPlugins = typeof import("./loader.js").loadKiboPlugins;
...
const loadKiboPluginsMock = vi.fn<LoadKiboPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/kibo.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadKiboPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: KiboConfig = {
    plugins: {},
  };
  const autoEnabledConfig: KiboConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadKiboPlugins: (...args: Parameters<LoadKiboPlugins>) =>
        loadKiboPluginsMock(...args),
