...
import { resolveRuntimeServiceVersion } from "kibo/plugin-sdk/cli-runtime";
...
const QQBOT_PLUGIN_GITHUB_URL = "https://github.com/kibo/kibo/tree/main/extensions/qqbot";
const QQBOT_UPGRADE_GUIDE_URL = "https://q.qq.com/qqbot/kibo/upgrade.html";
...
/**
 * /bot-ping — test current network latency between Kibo and QQ.
 */
registerCommand({
  name: "bot-ping",
  description: "测试 Kibo 与 QQ 之间的网络延迟",
  usage: [
    `/bot-ping`,
    ``,
    `测试当前 Kibo 宿主机与 QQ 服务器之间的网络延迟。`,
    `返回网络传输耗时和插件处理耗时。`,
  ].join("\n"),
  handler: (ctx) => {
    const now = Date.now();
...
/**
 * /bot-version — show the Kibo framework version.
 */
registerCommand({
  name: "bot-version",
  description: "查看 Kibo 框架版本",
  usage: [`/bot-version`, ``, `查看当前 Kibo 框架版本。`].join("\n"),
  handler: async () => {
    const frameworkVersion = resolveRuntimeServiceVersion();
    const lines = [`🦞 Kibo 版本：${frameworkVersion}`];
...
function getConfiguredLogFiles(): string[] {
  ...
  const files: string[] = [];
  for (const cli of ["kibo", "kibobot", "moltbot"]) {
    try {
...
function collectCandidateLogDirs(): string[] {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (/STATE_DIR$/i.test(key) && /(KIBO|KIBOBOT|MOLTBOT)/i.test(key)) {
      pushStateDir(value);
    }
  }

  for (const name of [".kibo", ".kibobot", ".moltbot", "kibo", "kibobot", "moltbot"]) {
  ...
  for (const root of searchRoots) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(kibo|kibobot|moltbot)/i.test(entry.name)) {
          continue;
        }
  ...
  if (!isWindows()) {
    for (const name of ["kibo", "kibobot", "moltbot"]) {
...
function collectRecentLogFiles(logDirs: string[]): LogCandidate[] {
  ...
  for (const dir of logDirs) {
    ...
    pushFile(path.join(dir, "kibo.log"), dir);
    pushFile(path.join(dir, "kibobot.log"), dir);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(gateway|kibo|kibobot|moltbot)/i.test(entry.name)) {
          continue;
        }
