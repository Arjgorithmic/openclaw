...
Kibo integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Kibo owns lifecycle when `channels.signal.autoStart=true`.
...
- Kibo spawns `imsg rpc` as a child process (legacy iMessage integration).
