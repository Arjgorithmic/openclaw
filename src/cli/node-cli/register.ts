    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "kibo node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["kibo node status", "Check node host service status."],
          ["kibo node install", "Install the node host service."],
          ["kibo node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.kibo.ai/cli/node")}\n`,
    );
