---
summary: "Use Amazon Bedrock (Converse API) models with Kibo"
read_when:
  - You want to use Amazon Bedrock models with Kibo
...
Kibo can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
Kibo can automatically discover Bedrock models that support **streaming**
...
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is `true`,
  Kibo will try discovery even when no AWS env marker is present.
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is unset,
  Kibo only auto-adds the
...
- `enabled` defaults to auto mode. In auto mode, Kibo only enables the
...
- For explicit `models.providers["amazon-bedrock"]` entries, Kibo can still
...
When running Kibo on an EC2 instance with an IAM role attached, the AWS SDK
can use the instance metadata service (IMDS) for authentication. For Bedrock
model discovery, Kibo only auto-enables the implicit provider from AWS env
...
kibo config set plugins.entries.amazon-bedrock.config.discovery.enabled true
kibo config set plugins.entries.amazon-bedrock.config.discovery.region us-east-1
...
Kibo discovers **regional and global inference profiles** alongside
...
foundation models in `kibo models list`.
...
- If you rely on auto mode, set one of the supported AWS auth env markers on the
  ...
  `plugins.entries.amazon-bedrock.config.discovery.enabled: true`.
- Kibo surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
