...
```bash
kibo tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `kibo tui --deliver`
...
- Local shell commands receive `KIBO_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `KIBO_THEME=light` before launching `kibo tui`.
- To force the original dark palette instead, set `KIBO_THEME=dark`.
...
- Check the Gateway logs: `kibo logs --follow`.
- Confirm the agent can run: `kibo status` and `kibo models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `kibo agents list` and your routing config.
