---
summary: "CLI reference for `kibo node` (headless node host)"
read_when:
...
`kibo node run` and `kibo node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `KIBO_GATEWAY_TOKEN` / `KIBO_GATEWAY_PASSWORD` are checked first.
...
```bash
kibo node install --host <gateway-host> --port 18789
```
...
Use `kibo node run` for a foreground node host (no service).
...
- `kibo approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Kibo prepares a canonical `systemRunPlan`
