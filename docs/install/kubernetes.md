---
summary: "Deploy Kibo Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Kibo on a Kubernetes cluster
  - You want to test Kibo in a Kubernetes environment
title: "Kubernetes"
---

# Kibo on Kubernetes

A minimal starting point for running Kibo on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Kibo is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/kibo 18789:18789 -n kibo
...
```bash
kubectl get secret kibo-secrets -n kibo -o jsonpath='{.data.KIBO_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: kibo (configurable via KIBO_NAMESPACE)
├── Deployment/kibo        # Single pod, init container + gateway
├── Service/kibo           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/kibo-config  # kibo.json + AGENTS.md
└── Secret/kibo-secrets    # Gateway token + API keys
```
...
Edit `kibo.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret kibo-secrets -n kibo \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/kibo -n kibo
```

### Custom namespace

```bash
KIBO_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/kibo/kibo:latest # or pin to a specific version from https://github.com/kibo/kibo/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # kibo.json + AGENTS.md
