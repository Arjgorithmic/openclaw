...
export function createLlmTaskTool(api: KiboPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Kibo Shell workflows via kibo.invoke.",
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredKiboTmpDir(), "kibo-llm-task-"),
        );
