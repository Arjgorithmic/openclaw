...
function asLegacyTtsConfig(value: unknown): KiboConfig {
  return value as KiboConfig;
}

function asLegacyKiboConfig(value: Record<string, unknown>): KiboConfig {
  return value as unknown as KiboConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): KiboConfig {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    const baseCfg: KiboConfig = asLegacyKiboConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.KIBO_TTS_PREFS;
      process.env.KIBO_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.KIBO_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: KiboConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
