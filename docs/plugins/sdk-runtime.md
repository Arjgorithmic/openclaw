...
Bind a Task Flow runtime to an existing Kibo session key or trusted tool
...
trusted Kibo session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "Kibo plugin SDK", count: 5 },
});
```
