...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "kibo";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "kibo" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (kibo|existing-session). Default: kibo")
    .action(
      async (
        opts: {
          name: string;
