...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("kibo-bundled-");
  const workspaceDir = await createTempDir("kibo-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: KiboConfig) {
