...
read_when:
  - Setting up Twitch chat integration for Kibo
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. Kibo connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current Kibo releases, so normal
...
```bash
kibo plugins install @kibo/twitch
```

Local checkout (when running from a git repo):

```bash
kibo plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `KIBO_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "kibo", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use KIBO_TWITCH_ACCESS_TOKEN env var)
...
```bash
KIBO_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
