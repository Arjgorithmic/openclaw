...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.kibo${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.KIBO_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.KIBO_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.KIBO_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.KIBO_CONFIG_PATH)) {
    env.KIBO_CONFIG_PATH = path.join(stateDir, "kibo.json");
  }

  if (profile === "dev" && !env.KIBO_GATEWAY_PORT?.trim()) {
    env.KIBO_GATEWAY_PORT = "19001";
  }
}
