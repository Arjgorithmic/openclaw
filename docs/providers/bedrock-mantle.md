---
summary: "Use Amazon Bedrock Mantle (OpenAI-compatible) models with Kibo"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with Kibo
...
Kibo includes a bundled **Amazon Bedrock Mantle** provider that connects to
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, Kibo uses it directly. Otherwise,
Kibo attempts to generate a Mantle bearer token from the AWS default
...
- Kibo can mint the Mantle bearer token for you from AWS SDK-compatible
