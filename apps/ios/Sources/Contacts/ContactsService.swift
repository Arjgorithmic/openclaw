...
final class ContactsService: ContactsServicing {
    ...
    func search(params: KiboContactsSearchParams) async throws -> KiboContactsSearchPayload {
        ...
        return KiboContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: KiboContactsAddParams) async throws -> KiboContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return KiboContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return KiboContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> KiboContactPayload {
        KiboContactPayload(
