...
type HomeEnvSnapshot = {
  ...
  KIBO_STATE_DIR: string | undefined;
  KIBO_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    KIBO_AGENT_DIR: process.env.KIBO_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".kibo", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.KIBO_STATE_DIR = path.join(home, ".kibo");
    process.env.KIBO_AGENT_DIR = path.join(home, ".kibo", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".kibo", "agent");
