...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.kibo/kibo.json. Prefer KIBO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.kibo/kibo.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.kibo/kibo.json. Restart the gateway to apply."
        }
