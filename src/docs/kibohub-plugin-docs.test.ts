...
const pluginDocs = [
  path.join(DOCS_ROOT, "tools", "kibohub.md"),
...
describe("KiboHub plugin docs", () => {
  it("keeps the canonical plugin-publish snippets contract-valid", async () => {
    ...
    const pluginManifest = JSON.parse(
      await fs.readFile(
        path.join(DOCS_ROOT, "snippets", "plugin-publish", "minimal-kibo.plugin.json"),
        "utf8",
      ),
  ...
  it("does not tell plugin authors to use bare kibohub publish", async () => {
    for (const docPath of pluginDocs) {
      const markdown = await fs.readFile(docPath, "utf8");
      expect(markdown).not.toMatch(/(^|[\s`])kibohub publish\b/);
    }
  });
  ...
  it("keeps the canonical package snippet embedded in the primary plugin docs", async () => {
    ...
    expect(extractNamedJsonBlock(sdkSetup, "kibo-kibohub-package.json")).toEqual(snippet);
  });
});
