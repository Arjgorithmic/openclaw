...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds KIBO_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `kibo gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service KIBO_GATEWAY_TOKEN does not match gateway.auth.token in kibo.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.KIBO_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.KIBO_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `kibo gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
