...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.KIBO_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.KIBO_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.KIBO_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.KIBO_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "kibo.json");
      ...
      try {
        ...
        await sendChatAndWait({
          ...
          message:
            "Best match for the attached image: shell, mouse, cat, horse. " +
        ...
        if (previous.port === undefined) {
          ...
          process.env.KIBO_GATEWAY_PORT = previous.port;
        }
