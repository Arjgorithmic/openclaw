---
summary: "CLI reference for `kibo plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
kibo plugins list
kibo plugins list --enabled
kibo plugins list --verbose
kibo plugins list --json
kibo plugins install <path-or-spec>
kibo plugins inspect <id>
kibo plugins inspect <id> --json
kibo plugins inspect --all
kibo plugins info <id>
kibo plugins enable <id>
kibo plugins disable <id>
kibo plugins uninstall <id>
kibo plugins doctor
kibo plugins update <id>
kibo plugins update --all
kibo plugins marketplace list <marketplace>
kibo plugins marketplace list <marketplace> --json
```

Bundled plugins ship with Kibo. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native Kibo plugins must ship `kibo.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: kibo` or `Format: bundle`. Verbose list/info
...
```bash
kibo plugins install <package>                      # KiboHub first, then npm
kibo plugins install kibohub:<package>              # KiboHub only
kibo plugins install <package> --force              # overwrite existing install
kibo plugins install <package> --pin                # pin version
kibo plugins install <package> --dangerously-force-unsafe-install
kibo plugins install <path>                         # local path
kibo plugins install <plugin>@<marketplace>         # marketplace
kibo plugins install <plugin> --marketplace <name>  # marketplace (explicit)
kibo plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
```

Bare package names are checked against KiboHub first, then npm. Security note:
...
run `kibo doctor --fix` first. The only documented exception is a narrow
...
the same id from a new local path, archive, KiboHub package, or npm artifact.
...
override, while `kibo skills install` remains a separate KiboHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`kibo.hooks` in `package.json`. Use `kibo hooks` for filtered hook
...
those to a prerelease, Kibo stops and asks you to opt in explicitly with a
prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as
`@1.2.3-beta.4`.

If a bare install spec matches a bundled plugin id (for example `diffs`), Kibo
...
KiboHub installs use an explicit `kibohub:<package>` locator:

```bash
kibo plugins install kibohub:kibo-codex-app-server
kibo plugins install kibohub:kibo-codex-app-server@1.2.3
```

Kibo now also prefers KiboHub for bare npm-safe plugin specs. It only falls
back to npm if KiboHub does not have that package or version:

```bash
kibo plugins install kibo-codex-app-server
```

Kibo downloads the package archive from KiboHub, checks the advertised
plugin API / minimum gateway compatibility, then installs it through the normal
archive path. Recorded installs keep their KiboHub source metadata for later
updates.
...
```bash
kibo plugins marketplace list <marketplace-name>
kibo plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
kibo plugins install <plugin-name> --marketplace <marketplace-name>
kibo plugins install <plugin-name> --marketplace <owner/repo>
kibo plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
kibo plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Kibo accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, Kibo auto-detects:

- native Kibo plugins (`kibo.plugin.json`)
...
```bash
kibo plugins install -l ./my-plugin
```
...
```bash
kibo plugins uninstall <id>
kibo plugins uninstall <id> --dry-run
...
```bash
kibo plugins update <id-or-npm-spec>
kibo plugins update --all
kibo plugins update <id-or-npm-spec> --dry-run
kibo plugins update @kibo/voice-call@beta
kibo plugins update kibo-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Kibo reuses the recorded install spec for that
...
or exact version. Kibo resolves that package name back to the tracked plugin
record, updates that installed plugin, and records the new npm spec for future
id-based updates.

When a stored integrity hash exists and the fetched artifact hash changes,
Kibo prints a warning and asks for confirmation before proceeding. Use
