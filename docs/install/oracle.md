---
summary: "Host Kibo on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Kibo on Oracle Cloud
  - Looking for free VPS hosting for Kibo
...
Run a persistent Kibo Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@kibo`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=kibo --reset` to re-authenticate.

**Gateway will not start** -- Run `kibo doctor --non-interactive` and check logs with `journalctl --user -u kibo-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep Kibo up to date
