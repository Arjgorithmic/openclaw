...
import { withFileLock } from "kibo/plugin-sdk/infra-runtime";
...
// `kibo matrix verify bootstrap`) can corrupt each other's state.
...
function resolveDefaultIdbSnapshotPath(): string {
  const stateDir =
    process.env.KIBO_STATE_DIR || path.join(process.env.HOME || "/tmp", ".kibo");
