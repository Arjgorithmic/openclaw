...
if [[ "${mode}" != "--dry-run" && "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/plugin-kibohub-publish.sh [--dry-run|--publish] <package-dir>" >&2
  exit 2
fi
...
if ! command -v kibohub >/dev/null 2>&1; then
  echo "kibohub CLI is required on PATH" >&2
  exit 1
fi
...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-kibo/kibo}}"
source_commit="${SOURCE_COMMIT:-$(git rev-parse HEAD)}"
source_ref="${SOURCE_REF:-$(git symbolic-ref -q HEAD || true)}"
kibohub_workdir="${KIBOHUB_WORKDIR:-${KIBOHUB_WORKDIR:-$(pwd)}}"
...
echo "Resolved KiboHub workdir: ${kibohub_workdir}"
echo "Publish auth: GitHub Actions OIDC via KiboHub short-lived token"
...
if [[ "${mode}" == "--dry-run" ]]; then
  KIBOHUB_WORKDIR="${kibohub_workdir}" "${publish_cmd[@]}" --dry-run
  exit 0
fi

KIBOHUB_WORKDIR="${kibohub_workdir}" "${publish_cmd[@]}"
