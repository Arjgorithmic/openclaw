      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["kibo gateway run", "Run the gateway in the foreground."],
            ["kibo gateway status", "Show service status and probe reachability."],
            ["kibo gateway discover", "Find local and wide-area gateway beacons."],
            ["kibo gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.kibo.ai/cli/gateway")}\n`,
      ),
  );
