...
private const val NOTIFICATION_CHANNEL_BASE_ID = "kibo.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Kibo Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Kibo Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Kibo Active")
      }
