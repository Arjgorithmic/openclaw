---
summary: "CLI reference for `kibo onboard` (interactive onboarding)"
read_when:
...
```bash
kibo onboard
kibo onboard --flow quickstart
kibo onboard --flow manual
kibo onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`KIBO_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `kibo gateway run`.
...
- Fastest first chat: `kibo dashboard` (Control UI, no channel setup).
