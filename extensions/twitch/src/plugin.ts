/**
 * Twitch channel plugin for Kibo.
 *
...
} from "kibo/plugin-sdk/channel-pairing";
import type { KiboConfig } from "kibo/plugin-sdk/config-runtime";
import { buildPassiveProbedChannelStatusSummary } from "kibo/plugin-sdk/extension-shared";
import {
 ...
 * for Kibo. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: KiboConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: KiboConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: KiboConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: KiboConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("kibo/plugin-sdk/runtime-env").RuntimeEnv;
