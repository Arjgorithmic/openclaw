...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: KiboProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.kibo", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: KiboKit.AnyCodable]()) {
                $0[$1.key] = KiboKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: KiboProtocol.AnyCodable?) -> [String: KiboProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: KiboProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: KiboKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: KiboProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("kibo.control.heartbeat")
    static let controlAgentEvent = Notification.Name("kibo.control.agent")
}
