...
const KIBO_OPENSHELL_E2E = process.env.KIBO_E2E_OPENSHELL === "1";
const KIBO_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const KIBO_OPENSHELL_COMMAND =
  process.env.KIBO_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "kibo-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && KIBO_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: KIBO_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(KIBO_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-openshell-e2e-"));
      ...
      const gatewayName = `kibo-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `kibo-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/kibo-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: KIBO_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("kibo-openshell-e2e");
