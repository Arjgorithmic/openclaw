...
export type ShellRunner = {
  run: (params: ShellRunnerParams) => Promise<ShellEnvelope>;
};
...
export function resolveShellCwd(cwdRaw: unknown): string {
...
function createLimitedSink(maxBytes: number, label: "stdout" | "stderr") {
  ...
  return new Writable({
    write(chunk, _encoding, callback) {
      ...
      if (bytes > maxBytes) {
        callback(new Error(`shell ${label} exceeded maxStdoutBytes`));
        return;
      }
      callback();
    },
...
function normalizeEnvelope(envelope: EmbeddedToolEnvelope): ShellEnvelope {
  if (envelope.ok) {
    return {
      ok: true,
  ...
  return {
    ...
    error: {
      ...
      message: envelope.error?.message ?? "shell runtime failed",
    },
  };
}

function throwOnErrorEnvelope(envelope: ShellEnvelope): Extract<ShellEnvelope, { ok: true }> {
  if (envelope.ok) {
    return envelope;
  }
...
async function resolveWorkflowFile(candidate: string, cwd: string) {
  ...
  if (![".shell", ".yaml", ".yml", ".json"].includes(ext)) {
    throw new Error("Workflow file must end in .shell, .yaml, .yml, or .json");
  }
  return resolved;
}
...
async function withTimeout<T>(
  ...
  return await new Promise<T>((resolve, reject) => {
    const onTimeout = () => {
      const error = new Error("shell runtime timed out");
...
async function importInstalledShellModule<T>(
  shellRoot: string,
  relativePath: string,
): Promise<T> {
  const target = path.join(shellRoot, relativePath);
  return (await import(pathToFileURL(target).href)) as T;
}

function resolveInstalledShellRoot() {
  const require = createRequire(import.meta.url);
  const sdkEntry = require.resolve("@kibobot/shell");
  ...
  while (true) {
    ...
    if (parentDir === currentDir) {
      throw new Error("Unable to resolve the installed @kibobot/shell package root");
    }
...
async function loadEmbeddedToolRuntimeFromPackage(): Promise<EmbeddedToolRuntime> {
  const shellRoot = resolveInstalledShellRoot();
  const coreIndexPath = path.join(shellRoot, "dist/src/core/index.js");
  ...
  const [
    ...
    importInstalledShellModule<{
      createDefaultRegistry: ToolRuntimeDeps["createDefaultRegistry"];
    }>(shellRoot, "dist/src/commands/registry.js"),
    importInstalledShellModule<{ parsePipeline: ToolRuntimeDeps["parsePipeline"] }>(
      shellRoot,
      "dist/src/parser.js",
    ),
    importInstalledShellModule<{ decodeResumeToken: ToolRuntimeDeps["decodeResumeToken"] }>(
      shellRoot,
      "dist/src/resume.js",
    ),
    importInstalledShellModule<{ encodeToken: ToolRuntimeDeps["encodeToken"] }>(
      shellRoot,
      "dist/src/token.js",
    ),
    importInstalledShellModule<{ runPipeline: ToolRuntimeDeps["runPipeline"] }>(
      shellRoot,
      "dist/src/runtime.js",
    ),
    importInstalledShellModule<{ runWorkflowFile: ToolRuntimeDeps["runWorkflowFile"] }>(
      shellRoot,
    ...
    }>(shellRoot, "dist/src/state/store.js"),
  ]);
...
export function createEmbeddedShellRunner(options?: {
