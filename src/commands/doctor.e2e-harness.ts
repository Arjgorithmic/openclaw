...
export const resolveKiboPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadKiboPlugins: () => createEmptyPluginRegistry(),
}));
...
vi.mock("kibo/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("kibo/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("kibo/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/kibo-root.js", () => ({
  ...
  resolveKiboPackageRootSync: vi.fn(() => "/tmp/kibo"),
}));
...
beforeEach(() => {
  ...
  resolveKiboPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.KIBO_STATE_DIR;
  originalUpdateInProgress = process.env.KIBO_UPDATE_IN_PROGRESS;
  process.env.KIBO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-doctor-state-"));
