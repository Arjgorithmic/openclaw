...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.kibo", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> KiboSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: KiboSessionPreviewEntry,
    ...
    private static func cache(payload: KiboSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: KiboChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [KiboKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [KiboKit.AnyCodable]) -> [KiboChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(KiboChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: KiboChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: KiboChatMessage) -> Bool {
    ...
    private static func toolNames(for message: KiboChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: KiboChatMessage) -> String? {
