...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `kibo-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      KIBO_LAUNCHD_LABEL: `ai.kibo.launchd-int-${testId}`,
      KIBO_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
