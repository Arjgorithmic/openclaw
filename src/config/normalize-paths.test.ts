  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.kibo/logs/kibo.log" },
        hooks: {
          path: "~/.kibo/hooks.json5",
        ...
        channels: {
          ...
          whatsapp: {
            accounts: {
              personal: {
                authDir: "~/.kibo/credentials/wa-personal",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".kibo", "logs", "kibo.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".kibo", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".kibo", "telegram.token"),
      );
      expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
        path.join(home, ".kibo", "credentials", "wa-personal"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".kibo", "agents", "main"));
