...
import { resolveKiboUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveKiboUserDataDir: vi.fn(() => "/tmp/kibo-test/kibo/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "kibo-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveKiboUserDataDir).mockReturnValue(userDataDir);
