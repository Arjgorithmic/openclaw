---
summary: "Optional Docker-based setup and onboarding for Kibo"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Kibo on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export KIBO_IMAGE="ghcr.io/kibo/kibo:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/kibo/kibo/pkgs/container/kibo).
  ...
  <Step title="Complete onboarding">
    ...
    `kibo-gateway` directly. `kibo-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm kibo-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm kibo-cli channels login

    # Telegram
    docker compose run --rm kibo-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm kibo-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t kibo:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node kibo-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `KIBO_EXTRA_MOUNTS`
or `KIBO_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `kibo-cli` shares `kibo-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d kibo-gateway`, run onboarding
and setup-time config writes through `kibo-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `KIBO_IMAGE`               | Use a remote image instead of building locally                   |
| `KIBO_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `KIBO_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `KIBO_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `KIBO_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `KIBO_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `KIBO_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec kibo-gateway node dist/index.js health --token "$KIBO_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `KIBO_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `KIBO_CONFIG_DIR` to `/home/node/.kibo` and
`KIBO_WORKSPACE_DIR` to `/home/node/.kibo/workspace`, so those paths
survive container replacement.

That mounted config directory is where Kibo keeps:

- `kibo.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `KIBO_GATEWAY_TOKEN`

For full persistence details on VM deployments, see
[Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** watch `media/`, session JSONL files, `cron/runs/*.jsonl`,
and rolling file logs under `/tmp/kibo/`.
...
If you installed ClawDock from the older `scripts/shell-helpers/kiboock-helpers.sh` raw path, rerun the install command above so your local helper file tracks the new location.

Then use `kiboock-start`, `kiboock-stop`, `kiboock-dashboard`, etc. Run
`kiboock-help` for all commands.
See [ClawDock](/install/kiboock) for the full helper guide.

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export KIBO_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export KIBO_SANDBOX=1
    export KIBO_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm kibo-cli gateway probe
    docker compose run -T --rm kibo-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `kibo-cli` uses `network_mode: "service:kibo-gateway"` so CLI
    ...
    `no-new-privileges` on `kibo-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.kibo`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/kibo-config /path/to/kibo-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export KIBO_HOME_VOLUME="kibo_home"`
    2. **Bake system deps**: `export KIBO_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `KIBO_HOME_VOLUME` or `KIBO_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/kibo/kibo/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Kibo runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm kibo-cli dashboard --no-open
    docker compose run --rm kibo-cli devices list
    docker compose run --rm kibo-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm kibo-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm kibo-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [ClawDock](/install/kiboock) — Docker Compose community setup
- [Updating](/install/updating) — keeping Kibo up to date
