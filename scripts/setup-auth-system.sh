#!/bin/bash
# Setup Kibo Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'kibo-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Kibo message: Send warning via Kibo itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/kibo-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/kibo-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/kibo-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now kibo-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/KiboAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/KiboAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Kibo*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (KiboAuth) shows status and opens auth URL if needed."
echo "The full widget (KiboAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status kibo-auth-monitor.timer"
echo ""
echo "Quick commands:"
