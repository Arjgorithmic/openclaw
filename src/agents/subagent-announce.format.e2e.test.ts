...
function setConfigOverride(next: KiboConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/kibo/kibo/issues/31298
    previousFastTestEnv = process.env.KIBO_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.KIBO_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // KIBO_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Kibo runtime context (internal):");
