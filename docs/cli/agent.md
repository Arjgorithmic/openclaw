---
summary: "CLI reference for `kibo agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
kibo agent --to +15555550123 --message "status update" --deliver
kibo agent --agent ops --message "Summarize logs"
...
kibo agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Kibo persists markers from the active source config snapshot, not from resolved runtime secret values.
