---
summary: "CLI reference for `kibo qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
kibo qr
kibo qr --setup-code-only
kibo qr --json
kibo qr --remote
kibo qr --url wss://gateway.example/ws
```
...
- With `--remote`, Kibo requires either `gateway.remote.url` or
