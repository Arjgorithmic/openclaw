...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    kibo: process.env.KIBO_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.kibo === undefined) {
      delete process.env.KIBO_GATEWAY_TOKEN;
    } else {
      process.env.KIBO_GATEWAY_TOKEN = envSnapshot.kibo;
    }
  });
  ...
  it("uses KIBO_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.KIBO_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
