...
read_when:
  - You want to understand how Kibo assembles model context
...
A **context engine** controls how Kibo builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Kibo ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.kibo/kibo.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Kibo plugin. Install
...
kibo plugins install -l ./my-context-engine
```
...
Every time Kibo runs a model prompt, the context engine participates at
...
Kibo currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Kibo
...
The built-in `legacy` engine preserves Kibo's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Kibo uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Kibo disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Kibo automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `kibo/plugin-sdk/core` to use
  Kibo's built-in compaction behavior.
...
Kibo resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Kibo's built-in summarization. Plugin engines can implement
...
- Use `kibo doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Kibo
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `kibo plugins install -l ./my-engine` to link a
