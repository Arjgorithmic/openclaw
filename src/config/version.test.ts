...
import {
  compareKiboVersions,
  isSameKiboStableFamily,
  parseKiboVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseKiboVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseKiboVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseKiboVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseKiboVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseKiboVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseKiboVersion("2026.3")).toBeNull();
    expect(parseKiboVersion("latest")).toBeNull();
  });
});

describe("compareKiboVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareKiboVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareKiboVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareKiboVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareKiboVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareKiboVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareKiboVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameKiboStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameKiboStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameKiboStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameKiboStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameKiboStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
