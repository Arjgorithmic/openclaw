...
function listConfiguredAgentIds(cfg: KiboConfig): string[] {
...
export function listAgentsForGateway(cfg: KiboConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: KiboConfig): string {
...
function resolveSessionStoreAgentId(cfg: KiboConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: KiboConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: KiboConfig): GatewaySessionsDefaults {
