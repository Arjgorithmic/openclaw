...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([KIBO_MODEL_ID, KIBO_DEFAULT_MODEL_ID]);
  ids.add(`kibo/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`kibo/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== KIBO_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
