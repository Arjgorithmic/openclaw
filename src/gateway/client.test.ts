...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("kibo doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with KIBO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with KIBO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.KIBO_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://kibo-gateway.ai:18789",
      onConnectError,
    });
