...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: KiboConfig, policy: ChannelAccessPolicy) => KiboConfig;
  resolveAllowlist: (params: { cfg: KiboConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: KiboConfig; resolved: TResolved }) => KiboConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: KiboConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: KiboConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: KiboConfig, policy: ChannelAccessPolicy): KiboConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: KiboConfig, policy: ChannelAccessPolicy): KiboConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: KiboConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: KiboConfig; resolved: string[] }) => {
      calls.push("apply");
