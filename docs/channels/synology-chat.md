---
summary: "Synology Chat webhook setup and Kibo config"
read_when:
  - Setting up Synology Chat with Kibo
...
Synology Chat ships as a bundled plugin in current Kibo releases, so normal
...
```bash
kibo plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `kibo onboard` now shows Synology Chat in the same channel setup list as `kibo channels add`.
   - Non-interactive setup: `kibo channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Kibo gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Kibo.
   - Guided: `kibo onboard`
   - Direct: `kibo channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Kibo accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `kibo pairing list synology-chat`
  - `kibo pairing approve synology-chat <CODE>`
...
```bash
kibo message send --channel synology-chat --target 123456 --text "Hello from Kibo"
kibo message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Kibo now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Kibo
- `Rate limit exceeded`:
