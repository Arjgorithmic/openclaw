...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-kibo-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://kibo.ai/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/kibo-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-kibo-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://kibo.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run kibo update --channel dev.
...
# kibo.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$kibo @args 2>&1
if (\$null -ne \$output) {
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "kibo-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_powershell "$(cat <<EOF
\$depsRoot = Join-Path \$env:LOCALAPPDATA 'Kibo\deps'
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/kibo-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
write_latest_install_runner_script() {
  ...
  WINDOWS_LATEST_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/kibo-install-latest.ps1"
...
try {
  ...
  & (Join-Path \$env:APPDATA 'npm\kibo.cmd') --version *>&1 | Tee-Object -FilePath \$LogPath -Append | Out-Null
  if (\$LASTEXITCODE -ne 0) {
    throw "kibo --version failed with exit code \$LASTEXITCODE"
  }
...
write_baseline_npm_install_runner_script() {
  WINDOWS_BASELINE_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/kibo-install-baseline-npm.ps1"
...
try {
  $portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Kibo\deps') 'portable-git') ''
  $env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
  $kibo = Join-Path $env:APPDATA 'npm\kibo.cmd'

  Write-ProgressLog 'install.start'
  Invoke-Logged 'npm install baseline release' {
    & npm.cmd install -g "kibo@$Version" --no-fund --no-audit --loglevel=error
  }

  Write-ProgressLog 'install.version'
  Invoke-Logged 'kibo --version' { & $kibo --version }
...
install_baseline_npm_release() {
  ...
  runner_name="kibo-install-baseline-$RANDOM-$RANDOM.ps1"
  log_name="kibo-install-baseline-$RANDOM-$RANDOM.log"
  done_name="kibo-install-baseline-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/kibo-install-baseline-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/kibo-install-baseline-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
...
install_latest_release() {
  ...
  runner_name="kibo-install-latest-$RANDOM-$RANDOM.ps1"
  log_name="kibo-install-latest-$RANDOM-$RANDOM.log"
  done_name="kibo-install-latest-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/kibo-install-latest-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
  startup_checked=0
...
install_main_tgz() {
  ...
  runner_name="kibo-install-$RANDOM-$RANDOM.ps1"
  log_name="kibo-install-$RANDOM-$RANDOM.log"
  done_name="kibo-install-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/kibo-install-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/kibo-install-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
...
write_dev_update_runner_script() {
  WINDOWS_DEV_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/kibo-update-dev.ps1"
...
try {
  ...
  $kibo = Join-Path $env:APPDATA 'npm\kibo.cmd'
  $gitRoot = Join-Path $env:USERPROFILE 'kibo'
  $gitEntry = Join-Path $gitRoot 'kibo.mjs'
...
"@ | Set-Content -Path $pnpmPsShim -Encoding UTF8
  ...
  Invoke-Logged 'kibo update --channel dev --yes --json' {
    & $kibo update --channel dev --yes --json
  }
  ...
  Invoke-Logged 'git kibo --version' { & node.exe $gitEntry --version }
  Invoke-Logged 'git kibo update status --json' { & node.exe $gitEntry update status --json }
...
run_dev_channel_update() {
  ...
  runner_name="kibo-update-dev-$RANDOM-$RANDOM.ps1"
  log_name="kibo-update-dev-$RANDOM-$RANDOM.log"
  done_name="kibo-update-dev-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/kibo-update-dev-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
  startup_checked=0
...
write_install_runner_script() {
  WINDOWS_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/kibo-install-main.ps1"
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_kibo "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/kibo-onboard-$PROVIDER.ps1"
...
try {
  \$kibo = Join-Path \$env:APPDATA 'npm\kibo.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice ${AUTH_CHOICE} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json > "{1}" 2>&1' -f \$kibo, \$LogPath)
...
run_ref_onboard() {
  ...
  runner_name="kibo-onboard-$RANDOM-$RANDOM.ps1"
  log_name="kibo-onboard-$RANDOM-$RANDOM.log"
  done_name="kibo-onboard-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
verify_gateway() {
  guest_run_kibo "" "" gateway status --deep --require-rpc
}
...
verify_gateway_reachable() {
  ...
  for attempt in 1 2 3 4 5 6; do
    probe_json="$(
      guest_run_kibo "" "" gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json
    )"
    printf '%s\n' "$probe_json"
...
$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Kibo\deps') 'portable-git') ''
$env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
$gitEntry = Join-Path (Join-Path $env:USERPROFILE 'kibo') 'kibo.mjs'
if (-not (Test-Path $gitEntry)) {
...
run_gateway_daemon_action() {
  ...
  local runner_name log_name done_name done_status launcher_state
  local poll_rc state_rc log_rc start_seconds poll_deadline startup_checked
  runner_name="kibo-gateway-$action-$RANDOM-$RANDOM.ps1"
  log_name="kibo-gateway-$action-$RANDOM-$RANDOM.log"
  done_name="kibo-gateway-$action-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
try {
  \$kibo = Join-Path \$env:APPDATA 'npm\kibo.cmd'
  & \$kibo gateway $action *>&1 | Tee-Object -FilePath \$log -Append | Out-Null
...
show_gateway_status_compat() {
  if guest_run_kibo "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_kibo "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_kibo "" "" gateway status --deep
}

verify_turn() {
  guest_run_kibo "" "" models set "$MODEL_ID"
  guest_run_kibo "$API_KEY_ENV" "$API_KEY_VALUE" \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "kibo-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-baseline-package" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "kibo-main-upgrade.tgz" || return $?
