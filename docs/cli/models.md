---
summary: "CLI reference for `kibo models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
kibo models status
kibo models list
kibo models set <model-or-alias>
kibo models scan
```

`kibo models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Kibo falls back to matching OAuth/API-key
...
the command uses `KIBO_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, Kibo resolves the input as an alias first, then
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `KIBO_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Kibo could not resolve a probeable
...
```bash
kibo models auth add
kibo models auth login --provider <id>
kibo models auth setup-token --provider <id>
kibo models auth paste-token
```
...
`kibo plugins list` to see which providers are installed.

Examples:

```bash
kibo models auth login --provider openai-codex --set-default
```
