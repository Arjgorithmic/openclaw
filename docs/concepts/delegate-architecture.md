---
summary: "Delegate architecture: running Kibo as a named agent on behalf of an organization"
...
Goal: run Kibo as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Kibo agent that:
...
Kibo's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Kibo's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.kibo/workspace-delegate",
  tools: {
...
- Cron run history: `~/.kibo/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.kibo/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Kibo's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
kibo agents add delegate
```

This creates:

- Workspace: `~/.kibo/workspace-delegate`
- State: `~/.kibo/agents/delegate/agent`
- Sessions: `~/.kibo/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.kibo/workspace" },
      {
        id: "delegate",
        workspace: "~/.kibo/workspace-delegate",
        tools: {
...
~/.kibo/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.kibo/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.kibo/workspace-org",
        agentDir: "~/.kibo/agents/org-assistant/agent",
...
recall view. Kibo redacts credential/token-like text, truncates long
