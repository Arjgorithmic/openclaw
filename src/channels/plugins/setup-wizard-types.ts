...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: KiboConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: KiboConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: KiboConfig, policy: DmPolicy, accountId?: string) => KiboConfig;
