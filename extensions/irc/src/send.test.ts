...
vi.mock("kibo/plugin-sdk/config-runtime", async () => {
  const original = (await vi.importActual("kibo/plugin-sdk/config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("kibo/plugin-sdk/text-runtime", async () => {
  const original = (await vi.importActual("kibo/plugin-sdk/text-runtime")) as Record<
    string,
    unknown
  >;
  return {
