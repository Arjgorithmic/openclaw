...
Zalo ships as a bundled plugin in current Kibo releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `kibo plugins install @kibo/zalo`
- Or from a source checkout: `kibo plugins install ./path/to/local/zalo-plugin`
...
This page reflects current Kibo behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `kibo pairing list zalo`
  - `kibo pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in Kibo.
...
- Example: `kibo message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `kibo channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `kibo logs --follow`
