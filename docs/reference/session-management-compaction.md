...
This document explains how Kibo manages sessions end-to-end:
...
Kibo is designed around a single **Gateway process** that owns session state.
...
Kibo persists sessions in two layers:
...
- Transcripts: `~/.kibo/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Kibo resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Kibo reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
kibo sessions cleanup --dry-run
kibo sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.kibo/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Kibo intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Kibo splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Kibo
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Kibo preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Kibo consumes the events, but Pi decides when to compact).
...
Kibo also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Kibo bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Kibo leaves it alone.
...
- If the provider fails or returns an empty result, Kibo falls back to built-in LLM summarization automatically.
...
Kibo supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Kibo strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Kibo also suppresses **draft/typing streaming** when a
...
Kibo uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Kibo’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `kibo status`.
