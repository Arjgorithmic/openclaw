...
read_when:
  - You see the KIBO_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the KIBO_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Kibo plugin
---

# Plugin SDK Migration

Kibo has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`kibo/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`kibo/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`kibo/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `kibo/plugin-sdk/slack`, `kibo/plugin-sdk/discord`,
`kibo/plugin-sdk/signal`, `kibo/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`kibo/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `kibo/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `kibo/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "kibo/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    import { runEmbeddedPiAgent } from "kibo/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `KiboSchema` |
...
```bash
KIBO_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 kibo gateway run
KIBO_SUPPRESS_EXTENSION_API_WARNING=1 kibo gateway run
```
