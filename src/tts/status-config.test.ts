...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withTempHome(async (home) => {
      const prefsPath = path.join(home, ".kibo", "settings", "tts.json");
  ...
  it("derives the default prefs path from KIBO_CONFIG_PATH when set", async () => {
    await withTempHome(
      async (home) => {
        const stateDir = path.join(home, ".kibo-dev");
        ...
        vi.stubEnv("KIBO_CONFIG_PATH", path.join(stateDir, "kibo.json"));
        try {
          expect(
            resolveStatusTtsSnapshot({
              cfg: {
