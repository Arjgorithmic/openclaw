...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["KIBO_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-subagent-"));
  ...
  it("uses isolated temp state when KIBO_STATE_DIR is unset in tests", async () => {
    delete process.env.KIBO_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "kibo-test-state"));
  });
});
