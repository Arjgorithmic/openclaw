---
summary: "Scripted onboarding and agent setup for the Kibo CLI"
read_when:
...
Use `--non-interactive` to automate `kibo onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but Kibo now prefers Claude CLI reuse when available.
