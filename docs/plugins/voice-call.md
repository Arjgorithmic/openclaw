...
read_when:
  - You want to place an outbound voice call from Kibo
...
Voice calls for Kibo via a plugin. Supports outbound notifications and
...
- Use `kibo voicecall ...` or the `voice_call` tool
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `kibo doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `kibo doctor --fix`:
...
kibo voicecall start --to "+15555550123"   # alias for call
...
kibo voicecall tail
kibo voicecall latency                     # summarize turn latency from logs
