    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kibo memory status", "Show index and provider status."],
          [
            "kibo memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["kibo memory status --deep", "Probe embedding provider readiness."],
          ["kibo memory index --force", "Force a full reindex."],
          ['kibo memory search "meeting notes"', "Quick search using positional query."],
          [
            'kibo memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `kibo memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "kibo memory rem-backfill --path ./memory",
          ...
          ["kibo memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.kibo.ai/cli/memory")}\n`,
    );
