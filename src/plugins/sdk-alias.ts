...
function hasTrustedKiboRootIndicator(params: {
  packageRoot: string;
  ...
  const hasKiboBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("kibo")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.kibo === "string");
  const hasKiboEntrypoint = fs.existsSync(path.join(params.packageRoot, "kibo.mjs"));
  return hasCliEntryExport || hasKiboBin || hasKiboEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedKiboRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedKiboRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveKiboPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedKiboRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveKiboPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedKiboRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["kibo/plugin-sdk", "@kibo/plugin-sdk"] as const;
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias
      ? { "kibo/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.kiboPackageRoot, "extensions"),
    path.join(params.kiboPackageRoot, "dist", "extensions"),
    path.join(params.kiboPackageRoot, "dist-runtime", "extensions"),
