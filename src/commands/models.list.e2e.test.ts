...
const ensureKiboModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveKiboAgentDir = vi.fn().mockReturnValue("/tmp/kibo-agent");
...
beforeEach(() => {
  ...
  ensureKiboModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureKiboModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureKiboModelsJson).toHaveBeenCalled();
    expect(ensureKiboModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
