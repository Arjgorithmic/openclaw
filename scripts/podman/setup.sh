#!/usr/bin/env bash
# One-time host setup for rootless Kibo in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.kibo by default, and uses the repo-local
# launch script at ./scripts/run-kibo-podman.sh.
#
...
#   Or set KIBO_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-kibo-podman.sh launch
#   ./scripts/run-kibo-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start kibo.service
set -euo pipefail

REPO_PATH="${KIBO_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-kibo-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/kibo.container.in"
KIBO_USER="$(id -un)"
KIBO_HOME="${HOME:-}"
KIBO_CONFIG_DIR="${KIBO_CONFIG_DIR:-}"
KIBO_WORKSPACE_DIR="${KIBO_WORKSPACE_DIR:-}"
KIBO_IMAGE="${KIBO_PODMAN_IMAGE:-${KIBO_IMAGE:-kibo:local}}"
KIBO_CONTAINER_NAME="${KIBO_PODMAN_CONTAINER:-kibo}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${KIBO_PODMAN_GATEWAY_HOST_PORT:-${KIBO_GATEWAY_PORT:-18789}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate KIBO_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${KIBO_PODMAN_QUADLET:-}" ]]; then
  case "${KIBO_PODMAN_QUADLET,,}" in
...
if [[ "$KIBO_IMAGE" == "kibo:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set KIBO_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$KIBO_HOME" ]]; then
  KIBO_HOME="$(resolve_user_home "$KIBO_USER")"
fi
if [[ -z "$KIBO_HOME" ]]; then
  echo "Unable to resolve HOME for user $KIBO_USER." >&2
  exit 1
fi
if [[ -z "$KIBO_CONFIG_DIR" ]]; then
  KIBO_CONFIG_DIR="$KIBO_HOME/.kibo"
fi
if [[ -z "$KIBO_WORKSPACE_DIR" ]]; then
  KIBO_WORKSPACE_DIR="$KIBO_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$KIBO_HOME"
validate_mount_source_path "config directory" "$KIBO_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$KIBO_WORKSPACE_DIR"
validate_container_name "$KIBO_CONTAINER_NAME"
validate_image_name "$KIBO_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$KIBO_CONFIG_DIR" "$KIBO_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$KIBO_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$KIBO_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${KIBO_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "KIBO_DOCKER_APT_PACKAGES=${KIBO_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${KIBO_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "KIBO_EXTENSIONS=${KIBO_EXTENSIONS}")
fi

if [[ "$KIBO_IMAGE" == "kibo:local" ]]; then
  echo "Building image $KIBO_IMAGE ..."
  podman build -t "$KIBO_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$KIBO_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $KIBO_IMAGE"
  else
    echo "Pulling image $KIBO_IMAGE ..."
    podman pull "$KIBO_IMAGE"
  fi
fi

ENV_FILE="$KIBO_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated KIBO_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "KIBO_PODMAN_CONTAINER" "$KIBO_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "KIBO_PODMAN_IMAGE" "$KIBO_IMAGE"

CONFIG_JSON="$KIBO_CONFIG_DIR/kibo.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$KIBO_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/kibo.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  KIBO_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$KIBO_HOME")"
  KIBO_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$KIBO_CONFIG_DIR")"
  KIBO_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$KIBO_WORKSPACE_DIR")"
  KIBO_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$KIBO_IMAGE")"
  KIBO_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$KIBO_CONTAINER_NAME")"
  sed \
    -e "s|{{KIBO_HOME}}|$KIBO_HOME_ESCAPED|g" \
    -e "s|{{KIBO_CONFIG_DIR}}|$KIBO_CONFIG_ESCAPED|g" \
    -e "s|{{KIBO_WORKSPACE_DIR}}|$KIBO_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$KIBO_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$KIBO_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start kibo.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start kibo.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-kibo-podman.sh launch"
echo "  ./scripts/run-kibo-podman.sh launch setup"
echo "  kibo --container $KIBO_CONTAINER_NAME dashboard --no-open"
