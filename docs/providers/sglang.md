---
summary: "Run Kibo with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Kibo against a local SGLang server
...
Kibo can connect to SGLang using the `openai-completions` API.

Kibo can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Kibo will query:
