...
import { markKiboExecEnv } from "../../infra/kibo-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "kibo.sandbox=1");
  args.push("--label", `kibo.sessionKey=${params.scopeKey}`);
  args.push("--label", `kibo.createdAtMs=${createdAtMs}`);
  args.push("--label", `kibo.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `kibo.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markKiboExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "kibo.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`kibo sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`kibo sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("kibo sandbox recreate --all");
}
