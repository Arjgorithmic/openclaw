...
describe("createOpenAIWebSocketStreamFn", () => {
  ...
  it("keeps native turn metadata stable across websocket retries and increments attempt", async () => {
    ...
    expect(firstPayload.metadata?.kibo_session_id).toBe("sess-turn-metadata-retry");
    expect(firstPayload.metadata?.kibo_transport).toBe("websocket");
    expect(firstPayload.metadata?.kibo_turn_id).toBeTruthy();
    expect(secondPayload.metadata?.kibo_turn_id).toBe(firstPayload.metadata?.kibo_turn_id);
    expect(firstPayload.metadata?.kibo_turn_attempt).toBe("1");
    expect(secondPayload.metadata?.kibo_turn_attempt).toBe("2");
  });
