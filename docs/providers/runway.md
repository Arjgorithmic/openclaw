---
title: "Runway"
summary: "Runway video generation setup in Kibo"
read_when:
  - You want to use Runway video generation in Kibo
...
Kibo ships a bundled `runway` provider for hosted video generation.
...
```bash
kibo onboard --auth-choice runway-api-key
```

2. Set Runway as the default video provider:

```bash
kibo config set agents.defaults.videoGenerationModel.primary "runway/gen4.5"
```
