...
const EXPECTED_REPOSITORY_URL = "https://github.com/kibo/kibo";
...
const skipPackValidationEnv = "KIBO_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "kibo") {
    errors.push(`package.json name must be "kibo"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.kibo !== "kibo.mjs") {
    errors.push(
      `package.json bin.kibo must be "kibo.mjs"; found "${pkg.bin?.kibo ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`kibo-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `kibo-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
