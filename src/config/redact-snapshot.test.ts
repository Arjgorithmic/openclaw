...
import type { ConfigFileSnapshot } from "./types.kibo.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/kibo/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/kibo/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/kibo/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/kibo/nickserv-password.txt");
