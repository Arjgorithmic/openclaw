---
summary: "CLI reference for `kibo cron` (schedule and run background jobs)"
read_when:
...
Tip: run `kibo cron --help` for the full command surface.
...
Note: `kibo cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `kibo cron runs --id <job-id>` to follow the eventual outcome.

Note: `kibo cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.kibo/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`kibo doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
kibo cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
kibo cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
kibo cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
kibo cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
kibo cron run <job-id>
kibo cron run <job-id> --due
kibo cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
kibo cron edit <job-id> --agent ops
kibo cron edit <job-id> --clear-agent
kibo cron edit <job-id> --session current
kibo cron edit <job-id> --session "session:daily-brief"
```
...
```bash
kibo cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
kibo cron edit <job-id> --best-effort-deliver
kibo cron edit <job-id> --no-best-effort-deliver
kibo cron edit <job-id> --no-deliver
```
