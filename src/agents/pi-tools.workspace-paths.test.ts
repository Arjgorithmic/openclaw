...
import "./test-helpers/fast-kibo-tools.js";
...
function createExecTool(workspaceDir: string) {
  const tools = createKiboCodingTools({
    workspaceDir,
...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("kibo-ws-", async (workspaceDir) => {
      await withTempDir("kibo-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createKiboCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "kibo" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("kibo-ws-", async (workspaceDir) => {
      await withTempDir("kibo-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("kibo-ws-", async (workspaceDir) => {
      const cfg: KiboConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createKiboCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-kibo.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("kibo-sandbox-", async (sandboxDir) => {
      await withTempDir("kibo-workspace-", async (workspaceDir) => {
        ...
        const tools = createKiboCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
