...
EXTRA_MOUNTS="${KIBO_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${KIBO_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${KIBO_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${KIBO_DOCKER_SOCKET:-}"
TIMEZONE="${KIBO_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$KIBO_CONFIG_DIR/kibo.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == KIBO_GATEWAY_TOKEN=* ]]; then
      token="${line#KIBO_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${KIBO_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$KIBO_GATEWAY_PORT" "$KIBO_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$KIBO_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$KIBO_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network kibo-cli service because it
  ...
  run_prestart_gateway --entrypoint node kibo-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" kibo-cli "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "KIBO_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "KIBO_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
KIBO_CONFIG_DIR="${KIBO_CONFIG_DIR:-$HOME/.kibo}"
KIBO_WORKSPACE_DIR="${KIBO_WORKSPACE_DIR:-$HOME/.kibo/workspace}"

validate_mount_path_value "KIBO_CONFIG_DIR" "$KIBO_CONFIG_DIR"
validate_mount_path_value "KIBO_WORKSPACE_DIR" "$KIBO_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "KIBO_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "KIBO_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "KIBO_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "KIBO_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "KIBO_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "KIBO_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$KIBO_CONFIG_DIR"
mkdir -p "$KIBO_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$KIBO_CONFIG_DIR/identity"
mkdir -p "$KIBO_CONFIG_DIR/agents/main/agent"
mkdir -p "$KIBO_CONFIG_DIR/agents/main/sessions"

export KIBO_CONFIG_DIR
export KIBO_WORKSPACE_DIR
export KIBO_GATEWAY_PORT="${KIBO_GATEWAY_PORT:-18789}"
export KIBO_BRIDGE_PORT="${KIBO_BRIDGE_PORT:-18790}"
export KIBO_GATEWAY_BIND="${KIBO_GATEWAY_BIND:-lan}"
export KIBO_IMAGE="$IMAGE_NAME"
export KIBO_DOCKER_APT_PACKAGES="${KIBO_DOCKER_APT_PACKAGES:-}"
export KIBO_EXTENSIONS="${KIBO_EXTENSIONS:-}"
export KIBO_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export KIBO_HOME_VOLUME="$HOME_VOLUME_NAME"
export KIBO_ALLOW_INSECURE_PRIVATE_WS="${KIBO_ALLOW_INSECURE_PRIVATE_WS:-}"
export KIBO_SANDBOX="$SANDBOX_ENABLED"
export KIBO_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export KIBO_TZ="$TIMEZONE"
...
if [[ -z "${KIBO_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    KIBO_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $KIBO_CONFIG_DIR/kibo.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      KIBO_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      KIBO_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      KIBO_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${KIBO_CONFIG_DIR}:/home/node/.kibo"
    gateway_workspace_mount="${KIBO_WORKSPACE_DIR}:/home/node/.kibo/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${KIBO_INSTALL_DOCKER_CLI:-}" ]]; then
    export KIBO_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  KIBO_EXTRA_MOUNTS \
  KIBO_HOME_VOLUME \
  KIBO_DOCKER_APT_PACKAGES \
  KIBO_EXTENSIONS \
  KIBO_SANDBOX \
  KIBO_DOCKER_SOCKET \
  DOCKER_GID \
  KIBO_INSTALL_DOCKER_CLI \
  KIBO_ALLOW_INSECURE_PRIVATE_WS \
  KIBO_TZ

if [[ "$IMAGE_NAME" == "kibo:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "KIBO_DOCKER_APT_PACKAGES=${KIBO_DOCKER_APT_PACKAGES}" \
    --build-arg "KIBO_EXTENSIONS=${KIBO_EXTENSIONS}" \
    --build-arg "KIBO_INSTALL_DOCKER_CLI=${KIBO_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Kibo metadata subdirectory
# (.kibo/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh kibo-gateway -c \
  'find /home/node/.kibo -xdev -exec chown node:node {} +; \
   [ -d /home/node/.kibo/workspace/.kibo ] && chown -R node:node /home/node/.kibo/workspace/.kibo || true'

echo ""
echo "==> Onboarding (interactive)"
echo "Docker setup pins Gateway mode to local."
echo "Gateway runtime bind comes from KIBO_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $KIBO_GATEWAY_BIND"
echo "Gateway token: $KIBO_GATEWAY_TOKEN"
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm kibo-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm kibo-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm kibo-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.kibo.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d kibo-gateway

# --- Sandbox setup (opt-in via KIBO_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: kibo-sandbox:bookworm-slim"
    run_docker_build \
      -t "kibo-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker kibo-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg KIBO_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (KIBO_IMAGE=kibo:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: KIBO_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.kibo.ai/gateway/sandboxing"
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate kibo-gateway
  fi
else
...
echo "Config: $KIBO_CONFIG_DIR"
echo "Workspace: $KIBO_WORKSPACE_DIR"
echo "Token: $KIBO_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f kibo-gateway"
echo "  ${COMPOSE_HINT} exec kibo-gateway node dist/index.js health --token \"$KIBO_GATEWAY_TOKEN\""
