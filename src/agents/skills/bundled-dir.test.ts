...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["KIBO_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns KIBO_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-bundled-override-"));
    process.env.KIBO_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "kibo" }));
