...
import { createEmbeddedShellRunner, resolveShellCwd } from "./shell-runner.js";

describe("resolveShellCwd", () => {
  it("defaults to the current working directory", () => {
    expect(resolveShellCwd(undefined)).toBe(process.cwd());
  });

  it("keeps relative paths inside the repo root", () => {
    expect(resolveShellCwd("extensions/shell")).toBe(
      path.resolve(process.cwd(), "extensions/shell"),
    );
  });
});

describe("createEmbeddedShellRunner", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("runs inline pipelines through the embedded runtime", async () => {
    ...
    const runner = createEmbeddedShellRunner({
  ...
  it("detects workflow files and parses argsJson", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-shell-runner-"));
    const workflowPath = path.join(tempDir, "workflow.shell");
  ...
  it("loads the embedded runtime once per runner", async () => {
    ...
    const runner = createEmbeddedShellRunner({ loadRuntime });
