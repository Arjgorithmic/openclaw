    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "kibo.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "kibo.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "kibo.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("kibo-");
  const acpxRoot = await tempDirs.make("kibo-acpx-plugin-");
  const helperRoot = await tempDirs.make("kibo-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("kibo-");
  const pluginRoot = await tempDirs.make("kibo-plugin-");
  const outsideDir = await tempDirs.make("kibo-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("kibo-");
    const pluginRoot = await tempDirs.make("kibo-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("kibo-");
    const pluginRoot = await tempDirs.make("kibo-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
