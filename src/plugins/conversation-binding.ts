...
const APPROVALS_PATH = "~/.kibo/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "kibo-app-server:thread:",
  "kibo-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("kibo.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("kibo.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Kibo instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
