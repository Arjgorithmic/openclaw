...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("kibo-daemon-install-int-");
    configPath = path.join(tempHome, "kibo.json");
    process.env.HOME = tempHome;
    process.env.KIBO_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.KIBO_GATEWAY_TOKEN).toBeUndefined();
  });
});
