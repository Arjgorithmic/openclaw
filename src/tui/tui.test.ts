...
describe("resolveInitialTuiAgentId", () => {
  const cfg: KiboConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/kibo" },
        { id: "ops", workspace: "/tmp/kibo/projects/ops" },
      ],
    },
  };
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run kibo devices list");
    expect(state.pairingHint).toContain("kibo devices list");
  });
