...
import { formatCliCommand, formatDocsLink } from "kibo/plugin-sdk/setup-tools";
import {
  normalizeE164,
...
export const signalDmPolicy = {
  ...
  resolveConfigKeys: (cfg: KiboConfig, accountId?: string) =>
  ...
  getCurrent: (cfg: KiboConfig, accountId?: string) =>
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "Kibo"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("kibo gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
