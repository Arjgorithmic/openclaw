...
Kibo has three related (but different) controls:
...
Use the inspector to see what Kibo is _actually_ doing:
...
- Kibo validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:kibo`: all built-in Kibo tools (excludes provider plugins)
