...
struct ChatMessageBubble: View {
    let message: KiboChatMessage
    let style: KiboChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? KiboChatTheme.userText : KiboChatTheme.assistantText
    ...
    private var inlineAttachments: [KiboChatMessageContent] {
    ...
    private var toolCalls: [KiboChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [KiboChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? KiboChatTheme.userBubble
        }
        if self.style == .onboarding {
            return KiboChatTheme.onboardingAssistantBubble
        }
        return KiboChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return KiboChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: KiboChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? KiboChatTheme.userText : KiboChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: KiboChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(KiboChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [KiboChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: KiboChatTheme.assistantText)
            }
        }
    }
}
