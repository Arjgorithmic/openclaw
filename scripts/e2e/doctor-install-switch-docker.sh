...
IMAGE_NAME="kibo-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/kibo-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/kibo-bin/systemctl

  cat > /tmp/kibo-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_log="/tmp/kibo-doctor-switch-npm-install.log"
  if ! npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz" >"$npm_log" 2>&1; then
    cat "$npm_log"
    exit 1
  fi

	  npm_bin="/tmp/npm-prefix/bin/kibo"
	  npm_root="/tmp/npm-prefix/lib/node_modules/kibo"
  ...
  run_flow() {
    ...
    local install_log="/tmp/kibo-doctor-switch-${name}-install.log"
    local doctor_log="/tmp/kibo-doctor-switch-${name}-doctor.log"

    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/kibo-switch-${name}.XXXXXX")
    ...
    unit_path="$HOME/.config/systemd/user/kibo-gateway.service"
    if [ ! -f "$unit_path" ]; then
