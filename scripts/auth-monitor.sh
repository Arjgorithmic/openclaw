...
#
# Suggested cron: */30 * * * * /home/admin/kibo/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Kibo notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., kibo-alerts)
...
STATE_FILE="$HOME/.kibo/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Kibo if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use kibo
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Kibo to $NOTIFY_PHONE..."
            kibo send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Kibo is down. Run: ssh l36 '~/kibo/scripts/mobile-reauth.sh'" "urgent"
    exit 1
