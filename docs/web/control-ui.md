...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/kibo`)
...
If the page fails to load, start the Gateway first: `kibo gateway`.
...
created. Re-run `kibo devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `kibo devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.kibo/kibo.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop kibo`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Kibo
...
```bash
kibo gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Kibo **blocks** Control UI connections without device identity.
...
```bash
KIBO_CONTROL_UI_BASE_PATH=/kibo/ pnpm ui:build
```
