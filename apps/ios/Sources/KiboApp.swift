...
@MainActor
enum KiboAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class KiboAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.kibo.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.kibo.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.kibo.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? KiboAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "kibo.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "kibo.watch.promptId"
    static let sessionKeyKey = "kibo.watch.sessionKey"
    static let actionPrimaryIDKey = "kibo.watch.action.primary.id"
    static let actionPrimaryLabelKey = "kibo.watch.action.primary.label"
    static let actionSecondaryIDKey = "kibo.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "kibo.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "kibo.watch.action.primary"
    static let actionSecondaryIdentifier = "kibo.watch.action.secondary"
    static let actionIdentifierPrefix = "kibo.watch.action."
    static let actionIDKeyPrefix = "kibo.watch.action.id."
    static let actionLabelKeyPrefix = "kibo.watch.action.label."
    static let categoryPrefix = "kibo.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: KiboWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> KiboWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return KiboWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Kibo" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [KiboWatchAction]) -> [UNNotificationAction] {
...
struct KiboApp: App {
    ...
    @UIApplicationDelegateAdaptor(KiboAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        KiboAppModelRegistry.appModel = appModel
...
extension KiboApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Kibo: installing uncaught exception handler")
