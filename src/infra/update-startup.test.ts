...
vi.mock("./kibo-root.js", async () => {
  const actual = await vi.importActual<typeof import("./kibo-root.js")>("./kibo-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "kibo-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["KIBO_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.KIBO_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveKiboPackageRoot } = await import("./kibo-root.js"));
    ...
    vi.mocked(resolveKiboPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveKiboPackageRoot).mockResolvedValue("/opt/kibo");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/kibo/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
