...
const MATRIX_REPO_INSTALL_COMMAND = `kibo plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    ...
    expect(
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/kibo-matrix-missing",
      'Reinstall with "kibo plugins install @kibo/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/kibo-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<kibo plugins install @kibo/matrix>" when you want to return to the standard Matrix plugin.',
