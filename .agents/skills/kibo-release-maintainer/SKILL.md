---
name: kibo-release-maintainer
description: Maintainer workflow for Kibo releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# Kibo Release Maintainer
...
- Core `kibo` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/Kibo/Resources/Info.plist`
...
- Every Kibo release ships the npm package and macOS app together.
- The production Sparkle feed lives at `https://raw.githubusercontent.com/kibo/kibo/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `kibo` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `kibo YYYY.M.D-beta.N`
...
```bash
  KIBO_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/kibo-npm-postpublish-verify.ts <published-version>
```
...
- Always validate the Kibo npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `kibo/kibo` and the real mac preflight in
  `kibo/releases-private` for every release.
...
- Kibo publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` is an explicit mode on
  `.github/workflows/kibo-npm-release.yml`, but it still needs a valid
...
- `.github/workflows/macos-release.yml` in `kibo/kibo` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `kibo/releases-private/.github/workflows/kibo-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `kibo/releases-private/.github/workflows/kibo-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@kibo/kibo-release-managers` before publish continues.
- Mac publish uses
  `kibo/releases-private/.github/workflows/kibo-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `kibo/kibo`
  automatically when `KIBO_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for Kibo releases.
- `@kibo/*` plugin publishes use a separate maintainer-only flow.
...
- Preserve the existing maintainer workflow Kibo uses: run it on a real Mac
...
9. Start `.github/workflows/kibo-npm-release.yml` with `preflight_only=true`
...
10. Start `.github/workflows/macos-release.yml` in `kibo/kibo` and wait
...
14. Start `.github/workflows/kibo-npm-release.yml` with the same tag for
...
15. Wait for `npm-release` approval from `@kibo/kibo-release-managers`.
16. If the stable release was published to `beta`, start
    `.github/workflows/kibo-npm-release.yml` again after beta validation
...
- Use `kibo-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
