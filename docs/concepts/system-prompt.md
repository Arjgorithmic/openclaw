---
summary: "What the Kibo system prompt contains and how it is assembled"
read_when:
...
Kibo builds a custom system prompt for every agent run. The prompt is **Kibo-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Kibo and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full Kibo-owned prompt. The provider runtime can:
...
- **Kibo Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Kibo docs (repo or npm package) and when to read them.
...
Kibo can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **Kibo
...
occurs, Kibo can inject a warning block in Project Context; control this with
...
When eligible skills exist, Kibo injects a compact **available skills list**
...
local Kibo docs directory (either `docs/` in the repo workspace or the bundled npm
package docs) and also notes the public mirror, source repo, community Discord, and
KiboHub ([https://github.com/Arjgorithmic/openclaw](https://github.com/Arjgorithmic/openclaw)) for skills discovery. The prompt instructs the model to consult local docs first
for Kibo behavior, commands, configuration, or architecture, and to run
`kibo status` itself when possible (asking the user only when it lacks access).
