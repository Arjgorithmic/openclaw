...
BASE_IMAGE="${BASE_IMAGE:-kibo-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-kibo-sandbox-common:bookworm-slim}"
...
KIBO_DOCKER_BUILD_USE_BUILDX="${KIBO_DOCKER_BUILD_USE_BUILDX:-0}"
KIBO_DOCKER_BUILD_CACHE_FROM="${KIBO_DOCKER_BUILD_CACHE_FROM:-}"
KIBO_DOCKER_BUILD_CACHE_TO="${KIBO_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${KIBO_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${KIBO_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${KIBO_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${KIBO_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${KIBO_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=kibo.sandbox=1)
NOTE
