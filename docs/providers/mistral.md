---
summary: "Use Mistral models and Voxtral transcription with Kibo"
read_when:
  - You want to use Mistral models in Kibo
...
Kibo supports Mistral for both text/image model routing (`mistral/...`) and
...
```bash
kibo onboard --auth-choice mistral-api-key
# or non-interactive
kibo onboard --mistral-api-key "$MISTRAL_API_KEY"
```
...
Kibo currently ships this bundled Mistral catalog:
...
Kibo maps the session **thinking** level to Mistral’s API:
