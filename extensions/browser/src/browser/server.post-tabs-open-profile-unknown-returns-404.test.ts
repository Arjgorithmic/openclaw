...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createKibo = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "legacykibo", driver: "kibo" }),
    });
    expect(createKibo.status).toBe(200);
    const createKiboBody = (await createKibo.json()) as {
      profile?: string;
      transport?: string;
      cdpPort?: number | null;
      userDataDir?: string | null;
    };
    expect(createKiboBody.profile).toBe("legacykibo");
    expect(createKiboBody.transport).toBe("cdp");
    expect(createKiboBody.cdpPort).toBeTypeOf("number");
    expect(createKiboBody.userDataDir).toBeNull();

    const explicitUserDataDir = "/tmp/kibo-brave-profile";
    ...
    const deleteDefault = await realFetch(`${base}/profiles/kibo`, {
      method: "DELETE",
    });
