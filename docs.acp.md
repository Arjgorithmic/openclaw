...
This document describes how the Kibo ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`kibo acp` exposes an ACP agent over stdio and forwards prompts to a running
Kibo Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Kibo Gateway
...
- ACP client spawns `kibo acp` and speaks ACP messages over stdio.
...
`kibo acp` resolves the Gateway URL and auth from CLI flags or config:
