...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): KiboConfig {
    const defaults: NonNullable<NonNullable<KiboConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: KiboConfig) {
