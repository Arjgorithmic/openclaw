import { createAccountStatusSink } from "kibo/plugin-sdk/channel-lifecycle";
import { runStoppablePassiveMonitor } from "kibo/plugin-sdk/extension-shared";
    ...
    if (changed) {
      if (nextSection && Object.keys(nextSection).length > 0) {
        ...
        if (Object.keys(nextChannels).length > 0) {
          nextCfg.channels = nextChannels as KiboConfig["channels"];
        } else {
