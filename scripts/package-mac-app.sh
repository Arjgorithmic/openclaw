...
# Build and bundle Kibo into a minimal .app we can open.
# Outputs to dist/Kibo.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Kibo.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Kibo"
BUNDLE_ID="${BUNDLE_ID:-ai.kibo.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/kibo/kibo/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Kibo/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :KiboBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :KiboGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Kibo"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Kibo"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Kibo"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Kibo" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Kibo/Resources/Kibo.icns" "$APP_ROOT/Contents/Resources/Kibo.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Kibo/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying KiboKit resources"
KIBOKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/KiboKit_KiboKit.bundle"
if [ -d "$KIBOKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/KiboKit_KiboKit.bundle"
  cp -R "$KIBOKIT_BUNDLE" "$APP_ROOT/Contents/Resources/KiboKit_KiboKit.bundle"
else
  echo "WARN: KiboKit resource bundle not found at $KIBOKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Kibo"
killall -q Kibo 2>/dev/null || true
