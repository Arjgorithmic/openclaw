...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers KIBO_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        KIBO_HANDSHAKE_TIMEOUT_MS: "75",
        KIBO_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        KIBO_HANDSHAKE_TIMEOUT_MS: "",
        KIBO_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
  ...
  test("getConnectChallengeTimeoutMsFromEnv reads KIBO_CONNECT_CHALLENGE_TIMEOUT_MS", () => {
    expect(getConnectChallengeTimeoutMsFromEnv({})).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ KIBO_CONNECT_CHALLENGE_TIMEOUT_MS: "15000" }),
    ).toBe(15_000);
    expect(
      getConnectChallengeTimeoutMsFromEnv({ KIBO_CONNECT_CHALLENGE_TIMEOUT_MS: "garbage" }),
    ).toBeUndefined();
  });

  test("resolveConnectChallengeTimeoutMs falls back to env override", () => {
    const original = process.env.KIBO_CONNECT_CHALLENGE_TIMEOUT_MS;
    try {
      process.env.KIBO_CONNECT_CHALLENGE_TIMEOUT_MS = "5000";
      ...
      if (original === undefined) {
        delete process.env.KIBO_CONNECT_CHALLENGE_TIMEOUT_MS;
      } else {
        process.env.KIBO_CONNECT_CHALLENGE_TIMEOUT_MS = original;
      }
    }
  });
});
