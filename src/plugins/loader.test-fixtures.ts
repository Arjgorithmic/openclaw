...
import { clearPluginLoaderCache, loadKiboPlugins } from "./loader.js";
import { clearPluginManifestRegistryCache } from "./manifest-registry.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadKiboPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadKiboPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "kibo-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.KIBO_BUNDLED_PLUGINS_DIR;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "kibo.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".kibo", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ KIBO_STATE_DIR: stateDir, ...params.env }, () =>
    loadKiboPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.KIBO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
}
