...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Kibo to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Kibo releases, so no separate plugin install
is required.
...
```bash
kibo plugins install @kibo/feishu
```
...
If you just installed Kibo, run onboarding:
...
2. Configuring app credentials in Kibo
...
1. You already ran `kibo channels add` for Feishu
2. The gateway is running (`kibo gateway status`)
...
```bash
kibo pairing approve feishu <CODE>
```
...
2. Run `kibo logs --follow` and look for `chat_id`
...
2. Run `kibo logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `kibo gateway status`  | Show gateway status           |
| `kibo gateway install` | Install/start gateway service |
| `kibo gateway stop`    | Stop gateway service          |
| `kibo gateway restart` | Restart gateway service       |
| `kibo logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `kibo gateway status`
6. Check logs: `kibo logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "kibo-fan",
        workspace: "/home/user/kibo-fan",
        agentDir: "/home/user/.kibo/agents/kibo-fan/agent",
      },
      {
        id: "kibo-xi",
        workspace: "/home/user/kibo-xi",
        agentDir: "/home/user/.kibo/agents/kibo-xi/agent",
      },
    ],
  },
  bindings: [
