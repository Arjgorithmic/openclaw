...
export function parseKiboVersion(raw: string | null | undefined): KiboVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeKiboVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseKiboVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameKiboStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseKiboVersion(a);
  const parsedB = parseKiboVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareKiboVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseKiboVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseKiboVersion(current);
  const parsedTouched = parseKiboVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameKiboStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareKiboVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: KiboVersion): number {
