...
- Wake word gating (default "kibo" plus aliases) with bypass flag `--no-wake`.
...
- CLI implemented with Commander (SwiftPM package `kibo/Commander`); core types are available via the SwiftPM library product `Swabble` for embedding.
