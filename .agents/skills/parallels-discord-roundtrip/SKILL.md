...
```bash
export KIBO_PARALLELS_DISCORD_TOKEN="$(
  ssh kibos-mac-studio-1 'jq -r ".channels.discord.token" ~/.kibo/kibo.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env KIBO_PARALLELS_DISCORD_TOKEN \
...
- Use the `kibo` wrapper for guest `message send/read`; `node kibo.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/kibo-parallels-smoke.*`
...
- host inbound nonce appears in `kibo message read` output
