...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Kibo Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(kibo completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(kibo completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.kibo.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $KIBO_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "kibo") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
