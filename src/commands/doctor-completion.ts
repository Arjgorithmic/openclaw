...
async function generateCompletionCache(): Promise<boolean> {
  const root = await resolveKiboPackageRoot({
...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(kibo completion ...)` */
 ...
 */
export async function ensureCompletionCacheExists(binName = "kibo"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
