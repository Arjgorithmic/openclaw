...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: KiboConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.KIBO_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.KIBO_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set KIBO_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: kibo doctor --fix",
