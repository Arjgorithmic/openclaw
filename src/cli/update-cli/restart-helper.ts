...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.KIBO_SYSTEMD_UNIT);
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.KIBO_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.KIBO_LAUNCHD_LABEL);
  if (override) {
    return override;
  }
...
function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.KIBO_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.KIBO_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `kibo-restart-${timestamp}.sh`;
...
`;
      ...
      filename = `kibo-restart-${timestamp}.bat`;
