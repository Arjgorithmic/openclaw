...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.KIBO_CONFIG_PATH;

  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "kibo-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.KIBO_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
