...
const serveKiboChannelMcp = mocks.serveKiboChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveKiboChannelMcp: mocks.serveKiboChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("kibo-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("kibo-cli-mcp-home-", async () => {
      ...
      expect(serveKiboChannelMcp).toHaveBeenCalledWith({
