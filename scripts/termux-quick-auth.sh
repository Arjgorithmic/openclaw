...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/kibo/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Kibo Re-Auth" -c "After getting key, run: ssh $SERVER '~/kibo/scripts/mobile-reauth.sh'" --id kibo-auth
        ;;
    *)
