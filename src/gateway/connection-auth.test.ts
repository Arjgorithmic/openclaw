...
function cfg(input: Partial<KiboConfig>): KiboConfig {
  return input as KiboConfig;
}
...
const DEFAULT_ENV = {
  ...
  KIBO_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when KIBO env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when KIBO env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when KIBO env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
