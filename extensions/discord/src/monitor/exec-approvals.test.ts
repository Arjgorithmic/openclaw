...
vi.mock("kibo/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("kibo/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<KiboConfig["channels"]>["discord"]>["execApprovals"],
): KiboConfig {
  return {
    channels: {
      discord: {
