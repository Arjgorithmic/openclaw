...
const { makeStorePath } = createCronStoreHarness({ prefix: "kibo-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses KIBO_HOME for tilde expansion", () => {
    vi.stubEnv("KIBO_HOME", "/srv/kibo-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/kibo-home", "cron", "jobs.json"));
  });
});
