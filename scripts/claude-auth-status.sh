...
# Checks both Claude Code and Kibo auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
KIBO_AUTH="$HOME/.kibo/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    kibo models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    kibo_status=$(check_kibo_auth 2>/dev/null || true)

    claude_expires=0
    kibo_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        kibo_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        kibo_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$KIBO_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            kibo: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$kibo_status" == EXPIRED* ]] || [[ "$kibo_status" == MISSING* ]]; then
        echo "KIBO_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$kibo_status" == EXPIRING* ]]; then
        echo "KIBO_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Kibo Auth (~/.kibo/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$KIBO_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$KIBO_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'kibo doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active kibo >/dev/null 2>&1; then
    echo -e "Kibo service: ${GREEN}running${NC}"
else
    echo -e "Kibo service: ${RED}NOT running${NC}"
fi
