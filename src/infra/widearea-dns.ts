...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.KIBO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*kibo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "kibo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "kibo");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "kibo-gw");
  ...
  records.push(`_kibo-gw._tcp IN PTR ${instanceLabel}._kibo-gw._tcp`);
  records.push(`${instanceLabel}._kibo-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._kibo-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; kibo-content-hash: ${contentHash}\n${contentBody}`;
}
