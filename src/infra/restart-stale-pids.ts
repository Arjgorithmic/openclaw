 ...
 * returning control to the caller (typically `triggerKiboRestart` →
...
/**
 * Parse kibo gateway PIDs from lsof -Fpc stdout.
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (
        currentPid != null &&
        currentCmd &&
        normalizeLowercaseStringOrEmpty(currentCmd).includes("kibo")
      ) {
...
/**
 * Windows: find listening PIDs on the port, then verify each is an kibo
 ...
 * Returns only PIDs that belong to kibo gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only kibo gateway processes.
