...
- `kibo gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `kibo doctor` reports no blocking config/service issues.
- `kibo channels status --probe` shows live per-account transport status and,
...
kibo logs --follow
kibo models status
kibo config get agents.defaults.models
```
...
- Kibo model runs fail only on normal agent turns
...
kibo infer model run --model <provider/model> --prompt "hi" --json
kibo logs --follow
```

Look for:

- direct tiny calls succeed, but Kibo runs fail only on larger prompts
...
- direct tiny requests succeed, but Kibo agent runs fail with backend/model
  crashes (for example Gemma on some `inferrs` builds) → Kibo transport is
...
2. Set `compat.supportsTools: false` for models/backends that cannot handle
   Kibo's tool schema surface reliably.
3. Lower prompt pressure where possible: smaller workspace bootstrap, shorter
   ...
   support.
4. If tiny direct requests keep passing while Kibo agent turns still crash
...
kibo status
kibo channels status --probe
kibo pairing list --channel <channel> [--account <id>]
kibo config get channels
kibo logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `kibo config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `kibo devices list` then `kibo devices approve <requestId>`.                                                                                                                                                                                            |
...
If `kibo devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `kibo devices rotate --scope ...` can only request operator scopes that
...
kibo logs --follow
kibo doctor
kibo gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `kibo onboard --mode local` / `kibo setup` to restamp the expected local-mode config. If you are running Kibo via Podman, the default config path is `~/.kibo/kibo.json`.
...
Use this when `kibo gateway probe` reaches something, but still prints a warning block.

```bash
kibo gateway probe
kibo gateway probe --json
kibo gateway probe --ssh user@gateway-host
```
...
kibo cron status
kibo cron list
kibo cron runs --id <jobId> --limit 20
kibo system heartbeat last
kibo logs --follow
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Kibo skips the model call.
...
kibo approvals get --node <idOrNameOrIp>
kibo logs --follow
kibo status
```
...
```bash
kibo browser status
kibo browser start --browser-profile kibo
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `kibo browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
kibo config get gateway.auth.mode
kibo config get gateway.auth.token
kibo gateway status
kibo logs --follow
```
