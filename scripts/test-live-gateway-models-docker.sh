...
elif [[ -n "${KIBO_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(kibo_live_collect_auth_dirs_from_csv "${KIBO_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(kibo_live_collect_auth_files_from_csv "${KIBO_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e KIBO_LIVE_TEST=1 \
  -e KIBO_LIVE_GATEWAY_MODELS="${KIBO_LIVE_GATEWAY_MODELS:-modern}" \
  -e KIBO_LIVE_GATEWAY_PROVIDERS="${KIBO_LIVE_GATEWAY_PROVIDERS:-}" \
  -e KIBO_LIVE_GATEWAY_SMOKE="${KIBO_LIVE_GATEWAY_SMOKE:-1}" \
  -e KIBO_LIVE_GATEWAY_MAX_MODELS="${KIBO_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e KIBO_LIVE_GATEWAY_STEP_TIMEOUT_MS="${KIBO_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e KIBO_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${KIBO_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
  -v "$ROOT_DIR":/src:ro \
