...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Kibo
...
Running Kibo via Node with `tsx` fails at startup with:

```
[kibo] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `kibo status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch kibo.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node kibo.mjs status` works on Node 25.
