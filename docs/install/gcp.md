---
summary: "Run Kibo Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Kibo running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Kibo on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Kibo Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Kibo 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Kibo Gateway in Docker
- Persist `~/.kibo` + `~/.kibo/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.kibo` state includes `kibo.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-kibo-project --name="Kibo Gateway"
    gcloud config set project my-kibo-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create kibo-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh kibo-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Kibo repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    KIBO_IMAGE=kibo:latest
    KIBO_GATEWAY_TOKEN=change-me-now
    KIBO_GATEWAY_BIND=lan
    KIBO_GATEWAY_PORT=18789

    KIBO_CONFIG_DIR=/home/$USER/.kibo
    KIBO_WORKSPACE_DIR=/home/$USER/.kibo/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.kibo
    ```
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `KIBO_GATEWAY_TOKEN`.
    ...
    services:
      kibo-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - KIBO_GATEWAY_BIND=${KIBO_GATEWAY_BIND}
          - KIBO_GATEWAY_PORT=${KIBO_GATEWAY_PORT}
          - KIBO_GATEWAY_TOKEN=${KIBO_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${KIBO_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`KIBO_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm kibo-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh kibo-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop kibo-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type kibo-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start kibo-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create kibo-deploy \
     --display-name="Kibo Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-kibo-project \
     --member="serviceAccount:kibo-deploy@my-kibo-project.iam.gserviceaccount.com" \
