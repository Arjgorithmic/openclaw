---
summary: "Install, configure, and manage Kibo plugins"
read_when:
...
Plugins extend Kibo with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with Kibo), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ```bash
    # From npm
    kibo plugins install @kibo/voice-call

    # From a local directory or archive
    kibo plugins install ./my-plugin
    kibo plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install kibohub:@kibo/voice-call
/plugin show voice-call
/plugin enable voice-call
```

The install path uses the same resolver as the CLI: local path/archive, explicit
`kibohub:<pkg>`, or bare package spec (KiboHub first, then npm fallback).

If config is invalid, install normally fails closed and points you at
`kibo doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`kibo.install.allowInvalidConfigRecovery`.

## Plugin types

Kibo recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `kibo.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to Kibo features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `kibo plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@kibo/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@kibo/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@kibo/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@kibo/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@kibo/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@kibo/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with Kibo)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `kibo browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `kibo gateway` path), that
...
Kibo scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.kibo/<plugin-root>/*.ts` and `\<workspace\>/.kibo/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.kibo/<plugin-root>/*.ts` and `~/.kibo/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with Kibo. Many are enabled by default (model providers, speech).
...
```bash
kibo plugins list                       # compact inventory
kibo plugins list --enabled            # only loaded plugins
kibo plugins list --verbose            # per-plugin detail lines
kibo plugins list --json               # machine-readable inventory
kibo plugins inspect <id>              # deep detail
kibo plugins inspect <id> --json       # machine-readable
kibo plugins inspect --all             # fleet-wide table
kibo plugins info <id>                 # inspect alias
kibo plugins doctor                    # diagnostics

kibo plugins install <package>         # install (KiboHub first, then npm)
kibo plugins install kibohub:<pkg>     # install from KiboHub only
kibo plugins install <spec> --force    # overwrite existing install
kibo plugins install <path>            # install from local path
kibo plugins install -l <path>         # link (no copy) for dev
kibo plugins install <plugin> --marketplace <source>
kibo plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
kibo plugins install <spec> --pin      # record exact resolved npm spec
kibo plugins install <spec> --dangerously-force-unsafe-install
kibo plugins update <id>             # update one plugin
kibo plugins update <id> --dangerously-force-unsafe-install
kibo plugins update --all            # update all
kibo plugins uninstall <id>          # remove config/install records
kibo plugins uninstall <id> --keep-files
kibo plugins marketplace list <source>
kibo plugins marketplace list <source> --json

kibo plugins enable <id>
kibo plugins disable <id>
```

Bundled plugins ship with Kibo. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `kibo plugins enable <id>`.
...
override instead, while `kibo skills install` remains the separate KiboHub
...
`kibo plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`kibo plugins` CLI reference](/cli/plugins) for full details.
...
Kibo loads the entry object and calls `register(api)` during plugin
