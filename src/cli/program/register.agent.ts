...
${formatHelpExamples([
  ['kibo agent --to +15555550123 --message "status update"', "Start a new session."],
  ['kibo agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'kibo agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'kibo agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['kibo agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'kibo agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.kibo.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.kibo.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['kibo agents set-identity --agent main --name "Kibo" --emoji "🦞"', "Set name + emoji."],
  ["kibo agents set-identity --agent main --avatar avatars/kibo.png", "Set avatar path."],
  [
    "kibo agents set-identity --workspace ~/.kibo/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "kibo agents set-identity --identity-file ~/.kibo/workspace/IDENTITY.md --agent main",
