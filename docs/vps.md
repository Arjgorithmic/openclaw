---
summary: "Run Kibo on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Kibo
title: "Linux Server"
...
Run the Kibo Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/kibo-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `KIBO_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `KIBO_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/kibo-compile-cache`
...
For the standard `kibo onboard --install-daemon` path, edit the user unit:
...
`kibo-gateway.service` via `sudo systemctl edit kibo-gateway.service`.
