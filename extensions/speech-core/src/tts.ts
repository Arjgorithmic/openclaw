...
import { normalizeChannelId, type ChannelId } from "kibo/plugin-sdk/channel-targets";
import type {
  KiboConfig,
  TtsAutoMode,
  TtsConfig,
...
import { isVerbose, logVerbose } from "kibo/plugin-sdk/runtime-env";
import { resolvePreferredKiboTmpDir } from "kibo/plugin-sdk/sandbox";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.KIBO_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: KiboConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: KiboConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: KiboConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: KiboConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: KiboConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: KiboConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredKiboTmpDir();
