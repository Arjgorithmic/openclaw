...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __kiboDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__kiboDiagnosticEventsState) {
    globalStore.__kiboDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__kiboDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: KiboConfig): boolean {
