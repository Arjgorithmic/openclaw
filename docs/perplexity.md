...
Kibo supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Kibo also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `kibo configure --section web`. It stores the key in
`~/.kibo/kibo.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.kibo/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
