...
import type { KiboConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://github.com/Arjgorithmic/openclaw/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: KiboConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: KiboConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: KiboConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = KiboSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as KiboConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: KiboConfig | null | undefined;
  ...
  const ensureCompatConfig = (): KiboConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Kibo config schema.
