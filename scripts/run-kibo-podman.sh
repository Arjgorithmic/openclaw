#!/usr/bin/env bash
# Rootless Kibo in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-kibo-podman.sh launch        # Start gateway
#   ./scripts/run-kibo-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   kibo --container kibo dashboard --no-open
#   kibo --container kibo channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      KIBO_GATEWAY_TOKEN|KIBO_PODMAN_CONTAINER|KIBO_PODMAN_IMAGE|KIBO_IMAGE|KIBO_PODMAN_PULL|KIBO_PODMAN_GATEWAY_HOST_PORT|KIBO_GATEWAY_PORT|KIBO_PODMAN_BRIDGE_HOST_PORT|KIBO_BRIDGE_PORT|KIBO_GATEWAY_BIND|KIBO_PODMAN_USERNS|KIBO_BIND_MOUNT_OPTIONS|KIBO_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-kibo-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${KIBO_CONFIG_DIR:-$EFFECTIVE_HOME/.kibo}"
ENV_FILE="${KIBO_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${KIBO_CONFIG_DIR:-$EFFECTIVE_HOME/.kibo}"
ENV_FILE="${KIBO_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${KIBO_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${KIBO_PODMAN_CONTAINER:-kibo}"
KIBO_IMAGE="${KIBO_PODMAN_IMAGE:-${KIBO_IMAGE:-kibo:local}}"
PODMAN_PULL="${KIBO_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${KIBO_PODMAN_GATEWAY_HOST_PORT:-${KIBO_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${KIBO_PODMAN_BRIDGE_HOST_PORT:-${KIBO_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${KIBO_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$KIBO_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v kibo >/dev/null 2>&1; then
    return 0
  fi
  KIBO_CONTAINER="" KIBO_CONFIG_DIR="$config_dir" \
    kibo config get gateway.bind 2>/dev/null || true
}
...
# KIBO_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${KIBO_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v kibo >/dev/null 2>&1; then
    echo "Warning: kibo not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    KIBO_CONTAINER="" KIBO_CONFIG_DIR="$config_dir" \
      kibo config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    KIBO_CONTAINER="" KIBO_CONFIG_DIR="$config_dir" \
      kibo config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  KIBO_CONTAINER="" KIBO_CONFIG_DIR="$config_dir" \
    kibo config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${KIBO_GATEWAY_TOKEN:-}" ]]; then
  export KIBO_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "KIBO_GATEWAY_TOKEN" "$KIBO_GATEWAY_TOKEN"
  echo "Generated KIBO_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/kibo.json"
...
PODMAN_USERNS="${KIBO_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported KIBO_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (KIBO_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${KIBO_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${KIBO_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$KIBO_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.kibo/.npm \
    -e KIBO_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.kibo:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.kibo/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$KIBO_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME kibo dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME kibo devices approve --latest  # if pairing required"
