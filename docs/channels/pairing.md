...
read_when:
  ...
  - Reviewing Kibo security posture
title: "Pairing"
---

# Pairing

“Pairing” is Kibo’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `kibo-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.kibo/credentials/`:
...
3. On your phone, open the Kibo iOS app → Settings → Gateway.
...
- The legacy `node.pair.*` API (CLI: `kibo nodes pending|approve|reject|rename`) is a
