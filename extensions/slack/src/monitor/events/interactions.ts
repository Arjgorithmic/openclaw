...
// Prefix for Kibo-generated action IDs to scope our handler
const KIBO_ACTION_PREFIX = "kibo:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${KIBO_ACTION_PREFIX}`);

  // Handle Kibo modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
