---
summary: "Use Anthropic Claude via API keys or Claude CLI in Kibo"
read_when:
  - You want to use Anthropic models in Kibo
title: "Anthropic"
---
...
Claude CLI. In Kibo, Anthropic API keys and Claude CLI reuse are both
...
Kibo treats Claude CLI reuse and `claude -p` usage as sanctioned for this
...
Kibo can reuse that login directly.
...
Kibo also supports other subscription-style options, including [OpenAI
...
# or non-interactive
kibo onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
```
...
- Anthropic Claude 4.6 models default to `adaptive` thinking in Kibo when no explicit thinking level is set.
...
Kibo's shared `/fast` toggle also supports direct public Anthropic traffic, including API-key and OAuth-authenticated requests sent to `api.anthropic.com`.
...
- Kibo only injects Anthropic service tiers for direct `api.anthropic.com` requests. If you route `anthropic/*` through a proxy or gateway, `/fast` leaves `service_tier` untouched.
...
Kibo supports Anthropic's prompt caching feature. This is **API-only**; legacy Anthropic token auth does not honor cache settings.
...
When using Anthropic API Key authentication, Kibo automatically applies `cacheRetention: "short"` (5-minute cache) for all Anthropic models. You can override this by explicitly setting `cacheRetention` in your config.
...
Anthropic's 1M context window is beta-gated. In Kibo, enable it per model
...
Kibo maps this to `anthropic-beta: context-1m-2025-08-07` on Anthropic
requests.
...
`context1m: true` with that legacy auth mode, Kibo logs a warning and
falls back to the standard context window by skipping the context1m beta
header while keeping the required OAuth betas.

## Claude CLI backend

The bundled Anthropic `claude-cli` backend is supported in Kibo.

- Anthropic staff told us this usage is allowed again.
- Kibo therefore treats Claude CLI reuse and `claude -p` usage as
...
- Anthropic's public Claude Code docs still document direct CLI usage such as
  `claude -p`, and Anthropic staff told us Kibo-style Claude CLI usage is
  allowed again. We are treating that guidance as settled unless Anthropic
  publishes a new policy change.
- Anthropic setup-token remains available in Kibo as a supported token-auth path, but Kibo now prefers Claude CLI reuse and `claude -p` when available.
...
- Re-run onboarding for that agent, or configure an API key on the gateway
  host, then verify with `kibo models status`.

**No credentials found for profile `anthropic:default`**

- Run `kibo models status` to see which auth profile is active.
- Re-run onboarding, or configure an API key for that profile path.

**No available auth profile (all in cooldown/unavailable)**

- Check `kibo models status --json` for `auth.unusableProfiles`.
