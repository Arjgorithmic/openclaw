...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "kibo.exec-approval"
    static let reviewActionIdentifier = "kibo.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.kiboPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.kiboPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func kiboPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
