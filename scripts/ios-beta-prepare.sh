...
- regenerates apps/ios/Kibo.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${KIBO_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid KIBO_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid KIBO_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid KIBO_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid KIBO_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing KIBO_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(KIBO_URL_SLASH)$(KIBO_URL_SLASH)#g'
)"
...
KIBO_CODE_SIGN_STYLE = Automatic
KIBO_DEVELOPMENT_TEAM = ${TEAM_ID}
KIBO_IOS_SELECTED_TEAM = ${TEAM_ID}
KIBO_APP_BUNDLE_ID = ai.kibo.client
KIBO_SHARE_BUNDLE_ID = ai.kibo.client.share
KIBO_ACTIVITY_WIDGET_BUNDLE_ID = ai.kibo.client.activitywidget
KIBO_WATCH_APP_BUNDLE_ID = ai.kibo.client.watchkitapp
KIBO_WATCH_EXTENSION_BUNDLE_ID = ai.kibo.client.watchkitapp.extension
KIBO_APP_PROFILE =
KIBO_SHARE_PROFILE =
KIBO_PUSH_TRANSPORT = relay
KIBO_PUSH_DISTRIBUTION = official
KIBO_URL_SLASH = /
KIBO_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
KIBO_PUSH_APNS_ENVIRONMENT = production
EOF
