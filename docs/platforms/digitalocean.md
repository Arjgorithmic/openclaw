---
summary: "Kibo on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Kibo on DigitalOcean
  - Looking for cheap VPS hosting for Kibo
title: "DigitalOcean (Platform)"
---

# Kibo on DigitalOcean

## Goal

Run a persistent Kibo Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
kibo config set gateway.bind tailnet
kibo gateway restart
```
...
- `~/.kibo/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u kibo-gateway.service --no-pager -n 50
```
