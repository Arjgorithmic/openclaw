...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "kibo-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  it("keeps writes inside an KIBO_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".kibo", "kibo.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "kibo.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "kibo.json"), "utf-8"),
      ) as {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/kibo-test-required-plugin",
          source: "/tmp/kibo-test-required-plugin/index.ts",
          manifestPath: "/tmp/kibo-test-required-plugin/kibo.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
