...
describe("doctor command", () => {
  ...
  it("does not warn when only the active workspace is present", async () => {
    mockDoctorConfigSnapshot({
      config: {
        agents: { defaults: { workspace: "/Users/kibo/kibo" } },
      },
    });

    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/Users/kibo");
    const realExists = fs.existsSync;
    const legacyPath = path.join("/Users/kibo", "kibo");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (
        value === "/Users/kibo/kibo" ||
        value === legacyPath ||
