---
summary: "How Kibo manages conversation sessions"
read_when:
...
Kibo organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `kibo security audit`.
...
Kibo automatically bounds session storage over time. By default, it runs
...
Preview with `kibo sessions cleanup --dry-run`.

## Inspecting sessions

- `kibo status` -- session store path and recent activity.
- `kibo sessions --json` -- all sessions (filter with `--active <minutes>`).
