...
describe("provider attribution", () => {
  it("resolves the canonical Kibo product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Kibo runtime wrapper.",
      product: "Kibo",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { KIBO_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "kibo",
        version: "2026.3.22",
        "User-Agent": "kibo/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { KIBO_VERSION: "2026.3.22" })).toEqual({
      originator: "kibo",
      version: "2026.3.22",
      "User-Agent": "kibo/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { KIBO_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ KIBO_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
