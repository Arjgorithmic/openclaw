...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: KiboConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): KiboConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("kibo/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("kibo/plugin-sdk/acp-runtime")>(
    "kibo/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("kibo/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("kibo/plugin-sdk/command-auth")>(
...
vi.mock("kibo/plugin-sdk/infra-runtime", async () => {
