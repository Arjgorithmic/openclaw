...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__kibo_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "kibo-root");
...
function setPackageRoot(root: string, name = "kibo") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./kibo-root.js").resolveKiboPackageRootSync,
  asyncResolver: typeof import("./kibo-root.js").resolveKiboPackageRoot,
  opts: Parameters<typeof import("./kibo-root.js").resolveKiboPackageRootSync>[0],
...
vi.mock("./kibo-root.fs.runtime.js", () => ({
  kiboRootFsSync: mockFsModule(),
  kiboRootFs: mockFsPromisesModule(),
}));

describe("resolveKiboPackageRoot", () => {
  let resolveKiboPackageRoot: typeof import("./kibo-root.js").resolveKiboPackageRoot;
  let resolveKiboPackageRootSync: typeof import("./kibo-root.js").resolveKiboPackageRootSync;
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ resolveKiboPackageRoot, resolveKiboPackageRootSync } =
      await import("./kibo-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "kibo");
        const pkgRoot = path.join(project, "node_modules", "kibo");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "kibo");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "kibo.mjs")));
    ...
    {
      name: "falls through from a non-kibo moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-kibo");
    ...
    {
      name: "returns null for non-kibo package roots",
      setup: () => {
        const pkgRoot = fx("not-kibo");
        setPackageRoot(pkgRoot, "not-kibo");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "kibo.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "kibo");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
