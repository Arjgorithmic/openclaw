...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): KiboConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as KiboConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedDefaultAccountCase(input: KiboConfig, expected: string) {
