...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.KIBO_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "KIBO_TEST_FAST", value: process.env.KIBO_TEST_FAST },
    {
      key: "KIBO_STRICT_FAST_REPLY_CONFIG",
      value: process.env.KIBO_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "KIBO_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.KIBO_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "KIBO_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.KIBO_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "KIBO_STATE_DIR", value: process.env.KIBO_STATE_DIR },
    { key: "KIBO_CONFIG_PATH", value: process.env.KIBO_CONFIG_PATH },
    { key: "KIBO_GATEWAY_PORT", value: process.env.KIBO_GATEWAY_PORT },
    { key: "KIBO_BRIDGE_ENABLED", value: process.env.KIBO_BRIDGE_ENABLED },
    { key: "KIBO_BRIDGE_HOST", value: process.env.KIBO_BRIDGE_HOST },
    { key: "KIBO_BRIDGE_PORT", value: process.env.KIBO_BRIDGE_PORT },
    { key: "KIBO_CANVAS_HOST_PORT", value: process.env.KIBO_CANVAS_HOST_PORT },
    { key: "KIBO_TEST_HOME", value: process.env.KIBO_TEST_HOME },
    { key: "KIBO_AGENT_DIR", value: process.env.KIBO_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.KIBO_TEST_HOME = tempHome;
  process.env.KIBO_TEST_FAST = "1";
  process.env.KIBO_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.KIBO_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.KIBO_BRIDGE_ENABLED;
  delete process.env.KIBO_BRIDGE_HOST;
  delete process.env.KIBO_BRIDGE_PORT;
  delete process.env.KIBO_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.KIBO_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.KIBO_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".kibo");
  const priorIsolatedHome = params.env.KIBO_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".kibo")
  ) {
    realStateDir = path.join(params.realHome, ".kibo");
  }
  const tempStateDir = path.join(params.tempHome, ".kibo");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.KIBO_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.KIBO_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "kibo.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "kibo.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.KIBO_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.KIBO_LIVE_USE_REAL_HOME);
