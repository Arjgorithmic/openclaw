...
Kibo supports **camera capture** for agent workflows:
...
```bash
kibo nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
kibo nodes camera snap --node <id> --facing front
kibo nodes camera clip --node <id> --duration 3000
kibo nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`kibo.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `kibo` CLI to invoke camera commands on the macOS node.

Examples:

```bash
kibo nodes camera list --node <id>            # list camera ids
kibo nodes camera snap --node <id>            # prints MEDIA:<path>
kibo nodes camera snap --node <id> --max-width 1280
kibo nodes camera snap --node <id> --delay-ms 2000
kibo nodes camera snap --node <id> --device-id <id>
kibo nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
kibo nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
kibo nodes camera clip --node <id> --device-id <id>
kibo nodes camera clip --node <id> --no-audio
```

Notes:

- `kibo nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
kibo nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
