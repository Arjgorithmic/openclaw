...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/kibo-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("kibo/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("kibo/plugin-sdk/security-runtime")>(
    "kibo/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("kibo/plugin-sdk/reply-runtime"),
      import("kibo/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
