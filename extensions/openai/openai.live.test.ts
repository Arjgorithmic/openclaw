...
import type { ResolvedTtsConfig } from "kibo/plugin-sdk/agent-runtime";
import type { KiboConfig } from "kibo/plugin-sdk/config-runtime";
import { loadConfig } from "kibo/plugin-sdk/config-runtime";
import { encodePngRgba, fillPixel } from "kibo/plugin-sdk/media-runtime";
...
const LIVE_MODEL_ID = process.env.KIBO_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL = process.env.KIBO_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const LIVE_VISION_MODEL = process.env.KIBO_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.KIBO_LIVE_TEST === "1";
...
function createLiveConfig(): KiboConfig {
