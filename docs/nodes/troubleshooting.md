...
```bash
kibo nodes describe --node <idOrNameOrIp>
kibo nodes canvas snapshot --node <idOrNameOrIp>
kibo logs --follow
```
...
kibo nodes status
kibo approvals get --node <idOrNameOrIp>
kibo approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`kibo approvals get --node ...`), not in the gateway pairing record.
