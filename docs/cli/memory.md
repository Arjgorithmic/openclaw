---
summary: "CLI reference for `kibo memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
kibo memory status
kibo memory status --deep
kibo memory status --fix
kibo memory index --force
kibo memory search "meeting notes"
kibo memory search --query "deployment" --max-results 20
kibo memory promote --limit 10 --min-score 0.75
kibo memory promote --apply
kibo memory promote --json --min-recall-count 0 --min-unique-queries 0
kibo memory promote-explain "router vlan"
kibo memory promote-explain "router vlan" --json
kibo memory rem-harness
kibo memory rem-harness --json
kibo memory status --json
kibo memory status --deep --index
kibo memory status --deep --index --verbose
kibo memory status --agent main
kibo memory index --agent main --verbose
```
...
```bash
kibo memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `kibo cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
kibo memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
kibo memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
