# Kibo iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Kibo Gateway as a `role: node`.
...
- Beta release uses canonical `ai.kibo.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `KiboPushTransport=relay`, `KiboPushDistribution=official`, and `KiboPushAPNsEnvironment=production`.
...
- `KIBO_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export KIBO_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Kibo-<version>.ipa`
   - `apps/ios/build/beta/Kibo-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Kibo.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `KiboPushTransport=direct` and `KiboPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `KIBO_APNS_TEAM_ID`, `KIBO_APNS_KEY_ID`, and either `KIBO_APNS_PRIVATE_KEY_P8` or `KIBO_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.kibo/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `KIBO_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `KiboPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `kibo.json`. `KIBO_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Kibo official builds and to ensure a
