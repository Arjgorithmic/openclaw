...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.kibo/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Kibo picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    kibo agent --message "give me a greeting"
    ```
...
| `metadata.kibo.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.kibo.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.kibo.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `kibo agent --message "..."` to test before sharing
- **Use KiboHub** — browse and contribute skills at [KiboHub](https://github.com/Arjgorithmic/openclaw)
...
| `~/.kibo/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Kibo) | Low        | Global                |
...
- [KiboHub](/tools/kibohub) — public skill registry
