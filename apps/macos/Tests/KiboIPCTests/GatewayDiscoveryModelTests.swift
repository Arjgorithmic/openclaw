...
struct GatewayDiscoveryModelTests {
    ...
    @Test func `local gateway matches display name`() {
        let local = GatewayDiscoveryModel.LocalIdentity(
            hostTokens: [],
            displayTokens: ["kibo's mac studio"])
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Kibo's Mac Studio (Kibo)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func `service name does not false positive on substring host token`() {
        ...
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            ...
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (Kibo)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "kibo (Kibo)",
            local: local))
    }
    ...
    @Test func `parses gateway TXT fields`() {
        let parsed = GatewayDiscoveryModel.parseGatewayTXT([
            "lanHost": "  studio.local  ",
            "tailnetDns": "  kibos-mac-studio-1.ts.net  ",
            "sshPort": " 2222 ",
            "gatewayPort": " 18799 ",
            "cliPath": " /opt/kibo ",
        ])
        #expect(parsed.lanHost == "studio.local")
        #expect(parsed.tailnetDns == "kibos-mac-studio-1.ts.net")
        #expect(parsed.sshPort == 2222)
        #expect(parsed.gatewayPort == 18799)
        #expect(parsed.cliPath == "/opt/kibo")
    }
    ...
    @Test func `builds SSH target`() {
        #expect(GatewayDiscoveryModel.buildSSHTarget(
            user: "kibo",
            host: "studio.local",
            port: 22) == "kibo@studio.local")
        #expect(GatewayDiscoveryModel.buildSSHTarget(
            user: "kibo",
            host: "studio.local",
            port: 2201) == "kibo@studio.local:2201")
    }
    ...
    @Test func `dedupe key prefers resolved endpoint across sources`() {
        let wideArea = GatewayDiscoveryModel.DiscoveredGateway(
            ...
            gatewayPort: 443,
            cliPath: nil,
            stableID: "wide-area|kibo.internal.|gateway-host",
            debugID: "wide-area",
            isLocal: false)
