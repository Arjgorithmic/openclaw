...
function getChannelConfig(cfg: KiboConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: KiboConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: KiboConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: KiboConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Kibo uses to send replies back to Chat.",
      ],
