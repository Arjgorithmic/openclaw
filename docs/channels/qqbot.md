...
read_when:
  - You want to connect Kibo to QQ
...
QQ Bot connects to Kibo via the official QQ Bot API (WebSocket gateway). The
...
Current Kibo releases bundle QQ Bot, so normal packaged builds do not need
a separate `kibo plugins install` step.
...
```bash
kibo channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `kibo channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single Kibo instance:
...
```bash
kibo channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
