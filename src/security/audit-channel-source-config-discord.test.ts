...
function stubDiscordPlugin(params: {
  resolveAccount: (cfg: KiboConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: KiboConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: KiboConfig) => boolean;
}): ChannelPlugin {
  return {
    id: "discord",
    meta: {
...
describe("security audit channel source-config fallback discord", () => {
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    ...
    const resolvedConfig: KiboConfig = {
      commands: { native: true },
      channels: {
        discord: {
          enabled: true,
