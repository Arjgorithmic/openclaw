...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `kibo.internal.`), **or**
...
Run `kibo devices list` again before approval.

4. Verify connection:

```bash
kibo nodes status
kibo gateway call node.list --params "{}"
```
...
- `KIBO_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Kibo iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Kibo builds.
...
```bash
export KIBO_APNS_TEAM_ID="TEAMID"
export KIBO_APNS_KEY_ID="KEYID"
export KIBO_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.kibo/credentials/apns
chmod 700 ~/.kibo/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.kibo/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.kibo/credentials/apns/AuthKey_KEYID.p8
export KIBO_APNS_PRIVATE_KEY_PATH="$HOME/.kibo/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_kibo-gw._tcp` on `local.` and, when configured, the same
...
`kibo.internal.`) and Tailscale split DNS.
...
```bash
kibo nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__kibo__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__kibo__/canvas/` and `/__kibo__/a2ui/`.
...
```bash
kibo nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__kibo; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
kibo nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `kibo devices list` and approve manually.
