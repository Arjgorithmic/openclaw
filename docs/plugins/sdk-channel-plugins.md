...
summary: "Step-by-step guide to building a messaging channel plugin for Kibo"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Kibo to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Kibo to a
...
Channel plugins do not need their own send/edit/react tools. Kibo keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `kibo/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `kibo/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `kibo/plugin-sdk/approval-auth-runtime`
- `kibo/plugin-sdk/approval-client-runtime`
- `kibo/plugin-sdk/approval-delivery-runtime`
...
- `kibo/plugin-sdk/approval-native-runtime`
- `kibo/plugin-sdk/approval-reply-runtime`
- `kibo/plugin-sdk/channel-runtime-context`

Likewise, prefer `kibo/plugin-sdk/setup-runtime`,
`kibo/plugin-sdk/setup-adapter-runtime`,
`kibo/plugin-sdk/reply-runtime`,
`kibo/plugin-sdk/reply-dispatch-runtime`,
`kibo/plugin-sdk/reply-reference`, and
`kibo/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `kibo/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `kibo/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `kibo/plugin-sdk/channel-setup` covers the optional-install setup
...
- use the broader `kibo/plugin-sdk/setup` seam only when you also need the
...
- `kibo/plugin-sdk/account-core`,
  `kibo/plugin-sdk/account-id`,
  `kibo/plugin-sdk/account-resolution`, and
  `kibo/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `kibo/plugin-sdk/inbound-envelope` and
  `kibo/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `kibo/plugin-sdk/messaging-targets` for target parsing/matching
- `kibo/plugin-sdk/outbound-media` and
  `kibo/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates
- `kibo/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `kibo/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `kibo/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `kibo/plugin-sdk/channel-inbound` for the shared layer.
...
`kibo/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#kibochannel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/kibo-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "kibo": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Kibo to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { KiboConfig } from "kibo/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "kibo/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Kibo
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "kibo/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Kibo loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    Kibo. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Kibo.
...
├── package.json              # kibo.channel metadata
├── kibo.plugin.json      # Manifest with config schema
