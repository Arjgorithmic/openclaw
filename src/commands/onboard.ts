...
export async function setupWizardCommand(
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        "Read: https://github.com/Arjgorithmic/openclaw/security",
        `Re-run with: ${formatCliCommand("kibo onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected - Kibo runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://github.com/Arjgorithmic/openclaw/windows",
      ].join("\n"),
    );
  }
