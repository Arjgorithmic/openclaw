...
read_when:
  - You want to use Volcano Engine or Doubao models with Kibo
...
```bash
kibo onboard --auth-choice volcengine-api-key
```
...
`kibo onboard --auth-choice volcengine-api-key` currently sets
...
loaded yet, Kibo falls back to the unfiltered catalog instead of showing an
