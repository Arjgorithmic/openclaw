...
kibo nodes status
kibo nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.kibo`):

- `~/.kibo/nodes/paired.json`
- `~/.kibo/nodes/pending.json`

If you override `KIBO_STATE_DIR`, the `nodes/` folder moves with it.
