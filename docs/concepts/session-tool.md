...
Kibo gives agents tools to work across sessions, inspect status, and
...
After the target responds, Kibo can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned Kibo
...
the completion origin only identifies a channel Kibo can still reuse the
