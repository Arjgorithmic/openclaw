...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `KIBO_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `KIBO_GATEWAY_TOKEN` / `KIBO_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Kibo’s highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `kibo`, `kibo/default`, and `kibo/<agentId>`.
- `kibo/default` is the stable alias that always maps to the configured default agent.
- Use `x-kibo-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `KIBO_GATEWAY_PORT` → `gateway.port` → `18789` |
...
```bash
kibo gateway status
kibo gateway status --deep   # adds a system-level service scan
...
LaunchAgent labels are `ai.kibo.gateway` (default) or `ai.kibo.<profile>` (named profile). `kibo doctor` audits and repairs service config drift.
...
Native Windows managed startup uses a Scheduled Task named `Kibo Gateway`
(or `Kibo Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Kibo falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now kibo-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/kibo-gateway[-<profile>].service` and adjust
`ExecStart=` if your `kibo` binary lives elsewhere.
...
- Unique `gateway.port`
- Unique `KIBO_CONFIG_PATH`
- Unique `KIBO_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
KIBO_CONFIG_PATH=~/.kibo/a.json KIBO_STATE_DIR=~/.kibo-a kibo gateway --port 19001
KIBO_CONFIG_PATH=~/.kibo/b.json KIBO_STATE_DIR=~/.kibo-b kibo gateway --port 19002
```
...
```bash
kibo --dev setup
kibo --dev gateway --allow-unconfigured
kibo --dev status
```
