---
summary: "Deploy Kibo on Render with Infrastructure-as-Code"
read_when:
  - Deploying Kibo to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Kibo on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/kibo/kibo)
...
repository configures everything needed to run Kibo:
...
services:
  - type: web
    ...
    envVars:
      - key: KIBO_GATEWAY_PORT
        value: "8080"
      - key: KIBO_STATE_DIR
        value: /data/.kibo
      - key: KIBO_WORKSPACE_DIR
        value: /data/workspace
      - key: KIBO_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Kibo state
...
`KIBO_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Kibo repository, Render will not auto-deploy your Kibo. To update it, run a manual Blueprint sync from the dashboard.
...
For Kibo, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `KIBO_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `KIBO_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `kibo backup create` in the Render shell.
