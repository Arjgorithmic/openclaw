...
read_when:
  - You want Kibo to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Kibo to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Kibo releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`kibo onboard`) and `kibo channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
kibo plugins install @kibo/nostr
```

Use a local checkout (dev workflows):

```bash
kibo plugins install --link <path-to-local-nostr-plugin>
```
...
kibo channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "kibo@example.com",
        lud16: "kibo@example.com",
      },
    },
  },
}
