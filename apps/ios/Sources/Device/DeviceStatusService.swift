...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> KiboDeviceStatusPayload {
        ...
        return KiboDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> KiboBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: KiboBatteryState = switch device.batteryState {
        ...
        return KiboBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> KiboThermalStatusPayload {
        let state: KiboThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return KiboThermalStatusPayload(state: state)
    }

    private func storageStatus() -> KiboStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return KiboStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
