---
summary: "Use MiniMax models in Kibo"
read_when:
  - You want MiniMax models in Kibo
...
Kibo's MiniMax provider defaults to **MiniMax M2.7**.
...
entries, Kibo materializes `MiniMax-M2.7` and
...
```bash
kibo onboard --auth-choice minimax-global-oauth
# or
kibo onboard --auth-choice minimax-cn-oauth
```
...
See the MiniMax plugin package README in the Kibo repo for details.
...
```bash
kibo onboard --auth-choice minimax-global-api
# or
...
On the Anthropic-compatible streaming path, Kibo now disables MiniMax
...
- On `api: "anthropic-messages"`, Kibo injects
...
- Kibo normalizes MiniMax coding-plan usage to the same `% left` display
  used by other providers. MiniMax's raw `usage_percent` / `usagePercent`
  fields are remaining quota, not consumed quota, so Kibo inverts them.
  Count-based fields win when present. When the API returns `model_remains`,
  Kibo prefers the chat-model entry, derives the window label from
...
- Use `kibo models list` to confirm the current provider id, then switch with
  `kibo models set minimax/MiniMax-M2.7` or
  `kibo models set minimax-portal/MiniMax-M2.7`.
...
- Running `kibo configure` and selecting a **MiniMax** auth option, or
