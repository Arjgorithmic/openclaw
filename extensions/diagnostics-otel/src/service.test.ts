...
import type { KiboPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/kibo-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): KiboPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("kibo.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("kibo.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("kibo.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("kibo.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("kibo.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("kibo.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("kibo.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("kibo.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("kibo.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("kibo.webhook.processed");
    expect(spanNames).toContain("kibo.message.processed");
    expect(spanNames).toContain("kibo.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["kibo.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("kibo.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "kibo.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["kibo.reason"]).toBe("string");
    expect(String(attrs?.["kibo.reason"])).not.toContain(
