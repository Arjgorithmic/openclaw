...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    KIBO_STATE_DIR: "/tmp/kibo-daemon",
    KIBO_CONFIG_PATH: "/tmp/kibo-daemon/kibo.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.KIBO_STATE_DIR ?? "/tmp/kibo-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.KIBO_CONFIG_PATH ?? `${stateDir}/kibo.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/kibo-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.KIBO_STATE_DIR = "/tmp/kibo-cli";
    process.env.KIBO_CONFIG_PATH = "/tmp/kibo-cli/kibo.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/kibo-daemon/kibo.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/kibo-cli/kibo.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.KIBO_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.KIBO_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.KIBO_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "kibo-gateway" }],
        hints: [],
      },
      healthy: false,
