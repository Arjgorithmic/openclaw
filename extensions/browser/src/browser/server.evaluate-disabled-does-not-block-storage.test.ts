  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "kibo",
        profiles: {
          kibo: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.KIBO_GATEWAY_PORT;
    process.env.KIBO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.KIBO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.KIBO_GATEWAY_PASSWORD;
