...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-bundled-skills-test-"));
    process.env.KIBO_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
