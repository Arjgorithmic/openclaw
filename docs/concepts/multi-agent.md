...
- **Session store** (chat history + routing state) under `~/.kibo/agents/<agentId>/sessions`.
...
`~/.kibo/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.kibo` (or `KIBO_STATE_DIR`)
- Workspace: `~/.kibo/workspace` (or `~/.kibo/workspace-<agentId>`)
- Agent dir: `~/.kibo/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.kibo/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Kibo runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.kibo/workspace` (or `~/.kibo/workspace-<profile>` when `KIBO_PROFILE` is set).
- State defaults to `~/.kibo/agents/main/agent`.
...
```bash
kibo agents add coding
kibo agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.kibo/agents/<agentId>`.
...
```bash
kibo channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.kibo/workspace-alex" },
      { id: "mia", workspace: "~/.kibo/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, Kibo upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, Kibo falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.kibo/workspace-main" },
      { id: "coding", workspace: "~/.kibo/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.kibo/workspace-main" },
      { id: "alerts", workspace: "~/.kibo/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
kibo channels login --channel whatsapp --account personal
kibo channels login --channel whatsapp --account biz
```

`~/.kibo/kibo.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.kibo/workspace-home",
        agentDir: "~/.kibo/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.kibo/workspace-work",
        agentDir: "~/.kibo/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.kibo/credentials/whatsapp/personal
          // authDir: "~/.kibo/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.kibo/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.kibo/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.kibo/workspace-opus",
