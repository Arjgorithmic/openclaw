...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[kibo] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.KIBO_WATCH_MODE = "1";
  childEnv.KIBO_WATCH_SESSION = watchSession;
  ...
  childEnv.KIBO_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.KIBO_WATCH_COMMAND = deps.args.join(" ");
  }
