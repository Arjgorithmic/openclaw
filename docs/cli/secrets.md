---
summary: "CLI reference for `kibo secrets` (reload, audit, configure, apply)"
read_when:
...
Use `kibo secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
kibo secrets reload
kibo secrets reload --json
kibo secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `kibo.json` refs)
...
kibo secrets audit
kibo secrets audit --check
kibo secrets audit --json
kibo secrets audit --allow-exec
```
...
```bash
kibo secrets configure
kibo secrets configure --plan-out /tmp/kibo-secrets-plan.json
kibo secrets configure --apply --yes
kibo secrets configure --providers-only
kibo secrets configure --skip-provider-setup
kibo secrets configure --agent ops
kibo secrets configure --json
```
...
- `configure` targets secret-bearing fields in `kibo.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Kibo fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
kibo secrets apply --from /tmp/kibo-secrets-plan.json --json
```
...
- `kibo.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.kibo/.env` known secret keys whose values were migrated
