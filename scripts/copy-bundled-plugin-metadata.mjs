...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@kibo/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "kibo.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "kibo.plugin.json");
    ...
    if (packageJson.kibo && "extensions" in packageJson.kibo) {
      packageJson.kibo = {
        ...packageJson.kibo,
        extensions: rewritePackageExtensions(packageJson.kibo.extensions),
        ...(typeof packageJson.kibo.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.kibo.setupEntry) }
          : {}),
      };
    }
