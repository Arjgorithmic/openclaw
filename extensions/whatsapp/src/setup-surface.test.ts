...
import { DEFAULT_ACCOUNT_ID, type KiboConfig } from "kibo/plugin-sdk/setup";
...
const hoisted = vi.hoisted(() => ({
  detectWhatsAppLinked: vi.fn<(cfg: KiboConfig, accountId: string) => Promise<boolean>>(
    async () => false,
  ),
  loginWeb: vi.fn(async () => {}),
  pathExists: vi.fn(async () => false),
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/kibo-whatsapp-test",
  })),
}));
...
vi.mock("kibo/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("kibo/plugin-sdk/setup")>(
    "kibo/plugin-sdk/setup",
  );
  return {
    ...actual,
...
function expectFinalizeResult(result: Awaited<ReturnType<typeof runFinalizeWithHarness>>): {
  ...
  return result as { cfg: KiboConfig };
}
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/kibo-whatsapp-test" });
  });
  ...
  it("writes named-account DM policy and allowFrom instead of the channel root", async () => {
    ...
    const named = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "work",
        cfg: createWhatsAppWorkAccountConfig() as KiboConfig,
      }),
    );
  ...
  it("uses configured defaultAccount for omitted-account setup status", async () => {
    hoisted.detectWhatsAppLinked.mockImplementation(
      async (_cfg: KiboConfig, accountId: string) => accountId === "work",
    );
  ...
  it("uses configured defaultAccount for omitted-account finalize writes", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "",
        cfg: createWhatsAppWorkAccountConfig({ defaultAccount: "work" }) as KiboConfig,
      }),
    );
  ...
  it("forces wildcard allowFrom for open policy without allowFrom follow-up prompts", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        cfg: createWhatsAppRootAllowFromConfig() as KiboConfig,
      }),
    );
