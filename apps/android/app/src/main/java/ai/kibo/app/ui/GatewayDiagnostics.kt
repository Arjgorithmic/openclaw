...
internal fun kiboAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Kibo Android gateway connection failure.
    ...
    - tell me whether `kibo devices list` should show a pending pairing request
    - if more signal is needed, ask for `kibo qr --json`, `kibo devices list`, and `kibo nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${kiboAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Kibo gateway diagnostics", report))
