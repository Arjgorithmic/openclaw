...
struct KiboMacCLI {
    static func main() async {
        ...
        default:
            fputs("kibo-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      kibo-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      kibo-mac discover [--timeout <ms>] [--json] [--include-local]
      kibo-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      kibo-mac connect
      kibo-mac connect --url ws://127.0.0.1:18789 --json
      kibo-mac discover --timeout 3000 --json
      kibo-mac wizard --mode local
    """)
}
