...
read_when:
  - Setting up Kibo for the first time
...
Kibo reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.kibo/kibo.json`.

If the file is missing, Kibo uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `kibo onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    kibo onboard       # full onboarding flow
    kibo configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    kibo config get agents.defaults.workspace
    kibo config set agents.defaults.heartbeat.every "2h"
    kibo config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.kibo/kibo.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Kibo only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

Schema tooling notes:

- `kibo config schema` prints the same JSON Schema family used by Control UI
  and config validation.
- Treat that schema output as the canonical machine-readable contract for
  `kibo.json`; this overview and the configuration reference summarize it.
...
- Only diagnostic commands work (`kibo doctor`, `kibo logs`, `kibo health`, `kibo status`)
- Run `kibo doctor` to see exact issues
- Run `kibo doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `kibo.json`.
    ...
    ```bash
    kibo config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `KIBO_APNS_RELAY_BASE_URL` and `KIBO_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `KIBO_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-kibo-token`); query-string tokens are rejected.
...
The Gateway watches `~/.kibo/kibo.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `kibo config set` for single keys.
    </Warning>
    ...
    ```bash
    kibo gateway call config.get --params '{}'  # capture payload.hash
    kibo gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.kibo/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    kibo gateway call config.patch --params '{
...
Kibo reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.kibo/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Kibo runs your login shell and imports only the missing keys:
...
Env var equivalent: `KIBO_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${KIBO_GATEWAY_TOKEN}" } },
