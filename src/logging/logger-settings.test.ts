...
let originalKiboLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.KIBO_TEST_FILE_LOG;
  originalKiboLogLevel = process.env.KIBO_LOG_LEVEL;
  delete process.env.KIBO_TEST_FILE_LOG;
  delete process.env.KIBO_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.KIBO_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalKiboLogLevel === undefined) {
    delete process.env.KIBO_LOG_LEVEL;
  } else {
    process.env.KIBO_LOG_LEVEL = originalKiboLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.KIBO_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();
