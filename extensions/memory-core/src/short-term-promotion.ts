...
import { formatMemoryDreamingDay } from "kibo/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "kibo/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "kibo-memory-promotion:";
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*kibo-memory-promotion:([^\n]+?)\s*-->/gi);
