...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: KiboConfig | null = null;
  ...
  get(key: string, cfg: KiboConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: KiboConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: KiboConfig): void {
