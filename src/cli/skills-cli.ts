...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.kibo.ai/cli/skills")}\n`,
    );

  skills
    .command("search")
    .description("Search KiboHub skills")
    ...
    .action(async (queryParts: string[], opts: { limit?: number; json?: boolean }) => {
      try {
        ...
        if (results.length === 0) {
          defaultRuntime.log("No KiboHub skills found.");
          return;
        }
  ...
  skills
    .command("install")
    .description("Install a skill from KiboHub into the active workspace")
    .argument("<slug>", "KiboHub skill slug")
  ...
  skills
    .command("update")
    .description("Update KiboHub-installed skills in the active workspace")
    .argument("[slug]", "Single skill slug")
    .option("--all", "Update all tracked KiboHub skills", false)
    .action(async (slug: string | undefined, opts: { all?: boolean }) => {
      try {
        ...
        const workspaceDir = resolveActiveWorkspaceDir();
        const tracked = await readTrackedKiboHubSkillSlugs(workspaceDir);
        if (opts.all && tracked.length === 0) {
          defaultRuntime.log("No tracked KiboHub skills to update.");
          return;
        }
