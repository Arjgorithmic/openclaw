...
export function resolveConfigDir(
  ...
  const configPath = env.KIBO_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".kibo");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.KIBO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$KIBO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via KIBO_STATE_DIR.
