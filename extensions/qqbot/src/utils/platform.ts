 ...
 * 3. the Kibo temp directory as a last resort
 */
...
/**
 * Return a path under `~/.kibo/qqbot`, creating it on demand.
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".kibo", "qqbot", ...subPaths);
...
/**
 * Return a path under `~/.kibo/media/qqbot`, creating it on demand.
 *
 * Unlike `getQQBotDataDir`, this lives under Kibo's core media allowlist so
...
export function getQQBotMediaDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".kibo", "media", "qqbot", ...subPaths);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  return dir;
...
/** Return the preferred Kibo temp directory. */
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".kibo", "workspace", "qqbot");
