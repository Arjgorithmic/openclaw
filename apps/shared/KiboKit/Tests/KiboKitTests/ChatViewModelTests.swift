...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> KiboChatHistoryPayload
{
    KiboChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> KiboChatSessionEntry {
    KiboChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> KiboChatSessionEntry
{
    KiboChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> KiboChatModelChoice {
    KiboChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [KiboChatHistoryPayload],
    sessionsResponses: [KiboChatSessionsListResponse] = [],
    modelResponses: [[KiboChatModelChoice]] = [],
    ...
    -> (TestChatTransport, KiboChatViewModel)
{
...
private func sendUserMessage(_ vm: KiboChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, KiboChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [KiboChatHistoryPayload]
    private let sessionsResponses: [KiboChatSessionsListResponse]
    private let modelResponses: [[KiboChatModelChoice]]
    ...
    private let stream: AsyncStream<KiboChatTransportEvent>
    private let continuation: AsyncStream<KiboChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<KiboChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<KiboChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> KiboChatHistoryPayload {
        ...
        return self.historyResponses.last ?? KiboChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [KiboChatAttachmentPayload]) async throws -> KiboChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return KiboChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> KiboChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? KiboChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [KiboChatModelChoice] {
    ...
    func emit(_ evt: KiboChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = KiboChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = KiboChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: KiboChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = KiboChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: KiboChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(KiboChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == KiboChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = KiboChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: KiboChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = KiboChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = KiboChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = KiboChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { KiboChatViewModel(sessionKey: "main", transport: transport) }
