...
This guide summarizes a sane workflow for working on the pi integration in Kibo.
...
```bash
KIBO_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm kibo agent --message "Hello" --thinking low`
...
State lives under the Kibo state directory. Default is `~/.kibo`. If `KIBO_STATE_DIR` is set, use that directory instead.

To reset everything:

- `kibo.json` for config
