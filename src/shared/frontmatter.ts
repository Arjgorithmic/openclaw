...
export function resolveKiboManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type KiboManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveKiboManifestRequires(
  metadataObj: Record<string, unknown>,
): KiboManifestRequires | undefined {
  const requiresRaw =
...
export function resolveKiboManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveKiboManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedKiboManifestInstallBase = {
...
export function parseKiboManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedKiboManifestInstallBase | undefined {
  ...
  const spec: ParsedKiboManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyKiboManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedKiboManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
