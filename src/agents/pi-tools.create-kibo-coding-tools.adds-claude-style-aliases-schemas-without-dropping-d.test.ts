...
const defaultTools = createKiboCodingTools();
...
describe("createKiboCodingTools", () => {
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-read-"));
    try {
      ...
      const contents = "Hello from kibo read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "kibo-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "kibo-workspace"),
    ...
    const tools = createKiboCodingTools({ sandbox });
