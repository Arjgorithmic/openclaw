---
summary: "Use Venice AI privacy-focused models in Kibo"
read_when:
  - You want privacy-focused inference in Kibo
...
```bash
kibo onboard --auth-choice venice-api-key
```
...
```bash
kibo agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Kibo shows all available Venice models. Pick based on your needs:
...
```bash
kibo models set venice/kimi-k2-5
kibo models set venice/claude-opus-4-6
```

List all available models:

```bash
kibo models list | grep venice
```

## Configure via `kibo configure`

1. Run `kibo configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Kibo automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
kibo agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
kibo agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
kibo agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
kibo agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
kibo agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `kibo models list` to see currently available models. Some models may be temporarily offline.
