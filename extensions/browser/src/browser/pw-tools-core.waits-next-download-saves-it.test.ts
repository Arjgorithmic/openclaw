...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredKiboTmpDir: vi.fn(() => "/tmp/kibo"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-kibo-dir.js");

describe("pw-tools-core", () => {
  beforeAll(async () => {
    vi.doMock("./pw-session.js", () => sessionMocks);
    vi.doMock("./chrome.js", () => chromeMocks);
    tmpDirModule = await import("../infra/tmp-kibo-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredKiboTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredKiboTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredKiboTmpDir.mockReturnValue("/tmp/kibo");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".kibo-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredKiboTmpDir.mockReturnValue("/tmp/kibo-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "kibo-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "kibo-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredKiboTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "kibo-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "kibo-preferred", "downloads")}${path.sep}`),
    );
  });
