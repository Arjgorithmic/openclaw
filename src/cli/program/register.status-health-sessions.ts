...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kibo status", "Show channel health + session summary."],
          ["kibo status --all", "Full diagnosis (read-only)."],
          ["kibo status --json", "Machine-readable output."],
          ["kibo status --usage", "Show model provider usage/quota snapshots."],
          [
            "kibo status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["kibo status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.kibo.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.kibo.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kibo sessions", "List all sessions."],
          ["kibo sessions --agent work", "List sessions for one agent."],
          ["kibo sessions --all-agents", "Aggregate sessions across agents."],
          ["kibo sessions --active 120", "Only last 2 hours."],
          ["kibo sessions --json", "Machine-readable output."],
          ["kibo sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.kibo.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kibo sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "kibo sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["kibo sessions cleanup --enforce", "Apply maintenance now."],
          ["kibo sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["kibo sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "kibo sessions cleanup --enforce --store ./tmp/sessions.json",
