---
summary: "Use OpenCode Zen and Go catalogs with Kibo"
read_when:
...
OpenCode exposes two hosted catalogs in Kibo:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Kibo keeps the runtime provider ids
...
kibo onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
kibo onboard --auth-choice opencode-go
kibo onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
...
- Gemini-backed OpenCode refs stay on the proxy-Gemini path, so Kibo keeps
