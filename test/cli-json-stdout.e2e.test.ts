...
describe("cli json stdout contract", () => {
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        const legacyDir = path.join(tempHome, ".kibobot");
        await fs.mkdir(legacyDir, { recursive: true });
        await fs.writeFile(path.join(legacyDir, "kibobot.json"), "{}", "utf8");
        ...
        delete env.KIBO_HOME;
        delete env.KIBO_STATE_DIR;
        delete env.KIBO_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "kibo.mjs");
