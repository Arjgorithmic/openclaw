...
read_when:
  - Building or debugging native Kibo plugins
...
This page covers the internal architecture of the Kibo plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Kibo. Every
native Kibo plugin registers against one or more capability types:
...
Kibo classifies every loaded plugin into a shape based on its actual
...
Use `kibo plugins inspect <id>` to see a plugin's shape and capability
...
When you run `kibo doctor` or `kibo plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `kibo status --all` and `kibo plugins doctor`.

## Architecture overview

Kibo's plugin system has four layers:

1. **Manifest + discovery**
   Kibo finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `kibo.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Kibo plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Kibo reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Kibo
...
That split lets Kibo validate config, explain missing/disabled plugins, and
...
normal chat actions. Kibo keeps one shared `message` tool in core, and
...
Kibo treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Kibo-facing
  surfaces
...
So if Kibo adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Kibo has shared
...
```ts
import type { KiboPluginDefinition } from "kibo/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "kibo/plugin-sdk/media-understanding";

const plugin: KiboPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Kibo already treats image/audio/video understanding as one shared
...
`KiboPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Kibo can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Kibo knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `KiboPluginApi` or
  `api.runtime`
...
Native Kibo plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Kibo currently treats them
...
name: `@kibo/<id>` by default, or an approved typed suffix such as
...
Kibo exports capabilities, not implementation convenience.
...
At startup, Kibo does roughly this:
...
The manifest is the control-plane source of truth. Kibo uses it to:
...
Kibo keeps short in-process caches for:
...
- Set `KIBO_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `KIBO_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `KIBO_PLUGIN_DISCOVERY_CACHE_MS` and
  `KIBO_PLUGIN_MANIFEST_CACHE_MS`.
...
Kibo still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Kibo calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | Kibo tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on Kibo's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Kibo's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Kibo adds those
...
- Kibo only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Kibo plugin runtime helpers",
    count: 5,
  },
});
```
...
const result = await api.runtime.imageGeneration.generate({
  config: api.config,
  args: { prompt: "A friendly shell mascot", size: "1024x1024" },
});
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-kibo-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-kibo-scopes` only when the header is explicitly present
  - if `x-kibo-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-kibo-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `kibo/plugin-sdk` import when
authoring plugins:

- `kibo/plugin-sdk/plugin-entry` for plugin registration primitives.
- `kibo/plugin-sdk/core` for the generic shared plugin-facing contract.
- `kibo/plugin-sdk/config-schema` for the root `kibo.json` Zod schema
  export (`KiboSchema`).
- Stable channel primitives such as `kibo/plugin-sdk/channel-setup`,
  `kibo/plugin-sdk/setup-runtime`,
  `kibo/plugin-sdk/setup-adapter-runtime`,
  `kibo/plugin-sdk/setup-tools`,
  `kibo/plugin-sdk/channel-pairing`,
  `kibo/plugin-sdk/channel-contract`,
  `kibo/plugin-sdk/channel-feedback`,
  `kibo/plugin-sdk/channel-inbound`,
  `kibo/plugin-sdk/channel-lifecycle`,
  `kibo/plugin-sdk/channel-reply-pipeline`,
  `kibo/plugin-sdk/command-auth`,
  `kibo/plugin-sdk/secret-input`, and
  `kibo/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `kibo/plugin-sdk/channel-config-helpers`,
  `kibo/plugin-sdk/allow-from`,
  `kibo/plugin-sdk/channel-config-schema`,
  `kibo/plugin-sdk/telegram-command-config`,
  `kibo/plugin-sdk/channel-policy`,
  `kibo/plugin-sdk/approval-gateway-runtime`,
  `kibo/plugin-sdk/approval-handler-adapter-runtime`,
  `kibo/plugin-sdk/approval-handler-runtime`,
  `kibo/plugin-sdk/approval-runtime`,
  `kibo/plugin-sdk/config-runtime`,
  `kibo/plugin-sdk/infra-runtime`,
  `kibo/plugin-sdk/agent-runtime`,
  `kibo/plugin-sdk/lazy-runtime`,
  `kibo/plugin-sdk/reply-history`,
  `kibo/plugin-sdk/routing`,
  `kibo/plugin-sdk/status-helpers`,
  `kibo/plugin-sdk/text-runtime`,
  `kibo/plugin-sdk/runtime-store`, and
  `kibo/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `kibo/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `kibo/plugin-sdk/*` subpaths. Kibo core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  Kibo is not yet serving a runtime snapshot.
...
- Avoid the root `kibo/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `kibo/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `kibo/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `kibo/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `kibo/plugin-sdk/channel-targets`; channel-specific
...
`kibo/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Kibo writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Kibo's
...
- if both `catalog` and `discovery` are registered, Kibo uses `catalog`
...
- Read-only command paths such as `kibo status`, `kibo status --all`,
  `kibo channels status`, `kibo channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `kibo.extensions`:
...
Security guardrail: every `kibo.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `kibo plugins install` installs plugin dependencies with
...
Optional: `kibo.setupEntry` can point at a lightweight setup-only module.
When Kibo needs setup surfaces for a disabled channel plugin, or
...
Optional: `kibo.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Kibo load the
...
Channel plugins can advertise setup/discovery metadata via `kibo.channel` and
install hints via `kibo.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@kibo/nextcloud-talk",
  "kibo": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Useful `kibo.channel` fields beyond the minimal example:
...
Kibo can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.kibo/mpm/plugins.json`
- `~/.kibo/mpm/catalog.json`
- `~/.kibo/plugins/catalog.json`

Or point `KIBO_PLUGIN_CATALOG_PATHS` (or `KIBO_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "kibo": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { buildMemorySystemPromptAddition } from "kibo/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `KiboPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Kibo stays opinionated without becoming hardcoded to one
