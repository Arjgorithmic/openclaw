 ...
 */
export type KiboPluginConfigSchema = {
...
export type KiboPluginToolContext = {
  config?: KiboConfig;
...
export type KiboPluginToolFactory = (
  ctx: KiboPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type KiboPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type KiboPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<KiboConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Kibo uses this to expose
 ...
 *
 * Runs after Kibo resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Kibo auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Kibo wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Kibo builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Kibo no
...
/**
 * Provider-owned stream wrapper hook after Kibo applies its generic
 ...
 *
 * Return `true` or `false` to override Kibo's built-in provider cache TTL
 ...
 *
 * Runs only after Kibo exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Kibo should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Kibo loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Kibo falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Kibo calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Kibo performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Kibo registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Kibo wrappers.
   *
   ...
   *
   * Called after Kibo resolves the raw configured credential but before the
   ...
   * hint than Kibo's generic auth-store guidance.
   */
   ...
   * Kibo's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Kibo should surface a provider-specific hint for
   ...
   * merging. Kibo deduplicates by `provider/id`, so plugins only need to
   ...
   * without replacing the full Kibo-owned system prompt.
   */
   ...
   *
   * Kibo uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `kibo doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `kibo doctor` should offer.
   *
   ...
   *
   * Kibo calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Kibo's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Kibo falls back to that stored profile.
   */
...
export type WebSearchProviderPlugin = {
   ...
   * when Kibo has no config-aware runtime context yet.
   *
  ...
  setConfiguredCredentialValue?: (configTarget: KiboConfig, value: unknown) => void;
  applySelectionConfig?: (config: KiboConfig) => KiboConfig;
  runSetup?: (ctx: WebSearchProviderSetupContext) => KiboConfig | Promise<KiboConfig>;
...
export type KiboPluginGatewayMethod = {
  method: string;
 ...
 */
export type KiboPluginCommandDefinition = {
...
export type KiboPluginHttpRouteAuth = "gateway" | "plugin";
export type KiboPluginHttpRouteMatch = "exact" | "prefix";

export type KiboPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type KiboPluginHttpRouteParams = {
  path: string;
...
export type KiboPluginCliContext = {
  program: Command;
  config: KiboConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type KiboPluginCliRegistrar = (ctx: KiboPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Kibo to keep a plugin command lazy-loaded while still
 ...
 */
export type KiboPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type KiboPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type KiboPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type KiboPluginSecurityAuditContext = {
  config: KiboConfig;
...
export type KiboPluginSecurityAuditCollector = (
  ctx: KiboPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

/** Context passed to long-lived plugin services. */
export type KiboPluginServiceContext = {
  config: KiboConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type KiboPluginService = {
  id: string;
  start: (ctx: KiboPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: KiboPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Kibo should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Kibo's bundle
...
export type KiboPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type KiboPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: KiboPluginReloadRegistration;
  nodeHostCommands?: KiboPluginNodeHostCommand[];
  securityAuditCollectors?: KiboPluginSecurityAuditCollector[];
  register?: (api: KiboPluginApi) => void | Promise<void>;
  activate?: (api: KiboPluginApi) => void | Promise<void>;
};

export type KiboPluginModule =
  | KiboPluginDefinition
  | ((api: KiboPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

export type PluginConfigMigration = (config: KiboConfig) =>
  | {
      config: KiboConfig;
      changes: string[];
    }
...
export type KiboPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | KiboPluginToolFactory,
    opts?: KiboPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: KiboPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: KiboPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: KiboPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: KiboPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Kibo can keep
  ...
  registerReload: (registration: KiboPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: KiboPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: KiboPluginSecurityAuditCollector) => void;
  registerService: (service: KiboPluginService) => void;
   ...
   */
  registerCommand: (command: KiboPluginCommandDefinition) => void;
...
export type PluginFormat = "kibo" | "bundle";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * Kibo does not await this callback before allowing or denying the tool call.
     */
...
export type PluginHookBeforeInstallPlugin = {
  /** Canonical plugin id Kibo will install under. */
  pluginId: string;
...
export type PluginHookBeforeInstallContext = {
  ...
  /** Normalized origin of the install target (e.g. "kibo-bundled", "plugin-package"). */
  origin?: string;
};
