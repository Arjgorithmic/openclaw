---
summary: "Kibo on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Kibo on a Raspberry Pi
  - Running Kibo on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Kibo on Raspberry Pi

## Goal

Run a persistent, always-on Kibo Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u kibo-gateway.service -f
```

## 9) Access the Kibo Dashboard
...
```bash
ssh user@gateway-host 'kibo dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/kibo-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/kibo-compile-cache
mkdir -p /var/tmp/kibo-compile-cache
export KIBO_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `KIBO_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Kibo, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit kibo-gateway.service
```

```ini
[Service]
Environment=KIBO_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/kibo-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Kibo state/cache on SSD-backed storage to avoid SD-card
...
Most Kibo features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled kibo-gateway.service

# Enable if not
systemctl --user enable kibo-gateway.service
...
```bash
# Check logs
journalctl --user -u kibo-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/kibo  # if using hackable install
npm run build
systemctl --user restart kibo-gateway.service
```
