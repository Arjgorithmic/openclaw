...
enum CommandResolver {
    private static let projectRootDefaultsKey = "kibo.gatewayProjectRootPath"
    private static let helperName = "kibo"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let kiboEntry = root.appendingPathComponent("kibo.mjs").path
        if FileManager().isReadableFile(atPath: kiboEntry) { return kiboEntry }
        let binEntry = root.appendingPathComponent("bin/kibo.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/kibo")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let kiboPaths = self.kiboManagedPaths(home: home)
        if !kiboPaths.isEmpty {
            extras.insert(contentsOf: kiboPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + kiboPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func kiboManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".kibo"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func kiboExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectKiboExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("kibo.mjs").path,
            root.appendingPathComponent("bin/kibo.js").path,
        ]
    ...
    static func hasAnyKiboInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.kiboExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let kiboPath = self.projectKiboExecutable(projectRoot: root) {
            return [kiboPath, subcommand] + extraArgs
        }
        if let kiboPath = self.kiboExecutable(searchPaths: searchPaths) {
            return [kiboPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "kibo", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            kibo entrypoint missing (looked for dist/index.js or kibo.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real kibo CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/kibo"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/kibo.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/kibo.mjs"
            node "$PRJ/kibo.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/kibo.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/kibo.js"
            node "$PRJ/bin/kibo.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent kibo"
          pnpm --silent kibo \(quotedArgs);
        else
          echo "kibo CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? KiboConfigFile.loadDict()
