...
// Keep integration tests deterministic: never read a real kibo.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "kibo-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.KIBO_SESSION_CACHE_TTL_MS;
    process.env.KIBO_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.KIBO_SESSION_CACHE_TTL_MS;
    } else {
      process.env.KIBO_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
