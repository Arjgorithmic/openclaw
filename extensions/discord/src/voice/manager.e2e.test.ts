...
vi.mock("kibo/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("kibo/plugin-sdk/routing")>(
...
vi.mock("kibo/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("kibo/plugin-sdk/agent-runtime")>(
