...
final class MotionService: MotionServicing {
    func activities(params: KiboMotionActivityParams) async throws -> KiboMotionActivityPayload {
        ...
        let mapped: [KiboMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return KiboMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: KiboPedometerParams) async throws -> KiboPedometerPayload {
        ...
        let payload: KiboPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = KiboPedometerPayload(
