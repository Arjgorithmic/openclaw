...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): KiboConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as KiboConfig;
  ...
  async function resolveReadToken(cfg: KiboConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: KiboConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
