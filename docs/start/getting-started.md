---
summary: "Get Kibo installed and run your first chat in minutes."
read_when:
...
Install Kibo, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.kibo/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.kibo/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Kibo as a service account or want custom paths:

- `KIBO_HOME` — home directory for internal path resolution
- `KIBO_STATE_DIR` — override the state directory
- `KIBO_CONFIG_PATH` — override the config file path
