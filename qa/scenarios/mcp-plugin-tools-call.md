...
surface: mcp
objective: Verify Kibo can expose plugin tools over MCP and a real MCP client can call one successfully.
successCriteria:
...
execution:
  kind: flow
  summary: Verify Kibo can expose plugin tools over MCP and a real MCP client can call one successfully.
  config:
