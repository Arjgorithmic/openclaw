...
IMAGE_NAME="${KIBO_IMAGE:-kibo-mcp-channels-e2e}"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="kibo-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t kibo-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "KIBO_SKIP_GMAIL_WATCHER=1" \
  -e "KIBO_SKIP_CRON=1" \
  -e "KIBO_SKIP_CANVAS_HOST=1" \
  -e "KIBO_STATE_DIR=/tmp/kibo-state" \
  -e "KIBO_CONFIG_PATH=/tmp/kibo-state/kibo.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "KIBO_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
