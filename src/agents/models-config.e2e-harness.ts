...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousKiboAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousKiboAgentDir = process.env.KIBO_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousKiboAgentDir === undefined) {
      delete process.env.KIBO_AGENT_DIR;
    } else {
      process.env.KIBO_AGENT_DIR = previousKiboAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "KIBO_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.KIBO_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.KIBO_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.KIBO_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: KiboConfig = {
  models: {
    providers: {
      "custom-proxy": {
