 ...
 * `kibo update`. Use it to verify the completion installation flow
...
\`kibo update\`, \`kibo doctor\`, and \`kibo onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Kibo Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`kibo completion --install\` later to enable.`));
    return;
  }
