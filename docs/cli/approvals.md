---
summary: "CLI reference for `kibo approvals` and `kibo exec-policy`"
read_when:
...
`kibo exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.kibo/exec-approvals.json`

Examples:

```bash
kibo exec-policy show
kibo exec-policy show --json

kibo exec-policy preset yolo
kibo exec-policy preset cautious --json

kibo exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `kibo exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `kibo approvals set --gateway`
or `kibo approvals set --node <id|name|ip>`.

## Common commands

```bash
kibo approvals get
kibo approvals get --node <id|name|ip>
kibo approvals get --gateway
```

`kibo approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
kibo approvals set --file ./exec-approvals.json
kibo approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
kibo approvals set --node <id|name|ip> --file ./exec-approvals.json
kibo approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Kibo policy aligned, also set:
...
- `--node` uses the same resolver as `kibo nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.kibo/exec-approvals.json`.
