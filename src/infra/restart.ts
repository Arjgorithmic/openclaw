...
export function triggerKiboRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.KIBO_SYSTEMD_UNIT,
      process.env.KIBO_PROFILE,
    );
  ...
  const label =
    process.env.KIBO_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.KIBO_PROFILE);
