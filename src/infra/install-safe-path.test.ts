...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@kibo/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@kibo/matrix", id: "matrix", expected: true },
    { packageName: "@kibo/matrix", id: "@kibo/matrix", expected: true },
    { packageName: "@kibo/matrix", id: "signal", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@kibo/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@kibo__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "kibo-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "kibo-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "kibo-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    await withTempDir({ prefix: "kibo-install-safe-" }, async (parentDir) => {
