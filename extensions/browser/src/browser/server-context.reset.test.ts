...
function localKiboProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "kibo",
...
function createLocalKiboResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localKiboProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveKiboUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-reset-"));
    const profileDir = path.join(tempRoot, "kibo");
    ...
    const ops = createLocalKiboResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveKiboUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-reset-no-own-"));
    const profileDir = path.join(tempRoot, "kibo");
    fs.mkdirSync(profileDir, { recursive: true });
