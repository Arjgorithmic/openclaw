...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["KIBO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/kibo")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("kibo-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("kibo-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/kibo/ (rolling or stdout), or nil if none exist.
