...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Kibo to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "kibo.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.KIBO_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.KIBO_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set KIBO_APNS_TEAM_ID and KIBO_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.KIBO_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.KIBO_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.KIBO_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set KIBO_APNS_PRIVATE_KEY_P8 or KIBO_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading KIBO_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
