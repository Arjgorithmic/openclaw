...
const GOOGLE_LIVE_SINGLE_KEY = "KIBO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "KIBO_LIVE_ANTHROPIC_KEY",
    listVar: "KIBO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "KIBO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `KIBO_LIVE_${base}_KEY`;
