...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const kiboTmpDir = path.resolve(resolvePreferredKiboTmpDir());
  if (!isPathInside(kiboTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: kiboTmpDir });
  return resolved;
}
