...
Tlon is a decentralized messenger built on Urbit. Kibo connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Kibo releases, so normal packaged
...
```bash
kibo plugins install @kibo/tlon
```

Local checkout (when running from a git repo):

```bash
kibo plugins install ./path/to/local/tlon-plugin
```
...
By default, Kibo blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `kibo message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Kibo replies in-thread.
