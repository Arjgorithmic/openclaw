...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "KIBO_ENV_TEST_A";
    const keyB = "KIBO_ENV_TEST_B";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "KIBO_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "KIBO_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "KIBO_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "KIBO_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "kibo-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousKiboHome = process.env.KIBO_HOME;
    const previousStateDir = process.env.KIBO_STATE_DIR;
    const previousBundledDir = process.env.KIBO_BUNDLED_PLUGINS_DIR;
    process.env.KIBO_HOME = "/srv/kibo-home";
    process.env.KIBO_STATE_DIR = "/srv/kibo-state";
    process.env.KIBO_BUNDLED_PLUGINS_DIR = "/srv/kibo-bundled";
    ...
    try {
      ...
      expect(env.KIBO_HOME).toBeUndefined();
      expect(env.KIBO_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.KIBO_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("KIBO_HOME", previousKiboHome);
      restoreEnvKey("KIBO_STATE_DIR", previousStateDir);
      restoreEnvKey("KIBO_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { KIBO_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processKiboHome: process.env.KIBO_HOME,
          processBundledDir: process.env.KIBO_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.KIBO_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processKiboHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.KIBO_HOME).toBe("/srv/kibo-home");
    } finally {
      restoreEnvKey("KIBO_HOME", previousKiboHome);
    }
  });
