...
function resolveAccount(cfg: KiboConfig, accountId = "default"): ResolvedDiscordAccount {
  return discordPlugin.config.resolveAccount(cfg, accountId);
}

function startDiscordAccount(cfg: KiboConfig, accountId = "default") {
...
describe("discordPlugin outbound", () => {
  ...
  it("splits text and video into separate sends for attached outbound delivery", async () => {
    ...
    const result = await discordPlugin.outbound!.sendMedia!({
      cfg: {} as KiboConfig,
      to: "channel:123",
      text: "done - tiny cyber-shell clip incoming",
    ...
    expect(sendMessageDiscord).toHaveBeenNthCalledWith(
      1,
      "channel:thread-123",
      "done - tiny cyber-shell clip incoming",
      expect.objectContaining({
        replyTo: "reply-123",
      }),
    );
