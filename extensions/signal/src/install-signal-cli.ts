...
import { runPluginCommandWithTimeout } from "kibo/plugin-sdk/run-command";
import type { RuntimeEnv } from "kibo/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "kibo/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredKiboTmpDir(), "kibo-signal-"));
