...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/kibo-state", "devices")).toEqual({
      dir: path.join("/tmp/kibo-state", "devices"),
      pendingPath: path.join("/tmp/kibo-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/kibo-state", "devices", "paired.json"),
    });
  });
