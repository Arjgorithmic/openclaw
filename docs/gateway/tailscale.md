...
Kibo can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Kibo requires a shared password.
...
- `token` (default when `KIBO_GATEWAY_TOKEN` is set)
- `password` (shared secret via `KIBO_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Kibo verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Kibo only treats a request as Serve when it arrives from loopback with
...
Prefer `KIBO_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
kibo gateway --tailscale serve
kibo gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Kibo to undo `tailscale serve`
