...
private let chatUILogger = Logger(subsystem: "ai.kibo", category: "KiboChatUI")

@MainActor
@Observable
public final class KiboChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [KiboChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [KiboChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [KiboPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [KiboChatPendingToolCall] = []
    public private(set) var sessions: [KiboChatSessionEntry] = []
    private let transport: any KiboChatTransport
    private var sessionDefaults: KiboChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: KiboChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any KiboChatTransport,
    ...
    public var sessionChoices: [KiboChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [KiboChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: KiboPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [KiboChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: KiboChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: KiboChatMessage) -> KiboChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> KiboChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return KiboChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: KiboChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: KiboChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: KiboChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [KiboChatMessage],
        incoming: [KiboChatMessage]) -> [KiboChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [KiboChatMessage]) -> [KiboChatMessage] {
        var result: [KiboChatMessage] = []
    ...
    private static func dedupeKey(for message: KiboChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [KiboChatMessageContent] = [
            KiboChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> KiboChatAttachmentPayload in
            KiboChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> KiboChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = KiboChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: KiboChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: KiboChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: KiboAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = KiboChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> KiboPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
