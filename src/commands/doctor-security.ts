...
import type { KiboConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: KiboConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: KiboConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Kibo default (full)";
  const defaultRequestedAskSource = "Kibo default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("kibo approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: KiboConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: KiboConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("kibo security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.kibo/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("kibo approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://github.com/Arjgorithmic/openclaw/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("kibo configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("kibo config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("kibo doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("kibo config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('kibo config set session.dmScope "per-channel-peer"') +
