---
summary: "CLI reference for `kibo dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: kibo.internal)
title: "dns"
---
...
```bash
kibo dns setup
kibo dns setup --domain kibo.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `kibo.internal`)
...
- zone file path
- current tailnet IPs
- recommended `kibo.json` discovery config
...
- If `--domain` is omitted, Kibo uses `discovery.wideArea.domain` from config.
