...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "kibo-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".kibo");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".kibo", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      KIBO_STATE_DIR: path.join(home, ".kibo"),
      ...env,
    };
