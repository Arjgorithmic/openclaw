...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: KiboConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: KiboConfig) {
...
function resolveHeartbeatAgents(cfg: KiboConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: KiboConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: KiboConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: KiboConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
