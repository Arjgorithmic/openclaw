...
import { patchChannelConfigForAccount } from "kibo/plugin-sdk/setup-runtime";
...
import { getChatChannelMeta, type ChannelPlugin, type KiboConfig } from "./channel-api.js";
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Kibo";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Kibo`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "Kibo"): string[] {
  return [
