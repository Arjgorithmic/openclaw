...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__KIBO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<KiboTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.kibo", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = KiboTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> KiboTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return KiboTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> KiboTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> KiboTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> KiboTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = KiboTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: KiboTalkPTTStopPayload) {
