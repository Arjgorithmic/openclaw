...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<KiboConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<KiboConfig["channels"]>["telegram"]>>,
): KiboConfig {
  return {
    channels: {
      telegram: {
