...
metadata: { "kibo": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Kibo `sessions_spawn`
- **File I/O** == Kibo `read`/`write`
- **Remote fetch** == Kibo `web_fetch` (or `exec` with curl when POST is required)
