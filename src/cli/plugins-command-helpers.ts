...
import { KIBOHUB_INSTALL_ERROR_CODE } from "../plugins/kibohub.js";
...
export function applySlotSelectionForPlugin(
  config: KiboConfig,
  pluginId: string,
): { config: KiboConfig; warnings: string[] } {
...
export function buildPreferredKiboHubSpec(raw: string): string | null {
  const parsed = parseRegistryNpmSpec(raw);
  if (!parsed) {
    return null;
  }
  return `kibohub:${parsed.name}${parsed.selector ? `@${parsed.selector}` : ""}`;
}

export const PREFERRED_KIBOHUB_FALLBACK_DECISION = {
  FALLBACK_TO_NPM: "fallback_to_npm",
  STOP: "stop",
} as const;

export type PreferredKiboHubFallbackDecision =
  (typeof PREFERRED_KIBOHUB_FALLBACK_DECISION)[keyof typeof PREFERRED_KIBOHUB_FALLBACK_DECISION];

export function decidePreferredKiboHubFallback(params: {
  code?: string;
}): PreferredKiboHubFallbackDecision {
  if (
    params.code === KIBOHUB_INSTALL_ERROR_CODE.PACKAGE_NOT_FOUND ||
    params.code === KIBOHUB_INSTALL_ERROR_CODE.VERSION_NOT_FOUND
  ) {
    return PREFERRED_KIBOHUB_FALLBACK_DECISION.FALLBACK_TO_NPM;
  }
  return PREFERRED_KIBOHUB_FALLBACK_DECISION.STOP;
}
