...
const AGENT_DIGEST_PATH = ".kibo-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".kibo-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- kibo:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- kibo:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- kibo:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- kibo:wiki:${group.dir}:index:end -->`,
