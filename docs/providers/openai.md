---
summary: "Use OpenAI via API keys or Codex subscription in Kibo"
read_when:
  - You want to use OpenAI models in Kibo
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Kibo.

## Default interaction style

Kibo can add a small OpenAI-specific prompt overlay for both `openai/*` and
...
without replacing the base Kibo system prompt. The friendly overlay also
...
- `"off"`: disable the overlay and use the base Kibo prompt only.
...
If you want the unmodified base Kibo prompt, set the overlay to `"off"`:
...
```bash
kibo config set plugins.entries.openai.config.personality off
```

Kibo normalizes this setting case-insensitively at runtime, so values like
...
- In Kibo, ChatGPT/Codex sign-in is routed through `openai-codex/*`, not `openai/*`

### CLI setup

```bash
kibo onboard --auth-choice openai-api-key
# or non-interactive
kibo onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Kibo forwards both through the `openai/*` Responses path.
Kibo intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Kibo does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Kibo.
...
- Supports `size`
- Current OpenAI-specific caveat: Kibo does not forward `aspectRatio` or
...
- Current OpenAI-specific caveat: Kibo currently only forwards `size`
...
kibo onboard --auth-choice openai-codex

# Or run OAuth directly
kibo models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Kibo
...
managed by Codex CLI. On expiry, Kibo re-reads the external Codex source
first and, when the provider can refresh it, writes the refreshed credential
back to Codex storage instead of taking ownership in a separate Kibo-only
copy.

If your Codex account is entitled to Codex Spark, Kibo also supports:

- `openai-codex/gpt-5.3-codex-spark`

Kibo treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Kibo also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Kibo treats the Codex model metadata and the runtime context cap as separate
values.
...
Kibo uses `pi-ai` for model streaming. For both `openai/*` and
`openai-codex/*`, default transport is `"auto"` (WebSocket-first, then SSE
fallback).

In `"auto"` mode, Kibo also retries one early, retryable WebSocket failure
...
After a connect or early-turn WebSocket failure in `"auto"` mode, Kibo marks
...
OpenAI Responses), Kibo also attaches stable session and turn identity state
...
Kibo also normalizes OpenAI usage counters across transport variants before
...
`prompt_tokens` / `completion_tokens`; Kibo treats those as the same input
and output counters for `/status`, `/usage`, and session logs. When native
WebSocket traffic omits `total_tokens` (or reports `0`), Kibo falls back to
...
For `openai/*` (Responses API), Kibo also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Kibo enables it by default for
...
Kibo, set `agents.defaults.models["<provider>/<model>"].params.serviceTier`
...
Kibo forwards `params.serviceTier` to both direct `openai/*` Responses
...
- if you route either provider through another base URL or proxy, Kibo leaves `service_tier` untouched

### OpenAI fast mode

Kibo exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Kibo maps it to OpenAI priority processing:
...
Kibo treats direct OpenAI, Codex, and Azure OpenAI endpoints differently
...
- hidden Kibo attribution headers (`originator`, `version`, and
...
`baseUrl` on `api.openai.com`), Kibo now auto-enables OpenAI server-side
