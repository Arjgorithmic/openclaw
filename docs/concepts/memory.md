...
summary: "How Kibo remembers things across sessions"
read_when:
...
Kibo remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.kibo/workspace`).
...
<Info>
Kibo auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Kibo
...
```bash
kibo memory rem-backfill --rollback
kibo memory rem-backfill --rollback-short-term
```

## CLI

```bash
kibo memory status          # Check index status and provider
kibo memory search "query"  # Search from the command line
kibo memory index --force   # Rebuild the index
```
