...
let lastCreateKiboToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/kibo-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateKiboToolsContext?.agentTo,
          agentThreadId: lastCreateKiboToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createKiboTools: (ctx: Record<string, unknown>) => {
      lastCreateKiboToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateKiboToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-kibo-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-kibo-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateKiboToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateKiboToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateKiboToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateKiboToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-kibo-message-to": "channel:24514",
        "x-kibo-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
