...
declare const __KIBO_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "kibo";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["KIBO_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["KIBO_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["KIBO_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.KIBO_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Kibo version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __KIBO_VERSION__ === "string" ? __KIBO_VERSION__ : undefined,
  bundledVersion: process.env.KIBO_BUNDLED_VERSION,
});
