...
type EnvSnapshot = {
  HOME?: string;
  KIBO_HOME?: string;
  KIBO_STATE_DIR?: string;
  KIBO_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    KIBO_HOME: process.env.KIBO_HOME,
    KIBO_STATE_DIR: process.env.KIBO_STATE_DIR,
    KIBO_OAUTH_DIR: process.env.KIBO_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: KiboConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: KiboConfig) {
...
async function runStateIntegrityText(cfg: KiboConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.KIBO_HOME = tempHome;
    process.env.KIBO_STATE_DIR = path.join(tempHome, ".kibo");
    delete process.env.KIBO_OAUTH_DIR;
    fs.mkdirSync(process.env.KIBO_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when KIBO_OAUTH_DIR is explicitly configured", async () => {
    process.env.KIBO_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: KiboConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("prints kibo-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: KiboConfig = {};
    ...
    expect(text).toMatch(/kibo sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/kibo sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /kibo sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
