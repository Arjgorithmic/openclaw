...
import type { KibobotConfig, RuntimeEnv } from "../runtime-api.js";
...
describe("Feishu bot menu handler", () => {
  beforeEach(() => {
    handlers = {};
    vi.clearAllMocks();
    process.env.KIBO_STATE_DIR = `/tmp/kibo-feishu-bot-menu-test-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  });
