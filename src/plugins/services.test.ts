...
import type { KiboPluginService, KiboPluginServiceContext } from "./types.js";
...
function createRegistry(services: KiboPluginService[]) {
...
function expectServiceLogger(ctx: KiboPluginServiceContext) {
...
function expectServiceContexts(
  contexts: KiboPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: KiboPluginServiceContext[] = [];
