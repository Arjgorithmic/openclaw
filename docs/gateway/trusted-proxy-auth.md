...
read_when:
  - Running Kibo behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Kibo
...
- You run Kibo behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Kibo checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Kibo extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that Kibo can verify.
...
- Kibo can stay on loopback HTTP behind the proxy.
...
If Kibo itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://kibo.example.com
    to: http://kibo-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
Kibo rejects ambiguous configurations where both a `gateway.auth.token` (or `KIBO_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-kibo-scopes`.

Examples:

- `x-kibo-scopes: operator.read`
- `x-kibo-scopes: operator.read,operator.write`
- `x-kibo-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Kibo honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-kibo-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-kibo-scopes` explicitly when you want a trusted-proxy request to
...
`kibo security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
Kibo rejected a loopback-source trusted-proxy request.
...
3. Update Kibo config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `kibo security audit` and review findings
