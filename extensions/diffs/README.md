...
Read-only diff viewer plugin for **Kibo** agents.
...
Set plugin-wide defaults in `~/.kibo/kibo.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/kibo",
        },
      },
    },
  },
...
Before:
Kibo supports plugins.

After:
Kibo supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/kibo-diffs`).
