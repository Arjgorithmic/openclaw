...
read_when:
  ...
  - You are defining plugin config schemas or package.json kibo metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`kibo.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `kibo` field that tells the plugin system what
...
**Provider plugin / KiboHub publish baseline:**

```json kibo-kibohub-package.json
{
...
If you publish the plugin externally on KiboHub, those `compat` and `build`
...
`kibo.channel` is cheap package metadata for channel discovery and setup
...
`kibo.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported Kibo version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `kibo doctor --fix`.
...
When enabled, Kibo loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `kibo.plugin.json` in the package root.
Kibo uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Kibo",
  "configSchema": {
    "type": "object",
...
For plugin packages, use the package-specific KiboHub command:
...
always use `kibohub package publish`.

## Setup entry

The `setup-entry.ts` file is a lightweight alternative to `index.ts` that
Kibo loads when it only needs setup surfaces (onboarding, config repair,
...
**When Kibo uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `kibo/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Kibo validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "kibo/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `kibo onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "kibo/plugin-sdk/channel-setup";
...
helpers from `kibo/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`kibo/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `kibo/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "kibo/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/kibo-my-channel",
  docsPath: "/channels/my-channel",
});
...
**External plugins:** publish to [KiboHub](/tools/kibohub) or npm, then install:

```bash
kibo plugins install @myorg/kibo-my-plugin
```

Kibo tries KiboHub first and falls back to npm automatically. You can also
force KiboHub explicitly:

```bash
kibo plugins install kibohub:@myorg/kibo-my-plugin   # KiboHub only
```

There is no matching `npm:` override. Use the normal npm package spec when you
want the npm path after KiboHub fallback:
...
<Info>
  For npm-sourced installs, `kibo plugins install` runs
