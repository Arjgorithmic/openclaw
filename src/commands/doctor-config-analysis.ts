...
import { KiboSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: KiboConfig): {
  config: KiboConfig;
  removed: string[];
} {
  const parsed = KiboSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: KiboConfig): void {
