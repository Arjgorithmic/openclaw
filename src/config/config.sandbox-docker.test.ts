...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("kibo-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "kibo-sandbox-browser-global" },
      agentBrowser: { network: "kibo-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("kibo-sandbox-browser-agent");
  });
