...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.KIBO_BASE_ENV).toBe("base");
    expect(resolved.KIBO_TEST_ENV).toBe("ok");
    expect(resolved.KIBO_TO_REMOVE).toBeUndefined();
    expect(resolved.KIBO_CLI).toBe(KIBO_CLI_ENV_VALUE);
  });
