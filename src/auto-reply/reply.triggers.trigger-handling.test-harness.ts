...
const piEmbeddedMocks = getSharedMocks("kibo.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("kibo.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("kibo.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("kibo.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    kiboHome: process.env.KIBO_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("KIBO_HOME", snapshot.kiboHome);
  restoreKey("KIBO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.KIBO_STATE_DIR = join(home, ".kibo");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "kibo-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".kibo", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): KiboConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "kibo"),
