...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.KIBO_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    KIBO_PROFILE: profile,
    KIBO_GATEWAY_PORT: String(port),
    KIBO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    KIBO_SYSTEMD_UNIT: systemdUnit,
    KIBO_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    KIBO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    KIBO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    KIBO_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.KIBO_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    KIBO_GATEWAY_TOKEN: gatewayToken,
    KIBO_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    KIBO_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    KIBO_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    KIBO_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    KIBO_LOG_PREFIX: "node",
    KIBO_SERVICE_MARKER: NODE_SERVICE_MARKER,
    KIBO_SERVICE_KIND: NODE_SERVICE_KIND,
    KIBO_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    KIBO_STATE_DIR: sharedEnv.stateDir,
    KIBO_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.KIBO_STATE_DIR;
  const configPath = env.KIBO_CONFIG_PATH;
