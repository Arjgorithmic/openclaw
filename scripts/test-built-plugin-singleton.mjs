...
const { clearPluginCommands, getPluginCommandSpecs, loadKiboPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadKiboPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "kibo.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'kibo/plugin-sdk';",
...
const registry = loadKiboPlugins({
  ...
  env: {
    ...process.env,
    KIBO_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
