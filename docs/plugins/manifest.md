...
read_when:
  - You are building an Kibo plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (kibo.plugin.json)

This page is for the **native Kibo plugin manifest** only.
...
Kibo auto-detects those bundle layouts too, but they are not validated
against the `kibo.plugin.json` schema described here.

For compatible bundles, Kibo currently reads bundle metadata plus declared
...
Kibo runtime expectations.

Every native Kibo plugin **must** ship a `kibo.plugin.json` file in the
**plugin root**. Kibo uses this manifest to validate configuration
...
`kibo.plugin.json` is the metadata Kibo reads before it loads your
plugin code.
...
| `providerAuthEnvVars`               | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Kibo can inspect without loading plugin code.                                                                                                                      |
| `providerAuthAliases`               | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                   |
| `channelEnvVars`                    | No       | `Record<string, string[]>`       | Cheap channel env metadata that Kibo can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                     |
...
Kibo reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Kibo falls back to `choiceId`.                                        |
...
Use `contracts` only for static capability ownership metadata that Kibo can
...
Use `modelSupport` when Kibo should infer your provider plugin from
...
Legacy top-level capability keys are deprecated. Use `kibo doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `kibo.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `kibo` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Kibo must know it before loading plugin code, put it in `kibo.plugin.json`
...
`kibo` block instead of `kibo.plugin.json`.

Important examples:

| Field                                                             | What it means                                                                                                                                |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `kibo.extensions`                                             | Declares native plugin entrypoints.                                                                                                          |
| `kibo.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding and deferred channel startup.                                                       |
| `kibo.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                         |
| `kibo.channel.configuredState`                                | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime. |
| `kibo.channel.persistedAuthState`                             | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.       |
| `kibo.install.npmSpec` / `kibo.install.localPath`         | Install/update hints for bundled and externally published plugins.                                                                           |
| `kibo.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                                                                          |
| `kibo.install.minHostVersion`                                 | Minimum supported Kibo host version, using a semver floor like `>=2026.3.22`.                                                            |
| `kibo.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                               |
| `kibo.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                         |

`kibo.install.minHostVersion` is enforced during install and manifest
registry loading. Invalid values are rejected; newer-but-valid values skip the
plugin on older hosts.

`kibo.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
`kibo.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`kibo.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
- The manifest is **required for native Kibo plugins**, including local filesystem loads.
