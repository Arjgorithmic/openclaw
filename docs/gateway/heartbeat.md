...
- During heartbeat runs, Kibo treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `kibo sessions --json` or the [sessions CLI](/cli/sessions)).
...
- If only alert delivery is disabled, Kibo can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
...
If **all three** are false, Kibo skips the heartbeat run entirely (no model call).
...
- If a task is blocked, write down _what is missing_ and ask Kibo next time.
```
...
- Kibo parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
kibo system event --text "Check for urgent follow-ups" --mode now
```
