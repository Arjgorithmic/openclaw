...
**Import:** `kibo/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `KiboPluginConfigSchema \| () => KiboPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: KiboPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `kibo.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Kibo resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `kibo/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: KiboPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: KiboPluginApi) => void`                               | No       | —                   |
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Kibo
...
Kibo loads this instead of the full entry when a channel is disabled,
...
- `kibo/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `kibo/plugin-sdk/channel-setup` for optional-install setup surfaces
- `kibo/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Kibo to lazy-load the real CLI module on first invocation
...
Kibo classifies loaded plugins by their registration behavior:
...
Use `kibo plugins inspect <id>` to see a plugin's shape.
