...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".kibo-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + shell-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateKiboProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_KIBO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_KIBO_BROWSER_COLOR).toUpperCase();
