...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers KIBO_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for KIBO_HOME",
      env: { KIBO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when KIBO_HOME is tilde-only and no fallback home exists",
      env: { KIBO_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores KIBO_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          KIBO_HOME: "/srv/kibo-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { KIBO_HOME: "/srv/kibo-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/kibo-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/kibo-home " },
      expected: "/srv/kibo-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/kibo-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of KIBO_HOME", () => {
    expect(
