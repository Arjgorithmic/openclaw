...
Kibo supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Kibo reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.kibo/.env`.

If you omit `baseUrl`, Kibo defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Kibo defaults to `kimi-k2.5`.
