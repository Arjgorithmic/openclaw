...
Kibo can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, or OpenAI.
It works anywhere Kibo can send audio.
...
When `messages.tts.provider` is unset, Kibo picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `kibo.json`.
...
- If `provider` is **unset**, Kibo uses the first configured speech provider in registry auto-select order.
...
`~/.kibo/settings/tts.json`, override with `KIBO_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Kibo retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Kibo registers
