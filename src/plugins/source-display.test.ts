...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "kibo", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".kibo", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".kibo", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "kibo-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".kibo", "extensions"),
      },
    });
  });
});
