...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "kibo-plugin-tools";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "kibo.plugin.json")) &&
...
function resolveAcpxPluginRootFromKiboLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Kibo layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromKiboLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveKiboRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const kiboRoot = resolveKiboRoot(pluginRoot);
  const distEntry = path.join(kiboRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(kiboRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
