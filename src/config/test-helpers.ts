...
import { resetConfigRuntimeState, type KiboConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        KIBO_DISABLE_PLUGIN_DISCOVERY_CACHE: undefined,
        KIBO_DISABLE_PLUGIN_MANIFEST_CACHE: undefined,
        KIBO_PLUGIN_CATALOG_PATHS: undefined,
        KIBO_MPM_CATALOG_PATHS: undefined,
        KIBO_PLUGIN_DISCOVERY_CACHE_MS: undefined,
        KIBO_PLUGIN_MANIFEST_CACHE_MS: undefined,
      },
    });
  } finally {
    resetConfigTestRuntimeState();
...
export async function writeKiboConfig(home: string, config: unknown): Promise<string> {
  const configPath = path.join(home, ".kibo", "kibo.json");
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.KIBO_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected KIBO_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeKiboConfig(home, config);
