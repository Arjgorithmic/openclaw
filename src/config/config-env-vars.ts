...
function collectConfigEnvVarsByTarget(cfg?: KiboConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: KiboConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: KiboConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: KiboConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.KIBO_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
