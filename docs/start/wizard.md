...
CLI onboarding is the **recommended** way to set up Kibo on macOS,
...
`kibo dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`kibo configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.kibo/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `kibo doctor` first.
</Note>
...
Use `kibo agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.kibo/workspace-<agentId>`.
...
- CLI command reference: [`kibo onboard`](/cli/onboard)
