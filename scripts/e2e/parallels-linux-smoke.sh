...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/kibo-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/kibo-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/kibo-install.sh
  guest_exec /usr/bin/env KIBO_NO_ONBOARD=1 bash /tmp/kibo-install.sh "${version_args[@]}" --no-onboard
  guest_exec kibo --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec kibo --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" kibo onboard \
    --non-interactive \
    --mode local \
...
rm -f /tmp/kibo-parallels-linux-gateway.log
setsid sh -lc 'exec env KIBO_HOME=/root KIBO_STATE_DIR=/root/.kibo KIBO_CONFIG_PATH=/root/.kibo/kibo.json ${API_KEY_ENV}=${api_key_value_q} kibo gateway run --bind loopback --port 18789 --force >/tmp/kibo-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec kibo gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec kibo gateway status --deep --require-rpc
    return
  fi
  guest_exec kibo gateway status --deep
}

verify_local_turn() {
  guest_exec kibo models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" kibo agent \
    --local \
    --agent main \
    --message ping \
    --json
