...
read_when:
  ...
  - Debugging why kibo is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Kibo-managed)"
---

# Browser (kibo-managed)

Kibo can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `kibo` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **kibo** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`kibo`, `work`, `remote`, ...).
...
```bash
kibo browser --browser-profile kibo status
...
If `kibo browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`kibo browser`),
...
If `kibo browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "kibo"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.kibo/kibo.json`.
...
- If you override the Gateway port (`gateway.port` or `KIBO_GATEWAY_PORT`),
...
- Default profile is `kibo` (Kibo-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `kibo` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
Kibo uses it automatically. Set `browser.executablePath` to override
auto-detection:

CLI example:

```bash
kibo config set browser.executablePath "/usr/bin/google-chrome"
```
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Kibo will not launch a local browser.

Stopping behavior differs by profile mode:

- local managed profiles: `kibo browser stop` stops the browser process that
  Kibo launched
- attach-only and remote CDP profiles: `kibo browser stop` closes the active
  ...
  though no browser process was launched by Kibo
...
Kibo preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Kibo can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Kibo treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. Kibo can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Kibo
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Kibo supports both:

- **HTTP(S) endpoints** — Kibo calls `/json/version` to discover the
  WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — Kibo connects directly,
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-kibo-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Kibo
  auto-generates `gateway.auth.token` on startup and persists it to config.
- Kibo does **not** auto-generate that token when `gateway.auth.mode` is
...
Kibo supports multiple named profiles (routing configs). Profiles can be:

- **kibo-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `kibo` profile is auto-created if missing.
...
Kibo can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Kibo attaches.
...
```bash
kibo browser --browser-profile user start
kibo browser --browser-profile user status
kibo browser --browser-profile user tabs
kibo browser --browser-profile user snapshot --format ai
```
...
- `kibo doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `kibo` profile because it can
  act inside your signed-in browser session.
- Kibo does not launch the browser for this driver; it attaches to an
  existing session only.
- Kibo uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, Kibo passes it through to target that explicit
...
When launching locally, Kibo picks the first available:
...
- `x-kibo-password: <gateway password>` or HTTP Basic auth with that password
...
- ARIA snapshots
- Page screenshots for the managed `kibo` browser when a per-tab CDP
...
```bash
docker compose run --rm kibo-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`KIBO_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `kibo browser snapshot --interactive --compact --depth 6`
- `kibo browser snapshot --efficient`
- `kibo browser snapshot --labels`
- `kibo browser snapshot --selector "#main" --interactive`
- `kibo browser snapshot --frame "iframe#main" --interactive`
- `kibo browser console --level error`

Lifecycle note:

- For attach-only and remote CDP profiles, `kibo browser stop` is still the
...
- `kibo browser requests --filter api --clear`
- `kibo browser pdf`
- `kibo browser responsebody "**/api" --max-chars 5000`
...
- `kibo browser storage local set theme dark`
- `kibo browser storage session clear`
- `kibo browser set offline on`
- `kibo browser set headers --headers-json '{"X-Debug":"1"}'`
- `kibo browser set credentials user pass`
- `kibo browser set credentials --clear`
- `kibo browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `kibo browser set geo --clear`
- `kibo browser set media dark`
- `kibo browser set timezone America/New_York`
- `kibo browser set locale en-US`
...
- Download and trace output paths are constrained to Kibo temp roots:
  - traces: `/tmp/kibo` (fallback: `${os.tmpdir()}/kibo`)
  - downloads: `/tmp/kibo/downloads` (fallback: `${os.tmpdir()}/kibo/downloads`)
- Upload paths are constrained to an Kibo temp uploads root:
  - uploads: `/tmp/kibo/uploads` (fallback: `${os.tmpdir()}/kibo/uploads`)
...
Kibo supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `kibo browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `kibo browser click 12`, `kibo browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `kibo browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `kibo browser click e12`, `kibo browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `kibo browser wait --url "**/dash"`
- Wait for load state:
  - `kibo browser wait --load networkidle`
- Wait for a JS predicate:
  - `kibo browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `kibo browser wait "#main"`
...
1. `kibo browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `kibo browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `kibo browser trace start`
   - reproduce the issue
   - `kibo browser trace stop` (prints `TRACE:<path>`)
...
```bash
kibo browser status --json
kibo browser snapshot --interactive --json
kibo browser requests --filter api --json
kibo browser cookies --json
```
...
- The kibo browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `kibo browser evaluate` and `wait --fn`
...
- `browser` accepts:
  - `profile` to choose a named browser profile (kibo, chrome, or remote CDP).
