---
summary: "How Kibo builds prompt context and reports token usage + costs"
read_when:
...
Kibo tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Kibo assembles its own system prompt on every run. It includes:
...
For images, Kibo downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Kibo session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `kibo status --usage` and `kibo channels list` show
...
available; otherwise Kibo falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, Kibo shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Kibo can
...
Anthropic's 1M context window is currently beta-gated. Kibo can inject the
...
Kibo skips the `context-1m-*` beta header because Anthropic currently
