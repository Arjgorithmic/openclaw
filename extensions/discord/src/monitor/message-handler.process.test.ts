import { DEFAULT_EMOJIS } from "kibo/plugin-sdk/channel-feedback";
import type { ReplyPayload } from "kibo/plugin-sdk/reply-dispatch-runtime";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/kibo-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("kibo/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("kibo/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("kibo/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/kibo-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/kibo-discord-process-test-sessions.json" },
      },
    });
