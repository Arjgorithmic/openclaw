...
const loadKiboPluginsMock = vi.fn();
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadKiboPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
