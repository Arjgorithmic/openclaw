...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "kibo", "--help"])).toEqual({
      argv: ["node", "kibo", "--help"],
      commandPath: [],
      primary: null,
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "kibo", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "kibo", "--profile", "work", "gateway", "status"],
