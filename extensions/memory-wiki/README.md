...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Kibo**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.kibo-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
kibo wiki doctor
kibo wiki init
kibo wiki ingest ./notes/alpha.md
kibo wiki compile
kibo wiki lint
kibo wiki search "alpha"
kibo wiki get entity.alpha --from 1 --lines 80

kibo wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

kibo wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

kibo wiki bridge import
kibo wiki unsafe-local import

kibo wiki obsidian status
kibo wiki obsidian search "alpha"
kibo wiki obsidian open syntheses/alpha-summary.md
kibo wiki obsidian command workspace:quick-switcher
kibo wiki obsidian daily
```
...
- The compiled agent digests in `.kibo-wiki/cache/agent-digest.json` and `.kibo-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
