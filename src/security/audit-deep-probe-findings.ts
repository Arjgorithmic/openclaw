...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("kibo status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("kibo security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set KIBO_GATEWAY_TOKEN/KIBO_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("kibo security audit --deep")}".`,
    });
  }
  return findings;
}
