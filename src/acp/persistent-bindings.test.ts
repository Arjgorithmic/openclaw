...
type ConfiguredBinding = NonNullable<KiboConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: KiboConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/kibo" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
  ...
  it("clears existing configured ACP sessions and lets the next turn recreate them", async () => {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockReturnValue({
      acp: {
        ...
        mode: "persistent",
        backend: "acpx",
        runtimeOptions: { cwd: "/home/bob/kibo" },
      },
    });
