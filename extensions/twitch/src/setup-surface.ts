...
function resolveSetupAccountId(cfg: KiboConfig): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: KIBO_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: KiboConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var KIBO_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
function resolveTwitchGroupPolicy(cfg: KiboConfig): "open" | "allowlist" | "disabled" {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.KIBO_TWITCH_ACCESS_TOKEN?.trim();
