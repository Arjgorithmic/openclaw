...
export const POSIX_KIBO_TMP_DIR = "/tmp/kibo";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredKiboTmpDirOptions = {
...
export function resolvePreferredKiboTmpDir(
  options: ResolvePreferredKiboTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "kibo" : `kibo-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[kibo] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Kibo temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Kibo temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_KIBO_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_KIBO_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_KIBO_TMP_DIR)) {
      return POSIX_KIBO_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_KIBO_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_KIBO_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_KIBO_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_KIBO_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_KIBO_TMP_DIR;
