...
private let webChatSwiftLogger = Logger(subsystem: "ai.kibo", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "kibo.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: KiboChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> KiboChatSessionsListResponse {
        ...
        let defaults = decoded.defaults.map {
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return KiboChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [KiboChatAttachmentPayload]) async throws -> KiboChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> KiboChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                KiboGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: KiboProtocol.ModelChoice) -> KiboChatModelChoice {
        KiboChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<KiboChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any KiboChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = KiboChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: KiboChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<KiboChatView>) -> NSViewController
    {
