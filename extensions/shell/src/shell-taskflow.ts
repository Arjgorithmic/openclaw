...
import type { ShellEnvelope, ShellRunner, ShellRunnerParams } from "./shell-runner.js";
...
export type RunManagedShellFlowParams = {
  taskFlow: BoundTaskFlow;
  runner: ShellRunner;
  runnerParams: ShellRunnerParams;
  controllerId: string;
  goal: string;
  stateJson?: JsonLike;
  currentStep?: string;
...
export type ResumeManagedShellFlowParams = {
  taskFlow: BoundTaskFlow;
  runner: ShellRunner;
  runnerParams: ShellRunnerParams & {
    action: "resume";
...
export type ManagedShellFlowResult =
  | {
      ok: true;
...
function buildApprovalWaitState(envelope: Extract<ShellEnvelope, { ok: true }>): JsonLike {
  if (!envelope.requiresApproval) {
    return {
      kind: "shell_approval",
      prompt: "",
      items: [],
    } satisfies ShellApprovalWaitState;
  }
  return {
    kind: "shell_approval",
...
function buildEnvelopeError(envelope: Extract<ShellEnvelope, { ok: false }>) {
  return new Error(envelope.error.message);
}

export async function runManagedShellFlow(
  params: RunManagedShellFlowParams,
): Promise<ManagedShellFlowResult> {
  const flow = params.taskFlow.createManaged({
    controllerId: params.controllerId,
    goal: params.goal,
    currentStep: params.currentStep ?? "run_shell",
  ...
  try {
    ...
    const mutation = applyEnvelopeToFlow({
      ...
      flow,
      envelope,
      waitingStep: params.waitingStep ?? "await_shell_approval",
    });
    if (!envelope.ok) {
      return {
        ok: false,
...
export async function resumeManagedShellFlow(
  params: ResumeManagedShellFlowParams,
): Promise<ManagedShellFlowResult> {
  const resumed = params.taskFlow.resume({
    flowId: params.flowId,
    expectedRevision: params.expectedRevision,
    status: "running",
    currentStep: params.currentStep ?? "resume_shell",
  });
