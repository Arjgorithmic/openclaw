...
read_when:
  - You want to use Groq with Kibo
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Kibo connects
...
media-understanding provider, Kibo uses Groq's `whisper-large-v3-turbo`
...
Groq's model catalog changes frequently. Run `kibo models list | grep groq`
