...
const SEARCH_TOOL = "https://github.com/Arjgorithmic/openclaw/mcp.SearchKibo";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.kibo.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('kibo docs "your query"')}`);
    } else {
      runtime.log("Docs: https://github.com/Arjgorithmic/openclaw/");
      runtime.log(`Search: ${formatCliCommand('kibo docs "your query"')}`);
    }
    return;
  }
