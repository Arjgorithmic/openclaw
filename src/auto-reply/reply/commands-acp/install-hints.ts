...
export function resolveAcpInstallCommandHint(cfg: KiboConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (workspaceLocalPath && existsSync(workspaceLocalPath)) {
      return `kibo plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^kibo plugins install /u, "");
    ...
    return "kibo plugins install acpx";
  }
