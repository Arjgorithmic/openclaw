...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("kibo-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("kibo-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("kibo-sbx-");
