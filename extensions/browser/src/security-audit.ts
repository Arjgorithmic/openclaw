import type { KiboPluginSecurityAuditContext } from "kibo/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "kibo/plugin-sdk/secret-input";
import { formatCliCommand } from "kibo/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "kibo/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: KiboPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("kibo security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.KIBO_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.KIBO_GATEWAY_PASSWORD) ||
