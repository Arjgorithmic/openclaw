...
read_when:
  - You want to use Vercel AI Gateway with Kibo
...
- Kibo auto-discovers the Gateway `/v1/models` catalog, so `/models vercel-ai-gateway`
...
```bash
kibo onboard --auth-choice ai-gateway-api-key
```
...
Kibo accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
