...
const TEMP_MEDIA_PREFIX = "kibo-media-";
...
function createGroqAudioConfig(): KiboConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): KiboConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): KiboConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: KiboConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredKiboTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        KIBO_AGENT_DIR: isolatedAgentDir,
