...
import { VERSION } from "kibo/plugin-sdk/cli-runtime";
import { resolveAmbientNodeProxyAgent } from "kibo/plugin-sdk/extension-shared";
import { danger, success } from "kibo/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "kibo/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "kibo/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("kibo channels login")}`,
              ),
            );
          }
        }
