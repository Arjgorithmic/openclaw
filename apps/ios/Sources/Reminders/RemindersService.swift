...
final class RemindersService: RemindersServicing {
    func list(params: KiboRemindersListParams) async throws -> KiboRemindersListPayload {
        ...
        let payload: [KiboReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return KiboRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: KiboRemindersAddParams) async throws -> KiboRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = KiboReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return KiboRemindersAddPayload(reminder: payload)
    }
