  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-kibo-account-id": "work",
        "x-kibo-message-channel": "telegram",
      },
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.kibo?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.kibo?.headers?.Authorization).toBe(
      "Bearer ${KIBO_MCP_TOKEN}",
    );
    expect(config.mcpServers?.kibo?.headers?.["x-kibo-message-channel"]).toBe(
      "${KIBO_MCP_MESSAGE_CHANNEL}",
    );
  });
});
