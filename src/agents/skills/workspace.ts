...
function resolveSkillsLimits(config?: KiboConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveKiboMetadata(frontmatter),
      invocation,
      exposure: {
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: KiboConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`kibo skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`kibo skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
