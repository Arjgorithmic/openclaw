...
function resolveGoogleChatAccountImpl(params: { cfg: KiboConfig; accountId?: string | null }) {
...
vi.mock("./channel.deps.runtime.js", () => {
  return {
    ...
    listGoogleChatAccountIds: (cfg: KiboConfig) => {
...
function createGoogleChatCfg(): KiboConfig {
  return {
    channels: {
      googlechat: {
        enabled: true,
