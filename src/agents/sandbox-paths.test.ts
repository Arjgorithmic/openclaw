...
async function withOutsideHardlinkInKiboTmp<T>(
  params: {
    kiboTmpDir: string;
  ...
  const hardlinkPath = path.join(params.kiboTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.kiboTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.kiboTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.kiboTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const kiboTmpDir = resolvePreferredKiboTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Kibo tmp root",
      media: path.join(kiboTmpDir, "image.png"),
      expected: path.join(kiboTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Kibo tmp root",
      media: pathToFileURL(path.join(kiboTmpDir, "photo.png")).href,
      expected: path.join(kiboTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Kibo tmp root",
      media: path.join(kiboTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(kiboTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(kiboTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside kibo tmp root",
      media: path.join(os.tmpdir(), "outside-kibo", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Kibo tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(kiboTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(kiboTmpDir, { recursive: true });
      const symlinkPath = path.join(kiboTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Kibo tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInKiboTmp(
      {
        kiboTmpDir,
  ...
  it("rejects symlinked Kibo tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInKiboTmp(
