...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.kibo.mac`) even if the shipped
    ...
    {
        if environment["KIBO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "kibo.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "kibo.nixMode") { return true }
