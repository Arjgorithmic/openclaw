import { formatCliCommand, parseDurationMs } from "kibo/plugin-sdk/cli-runtime";
import type {
  KiboPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type KiboConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "kibo/plugin-sdk/provider-auth";
import { cloneFirstTemplateModel } from "kibo/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "kibo/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("kibo models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("kibo models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("kibo doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: KiboPluginApi): void {
