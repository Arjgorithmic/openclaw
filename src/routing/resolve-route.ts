...
function listAgents(cfg: KiboConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: KiboConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<KiboConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: KiboConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: KiboConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: KiboConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<KiboConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  KiboConfig,
  {
    bindingsRef: KiboConfig["bindings"];
    agentsRef: KiboConfig["agents"];
    sessionRef: KiboConfig["session"];
...
function resolveRouteCacheForConfig(cfg: KiboConfig): Map<string, ResolvedAgentRoute> {
