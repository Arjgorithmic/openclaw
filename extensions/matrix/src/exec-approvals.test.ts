...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "kibo-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<KiboConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<KiboConfig["channels"]>["matrix"]>>,
): KiboConfig {
  return {
    channels: {
      matrix: {
