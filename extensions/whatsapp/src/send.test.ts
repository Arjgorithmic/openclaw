...
import { redactIdentifier } from "kibo/plugin-sdk/logging-core";
...
let resetLogger: typeof import("kibo/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("kibo/plugin-sdk/runtime-env").setLoggerOverride;
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("kibo/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `kibo-outbound-${crypto.randomUUID()}.log`);
