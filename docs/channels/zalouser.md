...
read_when:
  - Setting up Zalo Personal for Kibo
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Kibo.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current Kibo releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `kibo plugins install @kibo/zalouser`
- Or from a source checkout: `kibo plugins install ./path/to/local/zalouser-plugin`
...
- `kibo pairing list zalouser`
- `kibo pairing approve zalouser <code>`
...
- On startup, Kibo resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Kibo stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Kibo state. Example:
...
- Kibo sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Kibo sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `kibo channels logout --channel zalouser && kibo channels login --channel zalouser`
...
- The channel now runs fully in Kibo without external CLI binaries.
