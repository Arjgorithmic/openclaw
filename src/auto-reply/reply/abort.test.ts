...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop kibo!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@kibo_bot", { botUsername: "kibo_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@kibo_bot", { botUsername: "kibo_bot" })).toBe(true);
