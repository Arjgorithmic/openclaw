---
summary: "Use StepFun models with Kibo"
read_when:
  - You want StepFun models in Kibo
...
Kibo includes a bundled StepFun provider plugin with two provider ids:
...
```bash
kibo onboard --auth-choice stepfun-standard-api-key-intl --stepfun-api-key "$STEPFUN_API_KEY"
kibo onboard --auth-choice stepfun-plan-api-key-intl --stepfun-api-key "$STEPFUN_API_KEY"
```
...
- The provider is bundled with Kibo, so there is no separate plugin install step.
...
- Use `kibo models list` and `kibo models set <provider/model>` to inspect or switch models.
