import type { KiboConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "kibo-bundled": {
    ...
    canOverride: ["kibo-bundled"],
    canBeOverriddenBy: ["kibo-managed", "kibo-plugin"],
  },
  "kibo-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["kibo-bundled", "kibo-plugin"],
    canBeOverriddenBy: ["kibo-managed"],
  },
  "kibo-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["kibo-bundled", "kibo-managed", "kibo-plugin"],
    canBeOverriddenBy: ["kibo-managed"],
  },
  "kibo-workspace": {
    precedence: 40,
    ...
    canOverride: ["kibo-workspace"],
    canBeOverriddenBy: ["kibo-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "kibo-plugin") {
    return { enabled: true };
  }
