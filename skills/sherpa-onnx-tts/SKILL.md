...
1. Download the runtime for your OS (extracts into `$KIBO_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.kibo/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$KIBO_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.kibo/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Kibo config file (`$KIBO_CONFIG_PATH`, default `~/.kibo/kibo.json`):
