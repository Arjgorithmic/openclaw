...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.KIBO_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Kibo";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Kibo\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.KIBO_MDNS_HOSTNAME?.trim() || "kibo";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
