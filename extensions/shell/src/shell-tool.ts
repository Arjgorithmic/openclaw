...
import {
  createEmbeddedShellRunner,
  resolveShellCwd,
  type ShellRunner,
  type ShellRunnerParams,
} from "./shell-runner.js";
import {
  type ManagedShellFlowResult,
  resumeManagedShellFlow,
  runManagedShellFlow,
} from "./shell-taskflow.js";
...
function resolveManagedFlowToolResult(result: ManagedShellFlowResult) {
  if (!result.ok) {
    throw result.error;
  }
...
export function createShellTool(api: KiboPluginApi, options?: ShellToolOptions) {
  const runner = options?.runner ?? createEmbeddedShellRunner();
  return {
    name: "shell",
    label: "Shell Workflow",
    description:
      "Run Shell pipelines as a local-first workflow runtime (typed JSON envelope + resumable approvals).",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      const cwd = resolveShellCwd(params.cwd);
      const timeoutMs = typeof params.timeoutMs === "number" ? params.timeoutMs : 20_000;
      const maxStdoutBytes =
        typeof params.maxStdoutBytes === "number" ? params.maxStdoutBytes : 512_000;

      if (api.runtime?.version && api.logger?.debug) {
        api.logger.debug(`shell plugin runtime=${api.runtime.version}`);
      }

      const runnerParams: ShellRunnerParams = {
        action,
      ...
      if (action === "run") {
        ...
        if (flowParams) {
          return resolveManagedFlowToolResult(
            await resumeManagedShellFlow({
              taskFlow: requireTaskFlowRuntime(taskFlow, "resume"),
              runner,
              runnerParams: runnerParams as ShellRunnerParams & {
                action: "resume";
                token: string;
                approve: boolean;
              },
