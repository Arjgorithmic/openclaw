...
import { resolveMemoryDreamingConfig } from "kibo/plugin-sdk/memory-core-host-status";
import { normalizeLowercaseStringOrEmpty } from "kibo/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: KiboConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: KiboConfig, enabled: boolean): KiboConfig {
...
function formatStatus(cfg: KiboConfig): string {
...
export function registerDreamingCommand(api: KiboPluginApi): void {
  api.registerCommand({
    name: "dreaming",
