...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.kibo?.extensions)
    ? packageJson.kibo.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.kibo?.setupEntry === "string" &&
    packageJson.kibo.setupEntry.trim().length > 0
      ? packageJson.kibo.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "kibo.plugin.json"));
    }
    if (hasPackageJson) {
