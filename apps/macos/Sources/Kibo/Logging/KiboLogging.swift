...
enum KiboLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = KiboOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = KiboFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        KiboLogging.bootstrapIfNeeded()
        let label = KiboLogging.makeLabel(subsystem: subsystem, category: category)
...
struct KiboOSLogHandler: AppLogLevelBackedHandler {
...
struct KiboFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = KiboLogging.parseLabel(self.label)
