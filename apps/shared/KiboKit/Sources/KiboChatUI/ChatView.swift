...
@MainActor
public struct KiboChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: KiboChatViewModel
    ...
    private var visibleMessages: [KiboChatMessage] {
        let base: [KiboChatMessage]
    ...
    private func mergeToolResults(in messages: [KiboChatMessage]) -> [KiboChatMessage] {
    ...
    private func isToolResultMessage(_ message: KiboChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: KiboChatMessage) -> Bool {
    ...
    private func primaryText(in message: KiboChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: KiboChatMessage) -> Bool {
    ...
    private func toolCalls(in message: KiboChatMessage) -> [KiboChatMessageContent] {
    ...
    private func inlineToolResults(in message: KiboChatMessage) -> [KiboChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: KiboChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: KiboChatMessage) -> String {
