---
summary: "Use Synthetic's Anthropic-compatible API in Kibo"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Kibo registers it as the
...
```bash
kibo onboard --auth-choice synthetic-api-key
```
...
Note: Kibo's Anthropic client appends `/v1` to the base URL, so use
