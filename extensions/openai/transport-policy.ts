...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-kibo-turn-id": turnId,
      "x-kibo-turn-attempt": attempt,
    },
    metadata: {
      kibo_session_id: sessionHeaders["x-kibo-session-id"] ?? "",
      kibo_turn_id: turnId,
      kibo_turn_attempt: attempt,
      kibo_transport: ctx.transport,
    },
  };
}
