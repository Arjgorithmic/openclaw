...
This directory contains hooks that ship with Kibo. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.kibo/workspace`)
...
homepage: https://github.com/Arjgorithmic/openclaw/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.kibo/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.kibo/kibo.json`:
...
2. Restart gateway: `pkill -9 -f 'kibo.*gateway' && pnpm kibo gateway`
3. Enable the hook: `kibo hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://github.com/Arjgorithmic/openclaw/automation/hooks
