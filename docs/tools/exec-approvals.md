...
`ask: "always"` in `~/.kibo/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `kibo approvals get`, `kibo approvals get --gateway`, or
`kibo approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `kibo exec-policy show` exposes the same merged view and
`kibo exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`kibo exec-policy show` reports that scope as node-managed at runtime instead of
...
- **gateway host** → `kibo` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Kibo also tries to bind
...
- requested exec policy in Kibo config (`tools.exec.*`)
- host-local approvals policy in `~/.kibo/exec-approvals.json`
...
- In YOLO mode, Kibo does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
kibo config set tools.exec.host gateway
kibo config set tools.exec.security full
kibo config set tools.exec.ask off
...
- `kibo exec-policy` does not synchronize node approvals
- `kibo exec-policy set --host node` is rejected
- node exec approvals are fetched from the node at runtime, so node-targeted updates must use `kibo approvals --node ...`
...
When `tools.exec.strictInlineEval=true`, Kibo treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.kibo/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `kibo approvals allowlist ...`).
- `kibo security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `kibo doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Kibo still rejects the `env` builtin in safe-bin
...
`~/.kibo/exec-approvals.json` directly.

CLI: `kibo approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, Kibo sends a followup `agent` turn to the same session.
...
topics, Kibo preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Kibo prevents the agent from reusing
