...
import type { ApiContributor, Entry, MapConfig, User } from "./update-kibotributors.types.js";

const REPO = "kibo/kibo";
const PER_LINE = 10;

const mapPath = resolve("scripts/kibotributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing kibotributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README kibotributors: ${entries.length} entries`);
