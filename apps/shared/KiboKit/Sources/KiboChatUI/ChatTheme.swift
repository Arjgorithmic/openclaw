...
enum KiboPlatformImageFactory {
    static func image(_ image: KiboPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
