...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/kibo-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = KiboConfig>(
  overrides: T = {} as T,
): KiboConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): KiboConfig {
