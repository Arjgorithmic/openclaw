...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.KIBO_CONFIG_PATH) {
    configPaths.add(process.env.KIBO_CONFIG_PATH);
  }
  if (process.env.KIBO_STATE_DIR) {
    configPaths.add(path.join(process.env.KIBO_STATE_DIR, "kibo.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.KIBO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "kibo-test-no-bundled-extensions")
    : "kibo-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".kibo-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".kibo-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".kibo-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>kibo-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__kiboTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__kiboTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["KIBO_GATEWAY_TOKEN"]);
  const prev = process.env.KIBO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.KIBO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.KIBO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.KIBO_GATEWAY_PASSWORD;
