...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `kibo sandbox explain`.
...
Legacy `agent.*` configs are migrated by `kibo doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=kibo-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${KIBO_STATE_DIR:-$HOME/.kibo}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
