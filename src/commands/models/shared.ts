...
export async function loadValidConfigOrThrow(): Promise<KiboConfig> {
...
export async function updateConfig(
  mutator: (cfg: KiboConfig) => KiboConfig,
): Promise<KiboConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: KiboConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: KiboConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("kibo agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
