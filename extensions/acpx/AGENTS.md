...
The bundled ACPX extension is a thin Kibo wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `kibo/acpx`, not in this extension.
...
Use this flow when Kibo needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `kibo/acpx` repo first.
2. In Kibo, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Kibo and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Kibo ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Kibo-specific glue, move it to `kibo/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
