---
summary: "CLI reference for `kibo logs` (tail gateway logs via RPC)"
read_when:
...
`kibo logs` also accepts the standard Gateway client flags:
...
kibo logs
kibo logs --follow
kibo logs --follow --interval 2000
kibo logs --limit 500 --max-bytes 500000
kibo logs --json
kibo logs --plain
kibo logs --no-color
kibo logs --limit 500
kibo logs --local-time
kibo logs --follow --local-time
kibo logs --url ws://127.0.0.1:18789 --token "$KIBO_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `kibo logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
