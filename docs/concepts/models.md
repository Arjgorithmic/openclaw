...
Kibo selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Kibo can use (plus aliases).
...
- If you omit the provider, Kibo resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, Kibo
...
kibo models list
kibo models status
kibo models set <provider/model>
kibo models set-image <provider/model>

kibo models aliases list
kibo models aliases add <alias> <provider/model>
kibo models aliases remove <alias>

kibo models fallbacks list
kibo models fallbacks add <provider/model>
kibo models fallbacks remove <provider/model>
kibo models fallbacks clear

kibo models image-fallbacks list
kibo models image-fallbacks add <provider/model>
kibo models image-fallbacks remove <provider/model>
kibo models image-fallbacks clear
```

`kibo models` (no subcommand) is a shortcut for `models status`.
...
`kibo models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.kibo/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Kibo writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Kibo regenerates `models.json`, including command-driven paths like `kibo agent`.
