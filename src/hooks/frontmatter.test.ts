...
`;
    ...
    expect(parsed.kibo.emoji).toBe("💾");
    expect(parsed.kibo.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.kibo.emoji).toBe("📝");
    expect(parsed.kibo.events).toEqual(["command"]);
    expect(parsed.kibo.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.kibo.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"kibo": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"kibo": {"events": ["test"]}}');
  });
...
describe("resolveKiboMetadata", () => {
  it("extracts kibo metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveKiboMetadata(frontmatter);
  ...
  it("returns undefined when kibo key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        kibo: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Kibo" },
            { id: "npm", kind: "npm", package: "@kibo/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@kibo/hook");
  });
    ...
    "kibo":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Kibo" }],
      },
  }
---
...
`;
    ...
    const kibo = resolveKiboMetadata(frontmatter);
    expect(kibo).toBeDefined();
    expect(kibo?.emoji).toBe("💾");
    expect(kibo?.events).toEqual(["command:new", "command:reset"]);
    expect(kibo?.requires?.config).toEqual(["workspace.dir"]);
    expect(kibo?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const kibo = resolveKiboMetadata(frontmatter);
    expect(kibo?.emoji).toBe("disk");
    expect(kibo?.events).toEqual(["command:new"]);
  });
});
