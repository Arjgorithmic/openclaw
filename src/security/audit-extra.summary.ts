...
function summarizeGroupPolicy(cfg: KiboConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: KiboConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: KiboConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: KiboConfig): boolean {
...
function isBrowserEnabled(cfg: KiboConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: KiboConfig): SecurityAuditFinding[] {
