...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "KiboKit", "Tools", "CanvasA2UI");
const inputPaths = [
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.KIBO_SPARSE_PROFILE || process.env.KIBO_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because KIBO_A2UI_SKIP_MISSING=1 or KIBO_SPARSE_PROFILE is set.",
      );
      return;
    }
