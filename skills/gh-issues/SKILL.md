...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Kibo. Pass it as a Bearer token in all API calls:
...
If still empty, check `/data/.kibobot/kibo.json`:

```
cat /data/.kibobot/kibo.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Kibo dashboard or in the active Kibo config path (`$KIBO_CONFIG_PATH`, default `~/.kibo/kibo.json`) under `skills.entries.gh-issues`."
...
7. **Check claim-based in-progress tracking:**
   ...
   ```
   CLAIMS_FILE="/data/.kibobot/gh-issues-claims.json"
   if [ ! -f "$CLAIMS_FILE" ]; then
     mkdir -p /data/.kibobot
     echo '{}' > "$CLAIMS_FILE"
   fi
   ```
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  ```
  CURSOR_FILE="/data/.kibobot/gh-issues-cursor-{SOURCE_REPO_SLUG}.json"
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., kibo-kibo)
  ```
...
CONFIG_PATH="${KIBO_CONFIG_PATH:-${KIBO_STATE_DIR:-$HOME/.kibo}/kibo.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.kibobot/kibo.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.kibobot/kibo.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${KIBO_CONFIG_PATH:-${KIBO_STATE_DIR:-$HOME/.kibo}/kibo.json}"
