...
service via the external `kibo` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.kibo.gateway`
  (or `ai.kibo.<profile>` when using `--profile`/`KIBO_PROFILE`; legacy `com.kibo.*` is supported).
