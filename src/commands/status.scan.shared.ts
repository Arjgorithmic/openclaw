...
export function hasExplicitMemorySearchConfig(cfg: KiboConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: KiboConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: KiboConfig, agentId: string) => { store: { path: string } } | null;
