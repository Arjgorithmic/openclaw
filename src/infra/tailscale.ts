...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.KIBO_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Kibo. You can keep running the web gateway without it: \`${formatCliCommand("kibo gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
