...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`kibo pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`kibo pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends KiboConfig = KiboConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends KiboConfig = KiboConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends KiboConfig = KiboConfig>(
...
function setTopLevelChannelEnabledInConfigSection<Config extends KiboConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends KiboConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends KiboConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
