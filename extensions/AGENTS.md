...
- Extension production code should import from `kibo/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `kibo.plugin.json` and the package
  `kibo` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `kibo/plugin-sdk/*`.
