...
SMOKE_PREVIOUS_VERSION="${KIBO_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${KIBO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="kibo"
PACKAGE_NAME="${KIBO_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (KIBO_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${KIBO_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${KIBO_INSTALL_LATEST_OUT:-}"
fi
