...
summary: "Create your first Kibo plugin in minutes"
read_when:
  - You want to create a new Kibo plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to Kibo
---
...
You do not need to add your plugin to the Kibo repository. Publish to
[KiboHub](/tools/kibohub) or npm and users install with
`kibo plugins install <package-name>`. Kibo tries KiboHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Kibo to a messaging platform (Discord, IRC, etc.)
  </Card>
...
`kibo/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json kibo.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to Kibo",
      "configSchema": {
        "type": "object",
    ...
    [Manifest](/plugins/manifest) for the full schema. The canonical KiboHub
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with KiboHub, then install:

    ```bash
    kibohub package publish your-org/your-plugin --dry-run
    kibohub package publish your-org/your-plugin
    kibo plugins install kibohub:@myorg/kibo-my-plugin
    ```

    Kibo also checks KiboHub before npm for bare package specs like
    `@myorg/kibo-my-plugin`.
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, Kibo retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `kibo/plugin-sdk/error-runtime`
...
Always import from focused `kibo/plugin-sdk/<subpath>` paths:
...
package-root seam instead of promoting it into `kibo/plugin-sdk/*`.

Some generated `kibo/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `kibo` metadata</Check>
<Check>**kibo.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [kibo/kibo](https://github.com/kibo/kibo/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Kibo X account [@kibo](https://x.com/kibo) for release announcements.
