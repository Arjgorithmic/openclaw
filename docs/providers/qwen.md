---
summary: "Use Qwen Cloud via Kibo's bundled qwen provider"
read_when:
  - You want to use Qwen with Kibo
...
See [Issue #49557](https://github.com/kibo/kibo/issues/49557) for
background.
...
Kibo now treats Qwen as a first-class bundled provider with canonical id
...
kibo onboard --auth-choice qwen-api-key

# China Coding Plan endpoint
kibo onboard --auth-choice qwen-api-key-cn

# Global Standard (pay-as-you-go) endpoint
kibo onboard --auth-choice qwen-standard-api-key

# China Standard (pay-as-you-go) endpoint
kibo onboard --auth-choice qwen-standard-api-key-cn
```
...
shared `openai-completions` transport. Kibo keys that off endpoint
...
Kibo currently ships this bundled Qwen catalog. The configured catalog is
...
For video generation, Kibo maps the configured Qwen region to the matching
...
available to that process (for example, in `~/.kibo/.env` or via
`env.shellEnv`).
