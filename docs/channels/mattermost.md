---
summary: "Mattermost bot setup and Kibo config"
read_when:
...
Mattermost ships as a bundled plugin in current Kibo releases, so normal
...
```bash
kibo plugins install @kibo/mattermost
```

Local checkout (when running from a git repo):

```bash
kibo plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Kibo and start the gateway.
...
Native slash commands are opt-in. When enabled, Kibo registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Kibo derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when Kibo registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Kibo.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Kibo.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Kibo, not `404`.
...
- Approve via:
  - `kibo pairing list mattermost`
  - `kibo pairing approve mattermost <CODE>`
...
Use these target formats with `kibo message send` or cron/webhooks:
...
Kibo resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Kibo sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Kibo sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Kibo derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Kibo run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="kibo-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"kibo-mattermost-interactions",
...
`kibo message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: Kibo
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as Kibo. Set an
