...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("KIBO_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("KIBO_WIDE_AREA_DOMAIN", "kibo.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("KIBO_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("KIBO_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._kibo-gw._tcp.kibo.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.kibo.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Kibo\\226\\128\\153s Mac Studio (Kibo)\" \"gatewayPort=18789\" \"tailnetDns=kibos-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/kibo/kibo/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Kibo\u{2019}s Mac Studio (Kibo)"
        #expect(beacon.displayName == expectedDisplay)
        #expect(beacon.port == 18789)
        #expect(beacon.gatewayPort == 18789)
        #expect(beacon.tailnetDns == "kibos-mac-studio-1.sheep-coho.ts.net")
        #expect(beacon.cliPath == "/Users/kibo/kibo/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._kibo-gw._tcp.kibo.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
