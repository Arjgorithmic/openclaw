...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      KIBO_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when KIBO_THEME=light", async () => {
    const mod = await importThemeWithEnv({ KIBO_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when KIBO_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ KIBO_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats KIBO_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ KIBO_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("KIBO_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      KIBO_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ KIBO_THEME: "light" });
    const darkMod = await importThemeWithEnv({ KIBO_THEME: "dark" });
