...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Kibo tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideKibo = path.join(os.tmpdir(), "outside-kibo", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
