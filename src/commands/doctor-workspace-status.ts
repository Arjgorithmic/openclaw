...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("kibo tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("kibo tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: KiboConfig) {
