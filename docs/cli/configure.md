---
summary: "CLI reference for `kibo configure` (interactive configuration prompts)"
read_when:
...
Tip: `kibo config` without a subcommand opens the same wizard. Use
`kibo config get|set|unset` for non-interactive edits.

For web search, `kibo configure --section web` lets you choose a provider
...
kibo configure --section model --section channels
kibo configure --section gateway --section daemon
```
