...
```json
{
  "display_information": {
    "name": "Kibo",
    "description": "Slack connector for Kibo"
  },
  "features": {
    "bot_user": {
      "display_name": "Kibo",
    ...
    "slash_commands": [
      {
        "command": "/kibo",
        "description": "Send a message to Kibo",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `kibo pairing approve slack <code>`.
...
- If streaming fails mid-reply, Kibo falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Kibo is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are Kibo-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Kibo falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `kibo channels status --probe --json` shows `botTokenStatus` or
