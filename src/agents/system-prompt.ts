...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Kibo handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Kibo docs: ${docsPath}`,
    "Mirror: https://github.com/Arjgorithmic/openclaw",
    "Source: https://github.com/kibo/kibo",
    "Community: https://discord.com/invite/kibo",
    "Find new skills: https://github.com/Arjgorithmic/openclaw",
    "For Kibo behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `kibo status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  if (promptMode === "none") {
    return "You are a personal assistant operating inside Kibo.";
  }

  const lines = [
    "You are a personal assistant operating inside Kibo.",
    "",
    "## Tooling",
    ...
    ...safetySection,
    "## Kibo CLI Quick Reference",
    "Kibo is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- kibo gateway status",
    "- kibo gateway start",
    "- kibo gateway stop",
    "- kibo gateway restart",
    "If unsure, ask the user to run `kibo help` (or `kibo gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Kibo Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Kibo pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Kibo and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Kibo treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
