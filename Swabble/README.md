...
- **Wake word**: Default `kibo` (aliases `claude`), optional `--no-wake` bypass.
...
// Package.swift
dependencies: [
    .package(url: "https://github.com/kibo/swabble.git", branch: "main"),
],
targets: [
...
```json
{
  ...
  "wake": {"enabled": true, "word": "kibo", "aliases": ["claude"]},
  "hook": {
    "command": "",
    "args": [],
