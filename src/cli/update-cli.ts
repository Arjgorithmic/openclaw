...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["kibo update", "Update a source checkout (git)"],
        ["kibo update --channel beta", "Switch to beta channel (git + npm)"],
        ["kibo update --channel dev", "Switch to dev channel (git + npm)"],
        ["kibo update --tag beta", "One-off update to a dist-tag or version"],
        ["kibo update --tag main", "One-off package install from GitHub main"],
        ["kibo update --dry-run", "Preview actions without changing anything"],
        ["kibo update --no-restart", "Update without restarting the service"],
        ["kibo update --json", "Output result as JSON"],
        ["kibo update --yes", "Non-interactive (accept downgrade prompts)"],
        ["kibo update wizard", "Interactive update wizard"],
        ["kibo --update", "Shorthand for kibo update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run kibo update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.kibo.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kibo update status", "Show channel + version status."],
          ["kibo update status --json", "JSON output."],
          ["kibo update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.kibo.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
