...
function setConfiguredFetchProviderKey(configTarget: KiboConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: KiboConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: KiboConfig, provider: ProviderUnderTest): unknown {
