...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "kibo-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "kibo-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "kibo-bundled");
      const plugin = makeHookEntry("shared", "kibo-plugin");
      const managed = makeHookEntry("shared", "kibo-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("kibo-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "kibo-managed");
      const workspace = makeHookEntry("shared", "kibo-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "kibo-workspace");
      const second = makeHookEntry("shared", "kibo-workspace");
      second.hook.handlerPath = "/tmp/kibo-workspace/shared/handler-2.js";
