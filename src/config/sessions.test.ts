...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ KIBO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-kibo",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-kibo#general");
  });
  ...
  it("derives session transcripts dir from KIBO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { KIBO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when KIBO_STATE_DIR differs from stored paths", () => {
