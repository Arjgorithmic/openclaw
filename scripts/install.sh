...
# Kibo Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://kibo.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Kibo."
...
GUM_VERSION="${KIBO_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Kibo Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://kibo.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$KIBO_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.kibo.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_kibo_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.kibo-* "$npm_root"/kibo 2>/dev/null || true
}

extract_kibo_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_kibo_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/kibo" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/kibo"|"/usr/local/bin/kibo")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/kibo/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale kibo symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing kibo binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Kibo package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_kibo_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Kibo npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*kibo" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_kibo_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_kibo_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Kibo npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an kibo binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("I'll butter your workflow like a shell roll: messy, delicious, effective.")
...
pick_tagline() {
    ...
    if [[ -n "${KIBO_TAGLINE_INDEX:-}" ]]; then
        if [[ "${KIBO_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((KIBO_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${KIBO_NO_ONBOARD:-0}
NO_PROMPT=${KIBO_NO_PROMPT:-0}
DRY_RUN=${KIBO_DRY_RUN:-0}
INSTALL_METHOD=${KIBO_INSTALL_METHOD:-}
KIBO_VERSION=${KIBO_VERSION:-latest}
USE_BETA=${KIBO_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/kibo"
GIT_DIR=${KIBO_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${KIBO_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${KIBO_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${KIBO_VERBOSE:-0}"
VERIFY_INSTALL="${KIBO_VERIFY_INSTALL:-0}"
KIBO_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Kibo installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://kibo.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/kibo)
...
Environment variables:
  KIBO_INSTALL_METHOD=git|npm
  KIBO_VERSION=latest|next|main|<semver>|<spec>
  KIBO_BETA=0|1
  KIBO_GIT_DIR=...
  KIBO_GIT_UPDATE=0|1
  KIBO_NO_PROMPT=1
  KIBO_VERIFY_INSTALL=1
  KIBO_DRY_RUN=1
  KIBO_NO_ONBOARD=1
  KIBO_VERBOSE=1
  KIBO_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://kibo.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://kibo.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://kibo.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://kibo.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://kibo.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Kibo checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Kibo source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_kibo_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"kibo"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://kibo.ai/install.sh | bash"
}
...
ensure_kibo_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/kibo" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/kibo" ]]; then
        ln -sf "$npm_root/kibo/dist/entry.js" "${npm_bin}/kibo"
        ui_info "Created kibo bin link at ${npm_bin}/kibo"
    fi
    return 0
}

# Check for existing Kibo installation
check_existing_kibo() {
    if [[ -n "$(type -P kibo 2>/dev/null || true)" ]]; then
        ui_info "Existing Kibo installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make kibo show as \"command not found\" in new terminals."
...
warn_kibo_not_found() {
    ui_warn "Installed, but kibo is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t kibo 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named kibo; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_kibo_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P kibo 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/kibo" ]]; then
        echo "${npm_bin}/kibo"
        return 0
    fi
...
install_kibo_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/kibo/kibo.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Kibo from git checkout: ${repo_dir}"
    else
        ui_info "Installing Kibo from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Kibo" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Kibo" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/kibo" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/kibo"
    ui_success "Kibo wrapper installed to \$HOME/.local/bin/kibo"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view kibo dist-tags.beta 2>/dev/null || true)"
...
install_kibo() {
    local package_name="kibo"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            KIBO_VERSION="$beta_version"
    ...
    if [[ -z "${KIBO_VERSION}" ]]; then
        KIBO_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${KIBO_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${KIBO_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Kibo v${resolved_version}"
    else
        ui_info "Installing Kibo (${KIBO_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${KIBO_VERSION}")"

    if ! install_kibo_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_kibo_paths
        install_kibo_npm "${install_spec}"
    fi

    if [[ "${KIBO_VERSION}" == "latest" && "${package_name}" == "kibo" ]]; then
        if ! resolve_kibo_bin &> /dev/null; then
            ui_warn "npm install kibo@latest failed; retrying kibo@next"
            cleanup_npm_kibo_paths
            install_kibo_npm "kibo@next"
        fi
    fi

    ensure_kibo_bin_link || true

    ui_success "Kibo installed"
}
...
run_doctor() {
    ...
    local claw="${KIBO_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_kibo_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (kibo not on PATH yet)"
        warn_kibo_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${KIBO_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.kibo/workspace-${profile}"
    else
        echo "${HOME}/.kibo/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${KIBO_CONFIG_PATH:-$HOME/.kibo/kibo.json}"
    if [[ -f "${config_path}" || -f "$HOME/.kibobot/kibobot.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run kibo onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but kibo not on PATH; skipping onboarding"
        warn_kibo_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run kibo onboard to retry"
        return
    }
}
...
if ! declare -F extract_kibo_semver >/dev/null 2>&1; then
...
resolve_kibo_version() {
    local version=""
    local raw_version_output=""
    local claw="${KIBO_BIN:-}"
    if [[ -z "$claw" ]] && command -v kibo &> /dev/null; then
        claw="$(command -v kibo)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_kibo_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/kibo/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/kibo/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: kibo not on PATH yet"
        warn_kibo_not_found
        return 1
    fi

    run_quiet_step "Checking Kibo version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: kibo gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_kibo_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Kibo checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set KIBO_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    KIBO_BIN="$(resolve_kibo_bin || true)"
    ...
    installed_version=$(resolve_kibo_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Kibo installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Kibo installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            "Leveled up! New skills unlocked. You're welcome."
            "Fresh code, same shell. Miss me?"
            ...
            "The shell has molted. Harder shell, sharper claws."
        ...
        local completion_messages=(
            ...
            "The shell has landed. Your terminal will never be the same."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/kibo"
        ui_kv "Update command" "kibo update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://kibo.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running kibo doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                KIBO_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                KIBO_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                KIBO_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run kibo doctor and kibo plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run kibo onboard later"
        else
            local config_path="${KIBO_CONFIG_PATH:-$HOME/.kibo/kibo.json}"
            if [[ -f "${config_path}" || -f "$HOME/.kibobot/kibobot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (kibo not on PATH yet)"
                    warn_kibo_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run kibo onboard to finish setup"
            return 0
        fi
    fi

    if command -v kibo &> /dev/null; then
        local claw="${KIBO_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_kibo_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (kibo daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if KIBO_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: kibo daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${KIBO_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
