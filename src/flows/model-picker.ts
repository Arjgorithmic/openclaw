...
export type PromptDefaultModelResult = { model?: string; config?: KiboConfig };
...
function resolveConfiguredModelRaw(cfg: KiboConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: KiboConfig): string[] {
...
export function applyModelAllowlist(cfg: KiboConfig, models: string[]): KiboConfig {
