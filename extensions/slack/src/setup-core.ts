...
import {
  ...
  type KiboConfig,
} from "kibo/plugin-sdk/setup-runtime";
...
function enableSlackAccount(cfg: KiboConfig, accountId: string): KiboConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: KiboConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: KiboConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: KiboConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: KiboConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: KiboConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: KiboConfig; accountId: string }) =>
    ...
    disable: (cfg: KiboConfig) => setSetupChannelEnabled(cfg, channel, false),
