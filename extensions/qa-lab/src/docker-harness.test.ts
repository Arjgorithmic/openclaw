...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "kibo.json"),
    ...
    expect(compose).toContain('      - "http://kibo-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/kibo-qa-lab-ui"');
    expect(compose).toContain(":/opt/kibo-repo:ro");
    expect(compose).toContain("./state:/opt/kibo-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/kibo-scaffold/seed-workspace/. /tmp/kibo/workspace/",
    );
    expect(compose).toContain("KIBO_CONFIG_PATH: /tmp/kibo/kibo.json");
    expect(compose).toContain("KIBO_STATE_DIR: /tmp/kibo/state");
    expect(compose).toContain('KIBO_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("KIBO_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "kibo.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/kibo/workspace"');

    const kickoff = await readFile(
      path.join(outputDir, "state", "seed-workspace", "QA_KICKOFF_TASK.md"),
      "utf8",
    );
    expect(kickoff).toContain("Kibo Shell Invaders");
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/kibo",
        imageName: "kibo:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("kibo:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t kibo:qa-local-prebaked --build-arg KIBO_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/kibo",
      ),
    ]);
  });
});
