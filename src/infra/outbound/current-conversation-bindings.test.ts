...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.KIBO_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-current-bindings-"));
    process.env.KIBO_STATE_DIR = testStateDir;
