...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.memory-wiki.config.bridge.readMemoryCore is legacy; use plugins.entries.memory-wiki.config.bridge.readMemoryArtifacts. Run "kibo doctor --fix".',
    match: hasLegacyBridgeArtifactToggle,
  },
];

export function migrateMemoryWikiLegacyConfig(config: KiboConfig): {
  config: KiboConfig;
  changes: string[];
} | null {
...
export function normalizeCompatibilityConfig({ cfg }: { cfg: KiboConfig }): {
  config: KiboConfig;
  changes: string[];
} {
