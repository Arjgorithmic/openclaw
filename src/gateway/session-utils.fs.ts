...
export function attachKiboTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__kibo && typeof record.__kibo === "object" && !Array.isArray(record.__kibo)
      ? (record.__kibo as Record<string, unknown>)
      : {};
  return {
    ...record,
    __kibo: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachKiboTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "kibo" && model === "delivery-mirror";
