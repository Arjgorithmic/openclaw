...
describe("image-ops temp dir", () => {
  ...
  beforeEach(() => {
    process.env.KIBO_IMAGE_BACKEND = "sips";
  ...
  afterEach(() => {
    delete process.env.KIBO_IMAGE_BACKEND;
    vi.restoreAllMocks();
  });

  it("creates sips temp dirs under the secured Kibo tmp root", async () => {
    const secureRoot = resolvePreferredKiboTmpDir();

    await getImageMetadata(Buffer.from("image"));

    expect(fs.mkdtemp).toHaveBeenCalledTimes(1);
    expect(fs.mkdtemp).toHaveBeenCalledWith(path.join(secureRoot, "kibo-img-"));
    expect(createdTempDir.startsWith(path.join(secureRoot, "kibo-img-"))).toBe(true);
