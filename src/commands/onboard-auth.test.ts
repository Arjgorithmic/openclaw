...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under KIBO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("kibo-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-oauth-sync-"));
    ...
    process.env.KIBO_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-oauth-nosync-"));
    process.env.KIBO_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside KIBO_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kibo-oauth-external-"));
    process.env.KIBO_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* KIBO_STATE_DIR
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to KIBO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("kibo-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
