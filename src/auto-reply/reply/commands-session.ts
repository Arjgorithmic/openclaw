...
import { scheduleGatewaySigusr1Restart, triggerKiboRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Kibo in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerKiboRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Kibo via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
