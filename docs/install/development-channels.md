...
Kibo ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/kibo`, override with
  `KIBO_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
kibo update --tag 2026.4.1-beta.1
...
kibo update --tag kibo@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `kibo update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Kibo prompts for confirmation (skip with `--yes`).
...
Preview what `kibo update` would do without making changes:

```bash
kibo update --dry-run
kibo update --channel beta --dry-run
kibo update --tag 2026.4.1-beta.1 --dry-run
kibo update --dry-run --json
```
...
When you switch channels with `kibo update`, Kibo also syncs plugin
sources:
