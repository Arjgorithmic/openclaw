...
import { createKiboTools } from "./kibo-tools.js";
...
function resolveExecConfig(params: { cfg?: KiboConfig; agentId?: string }) {
...
export function createKiboCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createKiboReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
