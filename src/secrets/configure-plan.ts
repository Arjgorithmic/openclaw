...
export function buildConfigureCandidates(config: KiboConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: KiboConfig;
  authoredKiboConfig?: KiboConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredKiboConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const kiboCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...kiboCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "kibo secrets configure",
