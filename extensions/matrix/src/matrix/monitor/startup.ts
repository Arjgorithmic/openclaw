...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleKiboDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Kibo devices detected for ${params.auth.userId}: ${deviceHealth.staleKiboDevices.map((device) => device.deviceId).join(", ")}. Run 'kibo matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
