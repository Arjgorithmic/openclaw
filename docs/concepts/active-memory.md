...
With verbose enabled, Kibo can show:
...
`config.promptOverride` replaces the default Active Memory prompt. Kibo
...
5. Verify memory search itself works with `kibo memory status --deep`.
