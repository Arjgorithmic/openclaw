...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kibo security audit", "Run a local security audit."],
          ["kibo security audit --deep", "Include best-effort live Gateway probe checks."],
          ["kibo security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "kibo security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["kibo security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["kibo security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.kibo.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Kibo security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("kibo security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("kibo security audit --fix")}`));
        if (!fixResult) {
