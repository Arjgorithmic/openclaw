...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<KiboConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<KiboConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: KiboConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: KiboConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: KiboConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: KiboConfig, result: KiboConfig): KiboConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as KiboConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with kibo configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Kibo will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://github.com/Arjgorithmic/openclaw/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Kibo will store a reference instead of the API key.",
