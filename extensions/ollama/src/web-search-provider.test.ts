...
vi.mock("kibo/plugin-sdk/ssrf-runtime", () => ({
...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): KiboConfig {
  return {
    models: {
      providers: {
        ollama: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the Ollama experimental search endpoint", async () => {
      ...
      response: new Response(
        JSON.stringify({
          results: [
            {
              title: "Kibo",
              url: "https://github.com/Arjgorithmic/openclaw/docs",
    ...
    const result = await tool.execute({ query: "kibo docs", count: 3 });
    ...
    expect(result).toMatchObject({
      ...
      provider: "ollama",
      count: 1,
      results: [{ url: "https://github.com/Arjgorithmic/openclaw/docs" }],
    });
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest kibo release" })).rejects.toThrow(
      "ollama signin",
    );
  });
