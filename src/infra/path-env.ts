...
function candidateBinDirs(opts: EnsureKiboPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Kibo process is on.
  try {
  ...
  // Bundled macOS app: `kibo` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "kibo");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.KIBO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "kibo"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `kibo`
...
/**
 * Best-effort PATH bootstrap so skills that require the `kibo` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureKiboCliOnPath(opts: EnsureKiboPathOpts = {}) {
  if (isTruthyEnvValue(process.env.KIBO_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.KIBO_PATH_BOOTSTRAPPED = "1";
