---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Kibo deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Kibo cron scheduling for periodic checks, or version status checks on a machine running Kibo (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Kibo, then align it to a user-defined risk tolerance without breaking access. Use Kibo security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Kibo changes the host firewall, SSH, or OS updates; it does not.
...
5. Kibo gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Kibo can access sensitive data.
...
### 2) Run Kibo security audits (read-only)

As part of the default read-only checks, run `kibo security audit --deep`. Only offer alternatives if the user requests them:

1. `kibo security audit` (faster, non-probing)
2. `kibo security audit --json` (structured output)

Offer to apply Kibo safe defaults (numbered):

1. `kibo security audit --fix`

Be explicit that `--fix` only tightens Kibo defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Kibo version/update status (read-only)

As part of the default read-only checks, run `kibo update status`.
...
- Credential hygiene notes (location of Kibo creds, prefer disk encryption)
...
After Kibo install or first hardening pass, run at least one baseline audit and version check:

- `kibo security audit`
- `kibo security audit --deep`
- `kibo update status`

Ongoing monitoring is recommended. Use the Kibo cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `kibo cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `kibo update status`
...
Before creating, `kibo cron list` and match on exact `name`. If found, `kibo cron edit <id> ...`.
If not found, `kibo cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `kibo update status` (preferred for source checkouts and channels)
2. `npm view kibo version` (published npm version)

## Kibo command accuracy

Use only supported commands and flags:

- `kibo security audit [--deep] [--fix] [--json]`
- `kibo status` / `kibo status --deep`
- `kibo health --json`
- `kibo update status`
- `kibo cron add|list|runs|run`

Do not invent CLI flags or imply Kibo enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Kibo compaction:
