...
vi.mock("kibo/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("kibo/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
describe("probeMattermost", () => {
  ...
  it("normalizes base URL and returns bot info", async () => {
    mockFetchGuard.mockResolvedValueOnce({
      response: new Response(JSON.stringify({ id: "bot-1", username: "kibobot" }), {
        status: 200,
