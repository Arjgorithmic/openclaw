...
describe("canvas host state dir defaults", () => {
  ...
  it("uses KIBO_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("kibo-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Kibo Canvas");
      } finally {
        await handler.close();
      }
    });
