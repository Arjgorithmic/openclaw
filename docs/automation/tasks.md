...
- `kibo tasks list` shows all tasks; `kibo tasks audit` surfaces issues.
...
kibo tasks list --runtime acp
kibo tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
kibo tasks show <lookup>

# Cancel a running task (kills the child session)
kibo tasks cancel <lookup>

# Change notification policy for a task
kibo tasks notify <lookup> state_changes

# Run a health audit
kibo tasks audit

# Preview or apply maintenance
kibo tasks maintenance
kibo tasks maintenance --apply

# Inspect TaskFlow state
kibo tasks flow list
...
| CLI operations         | `cli`        | `kibo agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `kibo tasks cancel`                        |
...
When a task reaches a terminal state, Kibo notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, Kibo also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
```bash
kibo tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
kibo tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
kibo tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `kibo status` when issues are detected.
...
```bash
kibo tasks maintenance [--json]
kibo tasks maintenance --apply [--json]
```
...
```bash
kibo tasks flow list [--status <status>] [--json]
kibo tasks flow show <lookup> [--json]
kibo tasks flow cancel <lookup>
```
...
For the full operator ledger, use the CLI: `kibo tasks list`.

## Status integration (task pressure)

`kibo status` includes an at-a-glance task summary:
...
```
$KIBO_STATE_DIR/tasks/runs.sqlite
```
...
[Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `kibo tasks` to inspect individual task records and `kibo tasks flow` to inspect the orchestrating flow.

See [Task Flow](/automation/taskflow) for details.

### Tasks and cron

A cron job **definition** lives in `~/.kibo/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
