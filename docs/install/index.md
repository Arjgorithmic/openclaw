---
summary: "Install Kibo — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Kibo, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://github.com/Arjgorithmic/openclaw/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Kibo and Node kept under a local prefix such as
`~/.kibo`, without depending on a system-wide Node install:

```bash
curl -fsSL https://github.com/Arjgorithmic/openclaw/install-cli.sh | bash
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g kibo@latest
```
...
Or skip the link and use `pnpm kibo ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:kibo/kibo#main
```
...
```bash
kibo --version      # confirm the CLI is available
kibo doctor         # check for config issues
kibo gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `kibo onboard --install-daemon` or `kibo gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Kibo on a cloud server or VPS:
...
<CardGroup cols={3}>
  <Card title="Updating" href="/install/updating" icon="refresh-cw">
    Keep Kibo up to date.
  </Card>
  <Card title="Migrating" href="/install/migrating" icon="arrow-right">
    Move to a new machine.
  </Card>
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Kibo completely.
  </Card>
</CardGroup>

## Troubleshooting: `kibo` not found

If the install succeeded but `kibo` is not found in your terminal:
