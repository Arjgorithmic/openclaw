...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseKiboManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyKiboManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveKiboMetadata(
  frontmatter: ParsedSkillFrontmatter,
): KiboSkillMetadata | undefined {
  const metadataObj = resolveKiboManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveKiboManifestRequires(metadataObj);
  const install = resolveKiboManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveKiboManifestOs(metadataObj);
  return {
