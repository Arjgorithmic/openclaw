---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Kibo agents"
read_when:
...
OpenShell is a managed sandbox backend for Kibo. Instead of running Docker
containers locally, Kibo delegates sandbox lifecycle to the `openshell` CLI,
...
- Kibo Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Kibo creates an OpenShell
...
- You edit files locally outside Kibo and want those changes visible in the
...
- When the sandbox is first created, Kibo seeds the remote workspace from
...
- Kibo does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Kibo after the initial seed,
the remote sandbox does **not** see those changes. Use
`kibo sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"kibo"`  | Sandbox source for first-time create                  |
...
1. Kibo calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Kibo calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `kibo sandbox` commands
