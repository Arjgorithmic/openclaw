---
summary: "Expose Kibo channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Kibo-backed channels
  - Running `kibo mcp serve`
  - Managing Kibo-saved MCP server definitions
title: "mcp"
---

# mcp

`kibo mcp` has two jobs:

- run Kibo as an MCP server with `kibo mcp serve`
- manage Kibo-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Kibo acting as an MCP server
- `list` / `show` / `set` / `unset` is Kibo acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`kibo acp`](/cli/acp) when Kibo should host a coding harness
session itself and route that runtime through ACP.

## Kibo as an MCP server

This is the `kibo mcp serve` path.

## When to use `serve`

Use `kibo mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Kibo-backed channel conversations
- you already have a local or remote Kibo Gateway with routed sessions
- you want one MCP server that works across Kibo's channel backends instead
  of running separate per-channel bridges

Use [`kibo acp`](/cli/acp) instead when Kibo should host the coding
runtime itself and keep the agent session inside Kibo.

## How it works

`kibo mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Kibo Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `kibo mcp serve`
...
conversations. A conversation appears when Kibo already has session state
...
# Remote Gateway
kibo mcp serve --url wss://gateway-host:18789 --token-file ~/.kibo/gateway.token

# Remote Gateway with password auth
kibo mcp serve --url wss://gateway-host:18789 --password-file ~/.kibo/gateway.password

# Enable verbose bridge logs
kibo mcp serve --verbose

# Disable Claude-specific push notifications
kibo mcp serve --claude-channel-mode off
```
...
Kibo equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Kibo channel configuration
...
Kibo ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `kibo mcp serve`
...
## Kibo as an MCP client registry

This is the `kibo mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Kibo over MCP. They manage Kibo-owned MCP
server definitions under `mcp.servers` in Kibo config.

Those saved definitions are for runtimes that Kibo launches or configures
later, such as embedded Pi and other runtime adapters. Kibo stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Kibo config
...
Kibo also stores a lightweight MCP server registry in config for surfaces
that want Kibo-managed MCP definitions.

Commands:

- `kibo mcp list`
- `kibo mcp show [name]`
- `kibo mcp set <name> <json>`
- `kibo mcp unset <name>`
...
```bash
kibo mcp list
kibo mcp show context7 --json
kibo mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
kibo mcp set docs '{"url":"https://mcp.example.com"}'
kibo mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Kibo uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
