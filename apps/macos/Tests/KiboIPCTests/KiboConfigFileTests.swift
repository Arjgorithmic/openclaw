...
@Suite(.serialized)
struct KiboConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("kibo-config-\(UUID().uuidString)")
            .appendingPathComponent("kibo.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["KIBO_CONFIG_PATH": override]) {
            #expect(KiboConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["KIBO_CONFIG_PATH": override]) {
            ...
            #expect(KiboConfigFile.remoteGatewayPort() == 19999)
            #expect(KiboConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(KiboConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(KiboConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["KIBO_CONFIG_PATH": override]) {
            ...
            KiboConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = KiboConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["KIBO_CONFIG_PATH": override]) {
            ...
            KiboConfigFile.clearRemoteGatewayUrl()
            let root = KiboConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("kibo-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "KIBO_CONFIG_PATH": nil,
            "KIBO_STATE_DIR": dir,
        ]) {
            #expect(KiboConfigFile.stateDirURL().path == dir)
            #expect(KiboConfigFile.url().path == "\(dir)/kibo.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("kibo-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("kibo.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "KIBO_STATE_DIR": stateDir.path,
            "KIBO_CONFIG_PATH": configPath.path,
        ]) {
            KiboConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-kibo-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = KiboConfigFile.loadDict()
