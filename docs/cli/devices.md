---
summary: "CLI reference for `kibo devices` (device pairing + token rotation/revocation)"
read_when:
...
```
kibo devices remove <deviceId>
kibo devices remove <deviceId> --json
```

### `kibo devices clear --yes [--pending]`

Clear paired devices in bulk.

```
kibo devices clear --yes
kibo devices clear --yes --pending
kibo devices clear --yes --pending --json
```

### `kibo devices approve [requestId] [--latest]`

Approve a pending device pairing request. If `requestId` is omitted, Kibo
automatically approves the most recent pending request.

Note: if a device retries pairing with changed auth details (role/scopes/public
key), Kibo supersedes the previous pending entry and issues a new
`requestId`. Run `kibo devices list` right before approval to use the
current ID.

```
kibo devices approve
...
### `kibo devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
kibo devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```

Returns the new token payload as JSON.

### `kibo devices revoke --device <id> --role <role>`
...
```
kibo devices revoke --device <deviceId> --role node
```
...
```bash
kibo devices rotate --device <deviceId> --role operator
```
