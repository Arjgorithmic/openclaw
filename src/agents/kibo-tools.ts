...
import { resolveKiboPluginToolsForOptions } from "./kibo-plugin-tools.js";
import {
  collectPresentKiboTools,
  isUpdatePlanToolEnabledForKiboTools,
} from "./kibo-tools.registration.js";
...
const defaultKiboToolsDeps: KiboToolsDeps = {
  callGateway,
};

let kiboToolsDeps: KiboToolsDeps = defaultKiboToolsDeps;

export function createKiboTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? kiboToolsDeps.config;
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentKiboTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    createGatewayTool({
      agentSessionKey: options?.agentSessionKey,
      config: options?.config,
    }),
    ...
    ...(isUpdatePlanToolEnabledForKiboTools(resolvedConfig, options?.modelProvider)
    ...
    createSessionsListTool({
      ...
      sandboxed: options?.sandboxed,
      config: resolvedConfig,
      callGateway: kiboToolsDeps.callGateway,
    }),
    ...
    ...collectPresentKiboTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  ...
  const wrappedPluginTools = resolveKiboPluginToolsForOptions({
    options,
    resolvedConfig,
...
export const __testing = {
  setDepsForTest(overrides?: Partial<KiboToolsDeps>) {
    kiboToolsDeps = overrides
      ? {
          ...defaultKiboToolsDeps,
          ...overrides,
        }
      : defaultKiboToolsDeps;
  },
