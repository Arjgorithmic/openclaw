...
Designed to be called from workflow engines (for example, Kibo Shell via
`kibo.invoke --each`) without adding new Kibo code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Kibo Shell) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on Kibo internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Kibo extension (like `shell`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.kibo/extensions` as a standalone plugin directory.
